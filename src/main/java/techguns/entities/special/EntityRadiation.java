...
public class EntityRadiation extends Entity {
    ...
    public EntityRadiation(World worldIn, double posX, double posY, double posZ, int duration, float innter_radius,
                           int strength, float radius, int strength_outer) {
        super(worldIn);
        this.setPosition(posX, posY, posZ);
        this.setRadiationValues(duration, innter_radius, strength, radius, strength_outer);
    }

    public EntityRadiation(Entity otherEntPos, int duration, float innter_radius, int strength, float radius,
                           int strength_outer) {
    ...
    public EntityRadiation(TileEntity tilePos, int duration, float innter_radius, int strength, float radius,
                           int strength_outer) {
    ...
    public EntityRadiation setRadiationValues(int duration, float innter_radius, int strength, float radius,
                                              int strength_outer) {
    ...
    public void onEntityUpdate() {
        ...
        if (this.ticksExisted % RAD_INTERVAL == 0) {
            ...
            if (!this.world.isRemote) {
                ...
                TGRadiationSystem.applyRadToEntities(world, this.posX, this.posY, this.posZ, this.radius,
                        RAD_INTERVAL + 2, this.strength, this.inner_radius, this.strength_outer);
