...
public class Ghastling extends EntityMob implements IMob, INpcTGDamageSystem, INPCTechgunsShooter, ITGSpawnerNPC {
    ...
    private static final DataParameter<Boolean> ATTACKING = EntityDataManager.<Boolean>createKey(Ghastling.class,
    ...
    protected static class AIGhastlingAttack extends EntityAIBase {
        ...
        public void updateTask() {
            ...
            if (entitylivingbase.getDistanceSq(this.parentEntity) < 4096.0D &&
                 ...
                 * 
                 * TGPackets.network.sendToAllAround(new PacketPlaySound(TGSounds.HELICOPTER_BURST, this.parentEntity,
                 * 8.0f, 1.0f, false, false, TGSoundCategory.GUN_FIRE), TGPackets.targetPointAroundEnt(parentEntity,
                 * 100.0f));
                 * }
                 * 
                 * GenericProjectile bullet = new GenericProjectile(this.parentEntity.world, this.parentEntity,12.0f,
                 * 1.0f, 100, 0.05f, 30, 40, 8.0f, 0.25f,false,EnumBulletFirePos.CENTER);
                ...
                if (attackTimer == 15) {
                    ...
                    TGPackets.network.sendToAllAround(
                            new PacketPlaySound(TGSounds.NETHERBLASTER_FIRE, this.parentEntity, 1.0f, 1.0f, false,
                                    false, TGSoundCategory.GUN_FIRE),
                            TGPackets.targetPointAroundEnt(parentEntity, 100.0f));
                    // RocketProjectile rocket = new RocketProjectile(this.parentEntity.world, this.parentEntity,12.0f,
                    // 1.0f, 100, 0.05f, 30, 40, 8.0f,
                    // 0.25f,false,this.parentEntity.rand.nextBoolean()?EnumBulletFirePos.LEFT:EnumBulletFirePos.RIGHT,
                    // 4.0f, 0.0f);
                    AlienBlasterProjectile rocket = new AlienBlasterProjectile(this.parentEntity.world, parentEntity,
                            10, 1.5f, 200, 0.05f, 200, 200, 10, 0, false, EnumBulletFirePos.CENTER);
    ...
    protected static class AIFireballAttack extends EntityAIBase {
        ...
        public void updateTask() {
            ...
            if (d0 < 4.0D) {
                ...
                this.parentEntity.getMoveHelper().setMoveTo(entitylivingbase.posX, entitylivingbase.posY,
                ...
                double d2 = entitylivingbase.getEntityBoundingBox().minY + (double) (entitylivingbase.height / 2.0F) -
                        (this.parentEntity.posY + (double) (this.parentEntity.height / 2.0F));
                ...
                if (this.attackTime <= 0) {
                    ...
                    if (this.attackStep > 1) {
                        ...
                        this.parentEntity.world.playEvent((EntityPlayer) null, 1018,
                                new BlockPos((int) this.parentEntity.posX, (int) this.parentEntity.posY,
                                        (int) this.parentEntity.posZ),
                                0);

                        /*
                         * for (int i = 0; i < 1; ++i)
                         * {
                         * EntitySmallFireball entitysmallfireball = new EntitySmallFireball(this.parentEntity.world,
                         * this.parentEntity, d1 + this.parentEntity.getRNG().nextGaussian() * (double)f, d2, d3 +
                         * this.parentEntity.getRNG().nextGaussian() * (double)f);
                         * entitysmallfireball.posY = this.parentEntity.posY + (double)(this.parentEntity.height / 2.0F)
                         * + 0.5D;
                         * this.parentEntity.world.spawnEntity(entitysmallfireball);
                         * }
                         */
                        // TGPackets.network.sendToAllAround(new PacketPlaySound(TGSounds.NETHERBLASTER_FIRE,
                        // this.parentEntity, 1.0f, 1.0f, false, false, TGSoundCategory.GUN_FIRE),
                        // TGPackets.targetPointAroundEnt(parentEntity, 100.0f));
                        // RocketProjectile rocket = new RocketProjectile(this.parentEntity.world,
                        // this.parentEntity,12.0f, 1.0f, 100, 0.05f, 30, 40, 8.0f,
                        // 0.25f,false,this.parentEntity.rand.nextBoolean()?EnumBulletFirePos.LEFT:EnumBulletFirePos.RIGHT,
                        // 4.0f, 0.0f);
                        AlienBlasterProjectile rocket = new AlienBlasterProjectile(this.parentEntity.world,
                                parentEntity, 6, 1.5f, 200, 0.05f, 200, 200, 6, 0, false, EnumBulletFirePos.CENTER);
        ...
        private double getFollowDistance() {
            IAttributeInstance iattributeinstance = this.parentEntity
