...
public class AttackHelicopter extends GenericFlyingMob {
    ...
    private static final DataParameter<Boolean> ATTACKING = EntityDataManager.<Boolean>createKey(AttackHelicopter.class,
    ...
    protected void onDeathUpdate() {
        ...
        if (deathTime % 5 == 0) {
            ...
            // Techguns.proxy.spawnParticle("LargeSmoke", this.world, this.posX + (double)(this.rand.nextFloat() *
            // this.width * 2.0F) - (double)this.width, this.posY + (double)(this.rand.nextFloat() * this.height),
        ...
        if (deathTime % 6 == 0) {
            ...
            /*
             * Techguns.proxy.spawnParticle("Fireblast", this.world, this.posX+x, this.posY-yoffset, this.posZ+z, x*v*r,
             * r, z*v*r);
             * Techguns.proxy.spawnParticle("Fireblast", this.world, this.posX+x, this.posY-yoffset, this.posZ+z,
             * x*v*r*0.75f, r, z*v*r*0.75f);
             * Techguns.proxy.spawnParticle("Fireblast", this.world, this.posX+x, this.posY-yoffset, this.posZ+z,
             * x*v*r*1.25f, r, z*v*r*1.25f);
             */
        }

        if (this.deathTime == MAX_DEATH_TIME) {

            if (!this.world.isRemote && (this.isPlayer() ||
                    this.recentlyHit > 0 && this.canDropLoot() && this.world.getGameRules().getBoolean("doMobLoot"))) {
            ...
            // double d0 = this.rand.nextGaussian() * 0.02D;
            // double d1 = this.rand.nextGaussian() * 0.02D;
            // Techguns.proxy.spawnParticle("ExplosionHuge", this.world, this.posX + (double)(this.rand.nextFloat() *
            // this.width * 2.0F) - (double)this.width, this.posY + (double)(this.rand.nextFloat() * this.height*2.0f)
            // -10.f, this.posZ + (double)(this.rand.nextFloat() * this.width * 2.0F) - (double)this.width, d2, d0, d1);
            // Techguns.proxy.spawnParticle("LargeSmoke", this.world, this.posX + (double)(this.rand.nextFloat() *
            // this.width * 2.0F) - (double)this.width, this.posY + (double)(this.rand.nextFloat() * this.height*2.0f)
            // -10.f, this.posZ + (double)(this.rand.nextFloat() * this.width * 2.0F) - (double)this.width, d2, d0, d1);
            // }

            Techguns.proxy.createFX("HelicopterDeathExplosion", this.world, this.posX, this.posY - 8D, this.posZ, 0, 0,
                    0);
        }
    }
                ...
                if (this.attackTimer >= 14 && (this.attackTimer < 24) && (this.attackTimer % 2 == 0)) {
                    if (this.attackTimer == 14) {
                        ...
                        TGPackets.network.sendToAllAround(
                                new PacketPlaySound(TGSounds.HELICOPTER_BURST, this.parentEntity, 8.0f, 1.0f, false,
                    ...
                    TGPackets.network
                            .sendToAllAround(
                                    new PacketPlaySound(TGSounds.ROCKET_FIRE, this.parentEntity, 8.0f, 1.0f, false,
                                            false, TGSoundCategory.GUN_FIRE),
                                    TGPackets.targetPointAroundEnt(parentEntity, 100.0f));
                    RocketProjectile rocket = new RocketProjectile(this.parentEntity.world, this.parentEntity, 12.0f,
                            1.0f, 100, 0.05f, 30, 40, 8.0f, 0.25f, false,
                            this.parentEntity.rand.nextBoolean() ? EnumBulletFirePos.LEFT : EnumBulletFirePos.RIGHT,
                            4.0f, 0.0f);
