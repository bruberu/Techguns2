...
public class GuidedMissileProjectileHV extends GuidedMissileProjectile {
    ...
    public static class Factory implements IChargedProjectileFactory<GuidedMissileProjectileHV> {
        ...
        protected DamageModifier mod = new DamageModifier().setRadius(0.75f, 0f).setRange(0.75f, 0f).setVelocity(2.0f,
                0f);
        ...
        public GuidedMissileProjectileHV createProjectile(GenericGun gun, World world, EntityLivingBase p, float damage,
            ...
            if (p instanceof EntityPlayer) {
                ...
                if (epc.lockOnEntity != null &&
                        epc.lockOnTicks >= ((GenericGun) p.getActiveItemStack().getItem()).getLockOnTicks()) {
            ...
            if (target != null) {
                return new GuidedMissileProjectileHV(world, p, mod.getDamage(damage), mod.getVelocity(speed),
                        mod.getTTL(TTL), spread, mod.getRange(dmgDropStart), mod.getRange(dmgDropEnd),
                        mod.getDamage(dmgMin), penetration, blockdamage, firePos, mod.getRadius(radius), target);
            } else {
                return new GuidedMissileProjectileHV(world, p, mod.getDamage(damage), mod.getVelocity(speed),
                        mod.getTTL(TTL), spread, mod.getRange(dmgDropStart), mod.getRange(dmgDropEnd),
                        mod.getDamage(dmgMin), penetration, blockdamage, firePos, mod.getRadius(radius), 0.01f);
            }
        }
        ...
        public GuidedMissileProjectileHV createChargedProjectile(World world, EntityLivingBase p, float damage,
                                                                 ...
                                                                 float dmgDropEnd, float dmgMin,
                                                                 float penetration, boolean blockdamage,
                                                                 EnumBulletFirePos firePos, float radius,
                                                                 double gravity,
