...
public class RocketProjectile extends GenericProjectile implements ILightProvider {
    ...
    public RocketProjectile(World par2World, EntityLivingBase p, float damage, float speed, int TTL, float spread,
                            float dmgDropStart, float dmgDropEnd, float dmgMin,
                            float penetration, boolean blockdamage, EnumBulletFirePos leftGun, float radius,
                            double gravity) {
    ...
    public RocketProjectile(World worldIn, double posX, double posY, double posZ, float yaw, float pitch, float damage,
    ...
    protected void explodeRocket() {
        if (!this.world.isRemote) {
            ...
            // TGPackets.network.sendToAllAround(new PacketSpawnParticle("TestFX", this.posX,this.posY,this.posZ),
            // TGPackets.targetPointAroundEnt(this, 50.0f));

            // ProjectileExplosion explosion = new ProjectileExplosion(worldObj, this.posX, this.posY, this.posZ,
            // this.shooter, radius, (int)damage, radius*0.5f, radius*1.5f);
            // Explosion explosion = new Explosion(world, this,this.posX,this.posY, this.posZ, 5, blockdamage,
            // blockdamage);
            // explosion.doExplosionA();
            // explosion.doExplosionB(true);

            TGExplosion explosion = new TGExplosion(world, this.shooter, this, posX, posY, posZ, this.damage,
                    this.damageMin, this.damageDropStart, this.damageDropEnd, this.blockdamage ? 0.5 : 0.0);
    ...
    public static class Factory implements IProjectileFactory<RocketProjectile> {
        ...
        @Override
        public RocketProjectile createProjectile(GenericGun gun, World world, EntityLivingBase p, float damage,
                                                 float speed, int TTL, float spread, float dmgDropStart,
                                                 float dmgDropEnd,
                                                 float dmgMin, float penetration, boolean blockdamage,
                                                 EnumBulletFirePos firePos, float radius, double gravity) {
            return new RocketProjectile(world, p, damage, speed, TTL, spread, dmgDropStart, dmgDropEnd, dmgMin,
                    penetration, blockdamage, firePos, radius, gravity);
        }
