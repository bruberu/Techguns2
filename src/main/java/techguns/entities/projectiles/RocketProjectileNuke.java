...
public class RocketProjectileNuke extends RocketProjectile {
    ...
    public RocketProjectileNuke(World par2World, EntityLivingBase p, float damage, float speed, int TTL, float spread,
                                float dmgDropStart, float dmgDropEnd, float dmgMin, float penetration,
                                boolean blockdamage,
    ...
    protected void explodeRocket() {
        if (!this.world.isRemote) {
            TGPackets.network.sendToAllAround(new PacketSpawnParticle("NukeExplosion", this.posX, this.posY, this.posZ),
            ...
            this.world.playSound((EntityPlayer) null, this.posX, this.posY, this.posZ, TGSounds.NUKE_EXPLOSION,
                    SoundCategory.BLOCKS, 4.0F, 1.0F);

            /** DO RADIATION **/
            if (TGRadiationSystem.isEnabled()) {
                EntityRadiation rad = new EntityRadiation(this, 20 * 60, this.damageDropStart, 9, this.damageDropEnd,
                        2);
    ...
    public static class Factory implements IProjectileFactory<RocketProjectileNuke> {
        ...
        @Override
        public RocketProjectileNuke createProjectile(GenericGun gun, World world, EntityLivingBase p, float damage,
            ...
            return new RocketProjectileNuke(world, p, mod.getDamage(damage), speed, TTL, spread,
                    mod.getRange(dmgDropStart), mod.getRange(dmgDropEnd), mod.getDamage(dmgMin), penetration,
                    blockdamage, firePos, mod.getRadius(radius), gravity);
        }
