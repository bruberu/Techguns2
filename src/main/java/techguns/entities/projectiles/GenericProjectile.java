...
public class GenericProjectile extends Entity implements IProjectile, IEntityAdditionalSpawnData {
    ...
    public GenericProjectile(World par2World, EntityLivingBase p, float damage, float speed, int TTL, float spread,
        ...
        this(par2World);
        this.shooter = p;
        this.initProjectile(par2World, p.posX, p.posY + p.getEyeHeight(), p.posZ, p.rotationYawHead, p.rotationPitch,
                damage, speed,
    ...
    public GenericProjectile(World worldIn, double posX, double posY, double posZ, float yaw, float pitch, float damage,
        ...
        this(worldIn);
        this.shooter = null;
        this.initProjectile(worldIn, posX, posY, posZ, yaw, pitch, damage, speed, TTL, spread, dmgDropStart, dmgDropEnd,
                dmgMin, penetration, blockdamage, firePos);
    }

    private void initProjectile(World worldIn, double posX, double posY, double posZ, float yaw, float pitch,
                                float damage,
                                float speed, int TTL, float spread, float dmgDropStart, float dmgDropEnd, float dmgMin,
                                float penetration,
                                boolean blockdamage, EnumBulletFirePos firePos) {
    ...
    protected void sendImpactFX(double x, double y, double z, float pitch, float yaw, int type, boolean incendiary) {
        if (!this.world.isRemote) {
            TGPackets.network.sendToAllAround(new PacketGunImpactFX((short) type, x, y, z, pitch, yaw, incendiary),
                    new TargetPoint(this.world.provider.getDimension(), x, y, z, TGEntities.bulletTrackRange));
        }
    }
    ...
    protected float getDamage() {
        ...
        if (distance <= this.damageDropStart) {
            ...
            float factor = 1.0f -
                    (float) ((distance - this.damageDropStart) / (this.damageDropEnd - this.damageDropStart));
    ...
    public static void burnBlocks(World world, RayTraceResult mop, double chanceToIgnite) {
        ...
        if (Math.random() <= chanceToIgnite) {
            ...
            switch (mop.sideHit) {
                ...
                case UP:
                    if (world.isAirBlock(hit.up())) {
                        if (world.getBlockState(hit) == Blocks.FARMLAND)
             ...
             * // || b == Blocks.wheat || b == Blocks.pumpkin_stem || b == Blocks.melon_stem || b == Blocks.carrots || b
             * == Blocks.potatoes) {
    ...
    public static class Factory implements IProjectileFactory<GenericProjectile> {
        ...
        @Override
        public GenericProjectile createProjectile(GenericGun gun, World world, EntityLivingBase p, float damage,
            ...
            return new GenericProjectile(world, p, damage, speed, TTL, spread, dmgDropStart, dmgDropEnd, dmgMin,
