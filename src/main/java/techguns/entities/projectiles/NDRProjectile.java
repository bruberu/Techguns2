...
public class NDRProjectile extends AbstractBeamProjectile {
    ...
    public void onUpdate() {
        ...
        if (shooter != null) {
            dx = (double) (-MathHelper.sin(shooter.rotationYawHead / 180.0F * (float) Math.PI) *
                    MathHelper.cos(shooter.rotationPitch / 180.0F * (float) Math.PI) * f);
            dz = (double) (MathHelper.cos(shooter.rotationYawHead / 180.0F * (float) Math.PI) *
                    MathHelper.cos(shooter.rotationPitch / 180.0F * (float) Math.PI) * f);
            dy = (double) (-MathHelper.sin((shooter.rotationPitch) / 180.0F * (float) Math.PI) * f);
            ...
            if (Techguns.proxy.isClientPlayerAndIn1stPerson(shooter)) {
                ...
                offset = offset.rotatePitch((float) (shooter.rotationPitch * MathUtil.D2R))
                        .rotateYaw((float) ((-90.0 - shooter.rotationYawHead) * MathUtil.D2R));
    ...
    protected void doImpactEffects(Material mat, RayTraceResult rayTraceResult, SoundType sound) {
        ...
        // TGPackets.network.sendToAllAround(new PacketSpawnParticle("BeamGunImpactFX", hitVec.x, hitVec.y, hitVec.z),
        // TGPackets.targetPointAroundEnt(this, 35.0f));
    ...
    public static class Factory implements IProjectileFactory<NDRProjectile> {
        ...
        @Override
        public NDRProjectile createProjectile(GenericGun gun, World world, EntityLivingBase p, float damage,
                                              float speed,
                                              int TTL, float spread, float dmgDropStart, float dmgDropEnd, float dmgMin,
                                              float penetration,
                                              boolean blockdamage, EnumBulletFirePos firePos, float radius,
                                              double gravity) {
            return new NDRProjectile(world, p, damage, speed, BEAM_LIFETIME, spread, dmgDropStart, dmgDropEnd, dmgMin,
