...
public class GenericProjectileExplosive extends GenericProjectile {
    ...
    protected void explode(double x, double y, double z) {
        if (!this.world.isRemote) {
            TGPackets.network.sendToAllAround(new PacketSpawnParticle("MiningChargeBlockExplosion", x, y, z),
                    TGPackets.targetPointAroundEnt(this, 100.0f));

            // TGExplosion explosion = new TGExplosion(world, this.shooter, this, x, y, z, this.damage, this.damageMin,
            // 0.25f, 2.5f,/*this.damageDropStart,this.damageDropEnd,*/ this.blockdamage?0.05f:0.0);
    ...
    public static class Factory implements IProjectileFactory<GenericProjectileExplosive> {
        ...
        public GenericProjectileExplosive createProjectile(GenericGun gun, World world, EntityLivingBase p,
            ...
            return new GenericProjectileExplosive(world, p, mod.getDamage(damage), speed, TTL, spread, dmgDropStart,
                    dmgDropEnd, mod.getDamage(dmgMin), penetration, blockdamage, firePos);
        }
