...
public class FlamethrowerProjectile extends GenericProjectile implements ILightProvider {
    ...
    public FlamethrowerProjectile(World worldIn, double posX, double posY, double posZ, float yaw, float pitch,
                                  float damage, float speed, int TTL, float spread, float dmgDropStart,
                                  float dmgDropEnd, float dmgMin, float penetration, boolean blockdamage,
                                  EnumBulletFirePos leftGun, double gravity) {
    ...
    public FlamethrowerProjectile(World par2World, EntityLivingBase p, float damage, float speed, int TTL, float spread,
    ...
    protected void hitBlock(RayTraceResult mop) {
        ...
        // TGPackets.network.sendToAllAround(new PacketSpawnParticle("FlamethrowerImpact",
        // this.posX,this.posY,this.posZ), TGPackets.targetPointAroundEnt(this, 50.0f));
    ...
    public static class Factory implements IProjectileFactory<FlamethrowerProjectile> {
        ...
        @Override
        public FlamethrowerProjectile createProjectile(GenericGun gun, World world, EntityLivingBase p, float damage,
            ...
            TGPackets.network.sendToAllAround(
                    new PacketSpawnParticleOnEntity("FlamethrowerFireFX", p, offsetX, offsetY, offsetZ, true),
                    TGPackets.targetPointAroundEnt(p, 25.0f));

            return new FlamethrowerProjectile(world, p, damage, speed, TTL, spread, dmgDropStart, dmgDropEnd, dmgMin,
                    penetration, blockdamage, firePos, gravity);
        }
