...
public class TeslaProjectile extends AbstractBeamProjectile {
    ...
    // public TeslaProjectile(World worldIn, double posX, double posY, double posZ, float yaw, float pitch, float
    // damage,
    ...
    public TeslaProjectile(World par2World, EntityLivingBase p, float damage, float speed, int TTL, float spread,
                           ...
                           EnumBulletFirePos leftGun, int chainTargets) {
    ...
    public TeslaProjectile(World world, EntityLivingBase shooter, Entity source, EntityLivingBase target,
                           int chainTargets, float damage, float speed, int TTL, float dmgDropStart,
                           float dmgDropEnd, float dmgMin, float penetration, boolean blockdamage) {
        super(world, shooter, damage, speed, TTL, 0, dmgDropStart, dmgDropEnd, dmgMin, penetration, blockdamage,
    ...
    protected void onHit(RayTraceResult raytraceResultIn) {
        ...
        if (!world.isRemote) {
            ...
            if (lastTarget != null && this.chainTargets > 0) {
                ...
                if (nextTarget != null) {
                    TeslaProjectile proj = new TeslaProjectile(world, this.shooter, lastTarget, nextTarget,
                            chainTargets - 1, this.damage * CHAIN_DAMAGE_FACTOR, this.speed, TTL, this.damageDropStart,
                            this.damageDropEnd, this.damageMin, this.penetration, this.blockdamage);
    ...
    private EntityLivingBase findNextTarget(Entity lastTarget) {
        List<Entity> list = this.world.getEntitiesInAABBexcluding(lastTarget,
                new AxisAlignedBB(lastTarget.posX - CHAIN_RANGE, lastTarget.posY - CHAIN_RANGE,
                        lastTarget.posZ - CHAIN_RANGE, lastTarget.posX + CHAIN_RANGE, lastTarget.posY + CHAIN_RANGE,
                        lastTarget.posZ + CHAIN_RANGE),
                BULLET_TARGETS);
        ...
        for (int i1 = 0; i1 < list.size(); ++i1) {
            ...
            if (e instanceof EntityLivingBase) {
                ...
                double distance = entity.getDistance(lastTarget.posX,
                        lastTarget.posY + lastTarget.getEyeHeight() * 0.5f, lastTarget.posZ);

                if (distance < CHAIN_RANGE && entity.isEntityAlive() && entity != lastTarget) {
                    if (!entity.equals(shooter) && !entity.equals(prevTarget)) {
                        Vec3d vec3d1 = new Vec3d(lastTarget.posX, lastTarget.posY + lastTarget.getEyeHeight() * 0.5f,
                                lastTarget.posZ);
    ...
    public static class Factory implements IProjectileFactory<TeslaProjectile> {
        ...
        @Override
        public TeslaProjectile createProjectile(GenericGun gun, World world, EntityLivingBase p, float damage,
                                                float speed,
                                                int TTL, float spread, float dmgDropStart, float dmgDropEnd,
                                                float dmgMin, float penetration,
                                                boolean blockdamage, EnumBulletFirePos firePos, float radius,
                                                double gravity) {
            return new TeslaProjectile(world, p, damage, speed, TeslaProjectile.TTL, spread, dmgDropStart, dmgDropEnd,
                    dmgMin, penetration, blockdamage, firePos, CHAIN_TARGETS);
        }
