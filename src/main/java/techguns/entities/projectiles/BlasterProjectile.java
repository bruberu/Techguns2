...
public class BlasterProjectile extends GenericProjectile {
    ...
    public BlasterProjectile(World worldIn, double posX, double posY, double posZ, float yaw, float pitch, float damage,
    ...
    public BlasterProjectile(World par2World, EntityLivingBase p, float damage, float speed, int TTL, float spread,
    ...
    public static class Factory implements IProjectileFactory<BlasterProjectile> {
        ...
        @Override
        public BlasterProjectile createProjectile(GenericGun gun, World world, EntityLivingBase p, float damage,
                                                  float speed, int TTL, float spread, float dmgDropStart,
                                                  float dmgDropEnd,
                                                  float dmgMin, float penetration, boolean blockdamage,
                                                  EnumBulletFirePos firePos, float radius, double gravity) {
            return new BlasterProjectile(world, p, damage, speed, TTL, spread, dmgDropStart, dmgDropEnd, dmgMin,
    ...
    protected void hitBlock(RayTraceResult rayTraceResult) {
         ...
         * TGPackets.network.sendToAllAround(new PacketSpawnParticle("LaserGunImpact", hitVec.x, hitVec.y, hitVec.z),
         * TGPackets.targetPointAroundEnt(this, 35.0f));
         * }
         */
        Techguns.proxy.createFX("LaserGunImpact", world, rayTraceResult.hitVec.x, rayTraceResult.hitVec.y,
                rayTraceResult.hitVec.z, 0, 0, 0);
    }
