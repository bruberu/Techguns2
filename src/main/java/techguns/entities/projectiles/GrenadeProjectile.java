...
public class GrenadeProjectile extends GenericProjectile {
    ...
    public GrenadeProjectile(World worldIn, double posX, double posY, double posZ, float yaw, float pitch, float damage,
                             float speed, int TTL, float spread, float damageDropStart, float damageDropEnd,
                             float dmgMin, float penetration, boolean blockdamage, EnumBulletFirePos leftGun,
                             double gravity, float radius, int bounces) {
        super(worldIn, posX, posY, posZ, yaw, pitch, damage, speed, TTL, spread, damageDropStart, damageDropEnd, dmgMin,
    ...
    public GrenadeProjectile(World par2World, EntityLivingBase p, float damage, float speed, int TTL, float spread,
    ...
    public GrenadeProjectile(GrenadeProjectile other, double x, double y, double z) {
        super(other.world, x, y, z, other.rotationYaw, other.rotationPitch, other.damage, other.speed,
                other.ticksToLive, 1.0f, other.damageDropStart, other.damageDropEnd, other.damageMin, other.penetration,
                other.blockdamage, EnumBulletFirePos.CENTER);
    ...
    protected void playBounceSound() {
        ...
        world.playSound(this.posX, this.posY, this.posZ, SoundEvents.ENTITY_IRONGOLEM_HURT, SoundCategory.PLAYERS, 1.0f,
                1.0f, false);
    }
    ...
    protected void explode() {
        if (!this.world.isRemote) {
            TGPackets.network.sendToAllAround(
                    new PacketSpawnParticle("RocketExplosion", this.posX, this.posY, this.posZ),
                    TGPackets.targetPointAroundEnt(this, 50.0f));

            /*
             * Explosion explosion = new Explosion(world, this,this.posX,this.posY, this.posZ, 5, blockdamage,
             * blockdamage);
             * explosion.doExplosionA();
             * explosion.doExplosionB(true);
             */
            TGExplosion exp = new TGExplosion(world, shooter, this, posX, posY, posZ, this.damage, this.damageMin,
                    this.damageDropStart, this.damageDropEnd, 0);
    ...
    public static class Factory implements IGrenadeProjectileFactory<GrenadeProjectile> {
        ...
        @Override
        public GrenadeProjectile createProjectile(World world, EntityLivingBase p, float damage, float speed, int TTL,
                                                  float spread, float dmgDropStart, float dmgDropEnd, float dmgMin,
                                                  float penetration, boolean blockdamage,
                                                  EnumBulletFirePos leftGun, float radius, double gravity, float charge,
                                                  int bounces) {
            return new GrenadeProjectile(world, p, damage, speed, TTL, spread, dmgDropStart, dmgDropEnd, dmgMin,
                    penetration, blockdamage, leftGun, gravity, radius, bounces);
        }
        ...
        @Override
        public GrenadeProjectile createBounceProjectile(GrenadeProjectile proj, double bounceX, double bounceY,
                                                        double bounceZ) {
