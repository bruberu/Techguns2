...
public class BioGunProjectile extends GenericProjectile implements IEntityAdditionalSpawnData {
    ...
    public BioGunProjectile(World worldIn, double posX, double posY, double posZ, float yaw, float pitch, float damage,
                            float speed, int TTL, float spread, float dmgDropStart,
                            float dmgDropEnd, float dmgMin, float penetration, boolean blockdamage,
                            EnumBulletFirePos leftGun, double gravity, int level) {
    ...
    public BioGunProjectile(World par2World, EntityLivingBase p, float damage, float speed, int TTL, float spread,
    ...
    protected void onHitEffect(EntityLivingBase ent, RayTraceResult rayTraceResult) {
        ...
        // TGPackets.network.sendToAllAround(new PacketSpawnParticle("biogunImpact",
        // -rayTraceResult.hitVec.x,-rayTraceResult.hitVec.y,-rayTraceResult.hitVec.z),
        // TGPackets.targetPointAroundEnt(this, 25.0f));
    }
    ...
    protected void hitBlock(RayTraceResult mop) {
                    ...
                    if (this.blockdamage) {
                        ...
                        if (this.shooter instanceof EntityPlayer) {
                            final BlockEvent.PlaceEvent placeEvent = new BlockEvent.PlaceEvent(
                                    BlockSnapshot.getBlockSnapshot(world, blobPos), statehit,
                                    (EntityPlayer) this.shooter, EnumHand.MAIN_HAND);
                            MinecraftForge.EVENT_BUS.post(placeEvent);
                            canPlace = !placeEvent.isCanceled();
                        }

                        if (canPlace) {
                            IBlockState state = TGBlocks.BIOBLOB.getDefaultState()
                                    .withProperty(TGBlocks.BIOBLOB.FACING_ALL, mop.sideHit.getOpposite())
                                    .withProperty(TGBlocks.BIOBLOB.SIZE, 0);
                            int lvl = 0;
        ...
        // TGPackets.network.sendToAllAround(new PacketSpawnParticle("biogunImpact",
        // mop.hitVec.x,mop.hitVec.y,mop.hitVec.z), TGPackets.targetPointAroundEnt(this, 25.0f));
    }
     ...
     * if (newlvl >3){
     * new ProjectileExplosion(this.world, x, y, z, this.shooter, 1.5f, 30, 1.0f, 2.0f).doExplosion(false,
     * this.shooter);
    ...
    public void onUpdate() {
        ...
        if (this.isInWater()) {
            ...
            // float f5 = 0.1f;
            // this.worldObj.spawnParticle("slime", this.posX, this.posY, this.posZ, this.motionX*f5,
            // (this.motionY*f5)-this.getGravityVelocity(), this.motionZ*f5);
            // Techguns.proxy.spawnParticle("BioTrail", this.worldObj, this.posX, this.posY, this.posZ, this.motionX*f5,
            // (this.motionY*f5)-this.getGravityVelocity(), this.motionZ*f5);
    ...
    public static class Factory implements IChargedProjectileFactory<BioGunProjectile> {
        ...
        @Override
        public BioGunProjectile createProjectile(GenericGun gun, World world, EntityLivingBase p, float damage,
                                                 float speed, int TTL, float spread, float dmgDropStart,
                                                 float dmgDropEnd,
        ...
        @Override
        public BioGunProjectile createChargedProjectile(World world, EntityLivingBase p, float damage, float speed,
                                                        int TTL, float spread, float dmgDropStart, float dmgDropEnd,
                                                        float dmgMin, float penetration, boolean blockdamage,
                                                        EnumBulletFirePos firePos, float radius, double gravity,
                                                        float charge, int ammoConsumed) {
            return new BioGunProjectile(world, p, damage, speed, TTL, spread, dmgDropStart, dmgDropEnd, dmgMin,
                    penetration, blockdamage, firePos, gravity, ammoConsumed);
        }
    }
}
