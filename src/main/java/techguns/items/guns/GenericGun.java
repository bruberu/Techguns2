...
public class GenericGun extends GenericItem implements IGenericGun, IItemTGRenderer, ICamoChangeable {
    ...
    public GenericGun(String name, ProjectileSelector projectileSelector, boolean semiAuto, int minFiretime,
                      int clipsize, int reloadtime, float damage, SoundEvent firesound, SoundEvent reloadsound, int TTL,
                      float accuracy) {
        this(true, name, projectileSelector, semiAuto, minFiretime, clipsize, reloadtime, damage, firesound,
    ...
    public GenericGun(boolean addToGunList, String name, ProjectileSelector projectile_selector, boolean semiAuto,
                      int minFiretime, int clipsize, int reloadtime, float damage, SoundEvent firesound,
    ...
    protected void spawnProjectile(final World world, final EntityLivingBase player, final ItemStack itemstack,
                                   float spread, float offset, float damagebonus, EnumBulletFirePos firePos,
                                   Entity target) {
        /*
         * GenericProjectile proj = new GenericProjectile(world, player, damage * damagebonus, speed,
         * this.getScaledTTL(), spread, this.damageDropStart, this.damageDropEnd,
         * this.damageMin * damagebonus, this.penetration, getDoBlockDamage(player), leftGun);
         */

        IProjectileFactory<GenericProjectile> projectile = this.projectile_selector
                .getFactoryForType(this.getCurrentAmmoVariantKey(itemstack));

        GenericProjectile proj = projectile.createProjectile(this, world, player, damage * damagebonus, speed,
                this.getScaledTTL(), spread, this.damageDropStart,
                this.damageDropEnd, this.damageMin * damagebonus, this.penetration, getDoBlockDamage(player), firePos,
                radius, gravity);

        float f = 1.0f;
        if (this.muzzelight) {
            Techguns.proxy.createLightPulse(proj.posX + player.getLookVec().x * f,
                    proj.posY + player.getLookVec().y * f, proj.posZ + player.getLookVec().z * f, this.light_lifetime,
                    this.light_radius_start, this.light_radius_end, this.light_r, this.light_g, this.light_b);
        }
        if (silenced) {
            proj.setSilenced();
        }
        ...
        if (ammo > 0) {
            ...
            if (firedelay <= 0) {
                ...
                if (!world.isRemote) {
                    ...
                    float accuracybonus = MathUtil.clamp(
                            1.0f - GenericArmor.getArmorBonusForPlayer(player, TGArmorBonus.GUN_ACCURACY, false), 0f,
                            1f);
                    ...
                    if (!checkRecoil ||
                            !ShooterValues.isStillRecoiling(player, hand == EnumHand.OFF_HAND, ATTACK_TYPE)) {
                        // System.out.println("SettingRecoilTime");
                        ShooterValues.setRecoiltime(player, hand == EnumHand.OFF_HAND,
                                System.currentTimeMillis() + recoiltime_l, recoiltime_l, ATTACK_TYPE);

                    }

                    ClientProxy cp = ClientProxy.get();
                    if (!checkMuzzleFlash || ShooterValues.getMuzzleFlashTime(player, hand == EnumHand.OFF_HAND) <=
                            System.currentTimeMillis()) {
                        ShooterValues.setMuzzleFlashTime(player, hand == EnumHand.OFF_HAND,
                                System.currentTimeMillis() + muzzleFlashtime_l, muzzleFlashtime_l);
                ...
                if (maxLoopDelay > 0 && extendedPlayer.getLoopSoundDelay(hand) <= 0) {
                    ...
                    SoundUtil.playSoundOnEntityGunPosition(world, player, this.firesoundStart, SOUND_DISTANCE, 1.0F,
                            false, false, TGSoundCategory.GUN_FIRE);

                    extendedPlayer.setLoopSoundDelay(hand, this.maxLoopDelay);

                } else {

                    SoundUtil.playSoundOnEntityGunPosition(world, player, this.firesound, SOUND_DISTANCE, 1.0F, false,
                            false, TGSoundCategory.GUN_FIRE);
                ...
                if (!(rechamberSound == null)) {
                    ...
                    SoundUtil.playSoundOnEntityGunPosition(world, player, rechamberSound, 1.0F, 1.0F, false, false,
                            TGSoundCategory.RELOAD);
                }
            ...
            if (InventoryUtil.consumeAmmoPlayer(player, this.ammoType.getAmmo(this.getCurrentAmmoVariant(stack)))) {
                ...
                Arrays.stream(this.ammoType.getEmptyMag()).forEach(e -> {
                    if (!e.isEmpty()) {
                        ...
                        // ItemStack(emptyMag.getItem(),1,emptyMag.getItemDamage()));
                ...
                if (ammoCount > 1) {
                    int i = 1;
                    while (i < ammoCount && InventoryUtil.consumeAmmoPlayer(player,
                            this.ammoType.getAmmo(this.getCurrentAmmoVariant(stack)))) {
                        i++;
                    }
                ...
                SoundUtil.playReloadSoundOnEntity(world, player, reloadsound, 1.0F, 1.0F, false, true,
                        TGSoundCategory.RELOAD);

                if (world.isRemote) {

                    int time = (int) (((float) reloadtime / 20.0f) * 1000);
                    ShooterValues.setReloadtime(player, hand == EnumHand.OFF_HAND, System.currentTimeMillis() + time,
                            time, ATTACK_TYPE);
                    ...
                    TGPackets.network.sendToAllAround(
                            new ReloadStartedMessage(player, hand, msg_reloadtime, ATTACK_TYPE),
                            TGPackets.targetPointAroundEnt(player, 100.0));
                    //
                }
    ...
    protected void shootGun(World world, EntityLivingBase player, ItemStack itemstack, float accuracybonus,
                            float damagebonus, int attackType, EnumHand hand, EnumBulletFirePos firePos,
                            Entity target) {
        ...
        if (!world.isRemote) {
            ...
            TGPackets.network.sendToAllAround(new GunFiredMessage(player, msg_recoiltime, msg_muzzleflashtime,
                    attackType, checkRecoil, hand == EnumHand.OFF_HAND),
                    TGPackets.targetPointAroundEnt(player, 100.0f));
        }
        //
        spawnProjectile(world, player, itemstack, accuracy * accuracybonus, projectileForwardOffset, damagebonus,
                firePos, target);
        ...
        if (shotgun) {
            ...
            for (int i = 0; i < bulletcount; i++) {
                spawnProjectile(world, player, itemstack, spread * accuracybonus,
                        projectileForwardOffset + offset * (i + 1.0f), damagebonus, firePos, target);
            }
        }
    }
    ...
    protected String getTooltipTextDmg(ItemStack stack, boolean expanded) {
        DamageModifier mod = this.projectile_selector.getFactoryForType(this.getCurrentAmmoVariantKey(stack))
                .getDamageModifier();
    ...
    protected String getTooltipTextRange(ItemStack stack) {
        ...
        if (this.rangeTooltipType == RangeTooltipType.DROP) {
            return TextUtil.trans("techguns.gun.tooltip.range") + ": " + prefix + sStart + "," + sEnd + "," + ttl +
                    suffix;
    ...
    public void addInformation(ItemStack stack, World worldIn, List<String> list, ITooltipFlag flagIn) {
        ...
        if (Keyboard.isKeyDown(Keyboard.KEY_LSHIFT) || Keyboard.isKeyDown(Keyboard.KEY_RSHIFT)) {
            ...
            for (int i = 0; i < ammo.length; i++) {
                list.add(TextUtil.trans("techguns.gun.tooltip.ammo") + ": " +
                        (this.ammoCount > 1 ? this.ammoCount + "x " : "") + ChatFormatting.WHITE +
                        TextUtil.trans(ammo[i].getTranslationKey() + ".name"));
            }
            ...
            // list.add(TextUtil.trans("techguns.gun.tooltip.range")+":
            // "+this.damageDropStart+","+this.damageDropEnd+","+this.ticksToLive);
            ...
            list.add(TextUtil.trans("techguns.gun.tooltip.spread") + ": " + this.accuracy +
                    (this.zoombonus != 1.0 ? (" Z:" + this.zoombonus * this.accuracy) : ""));
            ...
            if (this.penetration > 0.0f) {
                list.add(TextUtil.trans("techguns.gun.tooltip.armorPen") + ": " +
                        String.format("%.1f", this.penetration));
            }
            if (this.canZoom) {
                list.add(TextUtil.trans("techguns.gun.tooltip.zoom") + ":" +
                        (this.toggleZoom ? "(" + TextUtil.trans("techguns.gun.tooltip.zoom.toogle") + ")" :
                                "(" + TextUtil.trans("techguns.gun.tooltip.zoom.hold") + ")") +
                        " " + TextUtil.trans("techguns.gun.tooltip.zoom.multiplier") + ":" + this.zoomMult);
            }
            ...
            list.add(TextUtil.trans("techguns.gun.tooltip.damage") +
                    (this.shotgun ? ("(x" + (this.bulletcount + 1) + ")") : "") + ": " +
                    getTooltipTextDmg(stack, false));
            list.add(TextUtil.trans("techguns.gun.tooltip.shift1") + " " + ChatFormatting.GREEN +
                    TextUtil.trans("techguns.gun.tooltip.shift2") + " " + ChatFormatting.GRAY +
                    TextUtil.trans("techguns.gun.tooltip.shift3"));
        }
        // } else {
        ...
        if (this.shootWithLeftClick) {
            ...
            if (targetEntity.canBeAttackedWithItem() && GenericProjectile.BULLET_TARGETS.apply(targetEntity)) {
                if (!targetEntity.hitByEntity(player)) {
                    float f = (float) player.getEntityAttribute(SharedMonsterAttributes.ATTACK_DAMAGE)
                            .getAttributeValue();
                    float f1;
                    ...
                    if (f > 0.0F || f1 > 0.0F) {
                        ...
                        if (flag && !flag2 && !flag1 && player.onGround && d0 < (double) player.getAIMoveSpeed()) {
                            ...
                            flag3 = this.hasSwordSweep() && this.getAmmoLeft(stack) > 0; // if (itemstack.getItem()
                        ...
                        if (flag5) {
                                ...
                                if (targetEntity instanceof EntityLivingBase) {
                                    ...
                                    targetEntity.addVelocity(
                                            (double) (-MathHelper.sin(player.rotationYaw * 0.017453292F) * (float) i *
                                                    0.5F),
                                            0.1D,
                                            (double) (MathHelper.cos(player.rotationYaw * 0.017453292F) * (float) i *
                                                    0.5F));
                                }
                            ...
                            if (flag2) {
                                player.world.playSound((EntityPlayer) null, player.posX, player.posY, player.posZ,
                                        SoundEvents.ENTITY_PLAYER_ATTACK_CRIT, player.getSoundCategory(),
                                        1.0F, 1.0F);
                            ...
                            player.world.playSound((EntityPlayer) null, player.posX, player.posY, player.posZ,
                                    SoundEvents.ENTITY_PLAYER_ATTACK_NODAMAGE, player.getSoundCategory(),
                                    1.0F, 1.0F);
    ...
    /**
     * Override in Subclass to define damagesource used for player melee attacks, only relevant if "shootWithLeftClick"
     * is not set
     * 
     * @return
     */
    ...
    public EntityAIRangedAttack getAIAttack(IRangedAttackMob shooter) {
        return new EntityAIRangedAttack(shooter, 1.0D, this.AI_attackTime / 3, this.AI_attackTime, this.AI_attackRange,
                this.AI_burstCount, this.AI_burstAttackTime);
    }
    ...
    public void fireWeaponFromNPC(EntityLivingBase shooter, float dmgscale, float accscale) {
        SoundUtil.playSoundOnEntityGunPosition(shooter.world, shooter, firesound, SOUND_DISTANCE, 1.0F, false, false,
                TGSoundCategory.GUN_FIRE);
        ...
        if (!shooter.world.isRemote) {
            this.shootGun(shooter.world, shooter, shooter.getHeldItemMainhand(), this.zoombonus * accscale, dmgscale, 0,
                    EnumHand.MAIN_HAND, firePos, null);
        }
    }
        ...
        if (this.ammoCount > 1 && this.getAmmoLeft(stack) > 0) {
            ...
            if (!this.isFullyLoaded(stack)) {
                ...
                for (int i = 0; i < amount; i++) {
                    ...
                    if (bulletsBack > 0) {
                        items.add(TGItems.newStack(this.getAmmoType().getBullet(this.getCurrentAmmoVariant(stack))[i],
                                bulletsBack));
                    }
            ...
            if (InventoryUtil.consumeAmmoPlayer(player, this.getReloadItem(item))) {
                ...
                for (int i = 0; i < ammos; i++) {
                    if (!this.ammoType.getEmptyMag()[i].isEmpty()) {
                        ...
                        int amount = InventoryUtil.addAmmoToPlayerInventory(player,
                                TGItems.newStack(this.ammoType.getEmptyMag()[i], 1));
                        if (amount > 0 && !world.isRemote) {
                            player.world.spawnEntity(new EntityItem(player.world, player.posX, player.posY, player.posZ,
                                    TGItems.newStack(this.ammoType.getEmptyMag()[i], amount)));
                        }

                        int bulletsBack = (int) Math
                                .floor(oldAmmo / this.ammoType.getShotsPerBullet(clipsize, oldAmmo));
                        if (bulletsBack > 0) {
                            int amount2 = InventoryUtil.addAmmoToPlayerInventory(player, TGItems.newStack(
                                    this.ammoType.getBullet(this.getCurrentAmmoVariant(item))[i], bulletsBack));
                            if (amount2 > 0 && !world.isRemote) {
                                player.world
                                        .spawnEntity(new EntityItem(player.world, player.posX, player.posY, player.posZ,
                                                TGItems.newStack(
                                                        this.ammoType.getBullet(this.getCurrentAmmoVariant(item))[i],
                                                        amount2)));
                            }
                        }
                ...
                if (ammoCount > 1) {
                    int i = 1;
                    while (i < (ammoCount - oldAmmo) && InventoryUtil.consumeAmmoPlayer(player,
                            this.ammoType.getAmmo(this.getCurrentAmmoVariant(item)))) {
                        i++;
                    }
                ...
                if (world.isRemote) {
                    ...
                    int msg_reloadtime = ((int) (((float) reloadtime / 20.0f) * 1000.0f));
                    TGPackets.network.sendToAllAround(new ReloadStartedMessage(player, hand, msg_reloadtime, 0),
                            new TargetPoint(player.dimension, player.posX, player.posY, player.posZ, 100.0f));
                    //
                }
