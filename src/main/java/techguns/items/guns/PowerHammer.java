...
public class PowerHammer extends GenericGunMeleeCharge {
    ...
    protected void spawnSweepParticle(World w, double x, double y, double z, double motionX, double motionY,
                                      double motionZ) {
        TGPackets.network.sendToAllAround(new PacketSpawnParticle("PowerhammerImpact", x, y, z),
                new TargetPoint(w.provider.getDimension(), x, y, z, 32.0f));
    }
    ...
    public void spawnChargedProjectile(final World world, final EntityLivingBase player, ItemStack itemStack,
        ...
        GenericProjectile proj = fact.createChargedProjectile(world, player, damage + extraDmg, speed, this.ticksToLive,
                spread, this.damageDropStart, damageDropEnd, this.damageMin + extraDmg, penetration,
                getDoBlockDamage(player), firePos, radius, gravity, charge, ammoConsumed);
