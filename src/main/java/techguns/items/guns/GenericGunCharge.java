...
public class GenericGunCharge extends GenericGun {
    ...
    public GenericGunCharge(String name, ChargedProjectileSelector projectile_selector, boolean semiAuto,
            ...
            if (ammo > 0) {
                ...
                if (firedelay <= 0) {
                    ...
                    if (this.startChargeSound != null) {
                        SoundUtil.playSoundOnEntityGunPosition(world, player, this.startChargeSound, SOUND_DISTANCE,
                                1.0F, false, true, false, TGSoundCategory.GUN_FIRE, EntityCondition.CHARGING_WEAPON);
                        // SoundUtil.playSoundOnEntityGunPosition(world, player ,this.startChargeSound, SOUND_DISTANCE,
                        // 1.0F, false, false, TGSoundCategory.GUN_FIRE);
                    }
                    ...
                    if (this.chargeFX != null) {
                        ...
                        TGPackets.network.sendToAllAround(
                                new PacketSpawnParticleOnEntity(this.chargeFX, player, x, this.chargeFXoffsetY,
                                        this.chargeFXoffsetZ, true, EntityCondition.CHARGING_WEAPON),
                                TGPackets.targetPointAroundEnt(player, 25.0f));
                    }
                }
                ...
                if (InventoryUtil.consumeAmmoPlayer(player, this.ammoType.getAmmo(this.getCurrentAmmoVariant(item)))) {
                    ...
                    Arrays.stream(this.ammoType.getEmptyMag()).forEach(e -> {
                        if (!e.isEmpty()) {
                            ...
                            if (amount > 0 && !world.isRemote) {
                                player.world.spawnEntity(new EntityItem(player.world, player.posX, player.posY,
                                        player.posZ, TGItems.newStack(e, amount)));
                            }
                        }
                    });
                    ...
                    SoundUtil.playSoundOnEntityGunPosition(world, player, reloadsound, 1.0f, 1.0f, false, true,
                    ...
                    if (world.isRemote) {
                        ...
                        ShooterValues.setReloadtime(player, handIn == EnumHand.OFF_HAND,
                                System.currentTimeMillis() + time, time, (byte) 1);
    ...
    public void onPlayerStoppedUsing(ItemStack item, World world, EntityLivingBase entityLiving, int timeLeft) {
        if (entityLiving instanceof EntityPlayer) {
            ...
            if (!world.isRemote) {
                ...
                // player.getPrimaryHand()==EnumHandSide.RIGHT?EnumBulletFirePos.RIGHT:EnumBulletFirePos.LEFT;
                EnumBulletFirePos firePos;
                ...
                if (this.hasChargedFireAnim) {
                    ...
                    ShooterValues.setRecoiltime(player, player.getActiveHand() == EnumHand.OFF_HAND,
                            System.currentTimeMillis() + recoiltime_l, recoiltime_l, (byte) 1, f);
                    ...
                    ShooterValues.setMuzzleFlashTime(player, player.getActiveHand() == EnumHand.OFF_HAND,
    ...
    protected void playChargedFiresound(ItemStack item, World world, EntityPlayer player, float chargeProgress) {
        SoundUtil.playSoundOnEntityGunPosition(world, player, firesound, SOUND_DISTANCE, 1.0f, false, false, true,
                TGSoundCategory.GUN_FIRE);

        if (!(rechamberSound == null)) {
            SoundUtil.playSoundOnEntityGunPosition(world, player, rechamberSound, 1.0f, 1.0f, false, false, true,
                    TGSoundCategory.GUN_FIRE);
        }
    }
    ...
    public void spawnChargedProjectile(final World world, final EntityLivingBase player, ItemStack itemStack,
                                       float spread, float charge, int ammoConsumed, EnumBulletFirePos firePos) {
        /*
         * world.spawnEntity(new GenericProjectile(world, player, damage, speed, this.ticksToLive, spread,
         * this.damageDropStart, this.damageDropEnd,
         * this.damageMin, this.penetration, getDoBlockDamage(player), leftGun));
         */
        IChargedProjectileFactory fact = this.chargedProjectile_selector
                .getFactoryForType(this.getCurrentAmmoVariantKey(itemStack));
        GenericProjectile proj = fact.createChargedProjectile(world, player, damage, speed, this.getScaledTTL(), spread,
                this.damageDropStart, damageDropEnd, this.damageMin, penetration, getDoBlockDamage(player), firePos,
                radius, gravity, charge, ammoConsumed);
