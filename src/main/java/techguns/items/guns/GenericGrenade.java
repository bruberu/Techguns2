...
public class GenericGrenade extends GenericItem implements IItemTGRenderer {
    ...
    public ActionResult<ItemStack> onItemRightClick(World worldIn, EntityPlayer playerIn, EnumHand handIn) {
        ...
        if (!worldIn.isRemote && this.startSound != null) {
            TGPackets.network.sendToAllAround(new PacketPlaySound(this.startSound, playerIn, 1, 1, false, false, true,
                    true, TGSoundCategory.PLAYER_EFFECT), TGPackets.targetPointAroundEnt(playerIn, 24.0f));
        }
    ...
    public void spawnGrenade(World world, EntityLivingBase entityLiving, float charge, EnumBulletFirePos firePos) {
        ...
        GrenadeProjectile proj = this.projectile_factory.createProjectile(world, entityLiving, this.damage, speed,
                this.ticksToLive, spread, this.radius, this.radiusMin, this.damageMin, penetration,
                GenericGun.getDoBlockDamage(entityLiving), firePos, radiusMin, grav, charge, maxBounces);

        world.spawnEntity(proj);

        // world.spawnEntityInWorld(new GrenadeProjectile(world, player,
        // this.damage,this.radius,0.5f+0.5f*charge,200,0.05f,grav,false,0.0f,this.maxBounces));
        // world.playSoundAtEntity(player, "random.bow", 1.0F, 1.0F / (itemRand.nextFloat() * 0.4F + 1.2F) + charge *
        // 0.5F);
    }
    ...
    public void addInformation(ItemStack stack, World worldIn, List<String> tooltip, ITooltipFlag flagIn) {
        ...
        if (Keyboard.isKeyDown(Keyboard.KEY_LSHIFT) || Keyboard.isKeyDown(Keyboard.KEY_RSHIFT)) {
            ...
            tooltip.add(TextUtil.trans("techguns.gun.tooltip.damage") + ": " + this.damage);
            tooltip.add(TextUtil.trans("techguns.gun.tooltip.shift1") + " " + ChatFormatting.GREEN +
