...
public class SonicShotgun extends GenericGun {
    ...
    public SonicShotgun(String name, ProjectileSelector projectileSelector, boolean semiAuto, int minFiretime,
                        int clipsize, int reloadtime, float damage, SoundEvent firesound, SoundEvent reloadsound,
                        int TTL,
                        float accuracy) {
        super(name, projectileSelector, semiAuto, minFiretime, clipsize, reloadtime, damage, firesound, reloadsound,
                TTL,
                accuracy);
    }
    ...
    protected void spawnProjectile(World world, EntityLivingBase player, ItemStack itemstack, float spread,
        ...
        SonicShotgunProjectile proj = new SonicShotgunProjectile(world, player, damage * damagebonus, speed,
                this.ticksToLive, 0f, this.damageDropStart, this.damageDropEnd, this.damageMin * damagebonus,
                this.penetration, getDoBlockDamage(player), firePos);
        ...
        TGPackets.network.sendToAllAround(new PacketSpawnParticleOnEntity("SonicShotgunTrail", proj),
                TGPackets.targetPointAroundEnt(proj, 100.0f));

        // Vec3 motion = Vec3.createVectorHelper(proj.motionX, proj.motionY, proj.motionZ).normalize(); //Shoot
        // direction
        ...
        for (int j = 1; j <= rings; j++) {
            ...
            for (int i = 0; i < (count * j); i++) {
                ...
                proj = new SonicShotgunProjectile(world, player, damage * damagebonus, speed, this.ticksToLive, 0f,
                        this.damageDropStart, this.damageDropEnd, this.damageMin * damagebonus, this.penetration,
                        getDoBlockDamage(player), firePos);
                proj.entitiesHit = entitiesHit;
