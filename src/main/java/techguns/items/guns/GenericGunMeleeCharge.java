...
public class GenericGunMeleeCharge extends GenericGunCharge implements IGenericGunMelee<GenericGunMeleeCharge> {
    ...
    public boolean onEntitySwing(EntityLivingBase entityLiving, ItemStack stack) {
        ...
        if (this.getCurrentAmmo(stack) > 0) {
            if (entityLiving.world.isRemote) {
                ...
                // server side:;
                // SoundUtil.playSoundOnEntityGunPosition(entityLiving.worldObj, entityLiving,
                // "techguns:guns.powerhammerSwing", SOUND_DISTANCE, 1.0F, false, false);
                if (!openingContainer && sendSound) {
                    TGPackets.network.sendToAllAround(
                            new PacketPlaySound(getSwingSound(), entityLiving, melee_sound_volume, 1.0f, false, false,
                                    true, true, TGSoundCategory.GUN_FIRE),
                            TGPackets.targetPointAroundEnt(entityLiving, 32.0f));
                }
    ...
    public boolean onBlockDestroyed(ItemStack stack, World worldIn, IBlockState state, BlockPos pos,
        ...
        if (this.getCurrentAmmo(stack) > 0) {
            ...
            if (entityLiving instanceof EntityPlayer) {
                ...
                if (sound != null) {
                    worldIn.playSound((EntityPlayer) entityLiving, pos.getX() + 0.5D, pos.getY() + 0.5D,
                            pos.getZ() + 0.5D, sound, SoundCategory.PLAYERS, melee_sound_volume, 1.0f);
                }
            }
            return true;
        }
    ...
    public Multimap<String, AttributeModifier> getAttributeModifiers(EntityEquipmentSlot slot, ItemStack stack) {
        ...
        if (slot == EntityEquipmentSlot.MAINHAND) {
            ...
            multimap.put(SharedMonsterAttributes.ATTACK_DAMAGE.getName(),
                    new AttributeModifier(ATTACK_DAMAGE_MODIFIER, "Weapon modifier", meleedmg, 0));
    ...
    protected void addMiningTooltip(ItemStack stack, World world, List<String> list, ITooltipFlag flagIn,
        ...
        if (longTooltip) {
            if (this.miningHeads != null) {
                list.add(TextUtil.trans("techguns.tooltip.mininghead") + ": " + ChatFormatting.WHITE +
                        TextUtil.trans(this.getCurrentMiningHeadForTooltip(stack)));
            }

            list.add(TextUtil.trans("techguns.tooltip.toolclasses") + ":");
            for (String s : this.getMiningLevels().keySet()) {
                if (!s.equals("default")) {
                    list.add(" " + TextUtil.transTG("toolclass." + s) + ": " +
                            (this.getMiningLevels().get(s) + this.getExtraMiningLevel(stack, s, null)));
                }
            }
    ...
    public boolean onBlockStartBreak(ItemStack itemstack, BlockPos pos, EntityPlayer player) {
        if (!player.world.isRemote && !player.isSneaking() && this.getMiningRadius(itemstack) > 0) {
            ...
            if (sideHit != null) {
                ...
                List<BlockPos> otherblocks = BlockUtils.getBlockPlaneAroundAxisForMining(player.world, player, pos,
                        sideHit.getAxis(), this.getMiningRadius(itemstack), false, this, itemstack);
                otherblocks.forEach(b -> {
                    if (w.isBlockLoaded(b)) {
                        ...
                        if (state.getBlock() != null && !(state.getBlock() == Blocks.AIR) &&
                                state.getPlayerRelativeBlockHardness(player, w, b) > 0f &&
                                state.getBlock().canHarvestBlock(w, b, player) &&
                                this.isEffectiveToolForState(itemstack, state)) {

                            EntityPlayerMP plyMP = (EntityPlayerMP) player;
                            int xpAmount = ForgeHooks.onBlockBreakEvent(w, plyMP.interactionManager.getGameType(),
                                    plyMP, b);
                            if (xpAmount >= 0) {
