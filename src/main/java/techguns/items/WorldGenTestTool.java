...
public class WorldGenTestTool extends GenericItem {
    ...
    public ActionResult<ItemStack> onItemRightClick(World world, EntityPlayer player, EnumHand hand) {
        ...
        if (player.isSneaking()) {
            ...
            if (world.isRemote)
                player.sendStatusMessage(new TextComponentString("WorldGen tool mode set to \"" + modes[mode] + "\"."),
                        false);
        }
    ...
    public EnumActionResult onItemUse(EntityPlayer player, World world, BlockPos pos, EnumHand hand,
        ...
        if (player.isSneaking()) {
            ...
            if (item.getTagCompound().hasKey("x1") && item.getTagCompound().hasKey("y1") &&
                    item.getTagCompound().hasKey("z1")) {

                if (world.isRemote)
                    player.sendStatusMessage(new TextComponentString("Position2 set (" + x + "/" + y + "/" + z + ")."),
                            false);
                ...
                if (world.isRemote) player.sendStatusMessage(
                        new TextComponentString("Size: (" + sizeX + "/" + sizeY + "/" + sizeZ + ")."), false);
                ...
                if (world.isRemote)
                    player.sendStatusMessage(new TextComponentString("Position1 set (" + x + "/" + y + "/" + z + ")."),
                            false);
            }
        }
    ...
    private void doOperation(World world, int x, int y, int z, int sizeX, int sizeY, int sizeZ, int mode) {
        ...
        switch (mode) {
            ...
            case 1: // dungeon
                ...
                e.setBlocks(world, px + 1 - preset.getSizeXZ() / 2, py, pz + 1 - preset.getSizeXZ() / 2, e.sizeX,
                        e.sizeY, e.sizeZ, BlockUtils.facingToRotation(dungeon.startRotation), BiomeColorType.WOODLAND,
                        rnd);
