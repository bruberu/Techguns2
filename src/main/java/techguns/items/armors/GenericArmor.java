...
public class GenericArmor extends ItemArmor implements ISpecialArmor, IItemTGRenderer {
    ...
    protected static DecimalFormat formatArmor = new DecimalFormat("#.##",
    ...
    public GenericArmor(String unlocalizedName, TGArmorMaterial material, String textureName,
                        EntityEquipmentSlot type) {
        this(Techguns.MODID, unlocalizedName, material, textureName, type);
    }

    public GenericArmor(String modid, String unlocalizedName, TGArmorMaterial material, String textureName,
                        EntityEquipmentSlot type) {
    ...
    public static float getArmorBonusForPlayer(EntityPlayer ply, TGArmorBonus type, boolean consumePower) {
        ...
        if (props != null) {
            ...
            bonus += getBonusForSlot(props.tg_inventory.inventory.get(TGPlayerInventory.SLOT_FACE), type, consumePower,
                    ply);
            bonus += getBonusForSlot(props.tg_inventory.inventory.get(TGPlayerInventory.SLOT_BACK), type, consumePower,
                    ply);
            bonus += getBonusForSlot(props.tg_inventory.inventory.get(TGPlayerInventory.SLOT_HAND), type, consumePower,
                    ply);
        }
    ...
    private static float getBonusForSlot(ItemStack stack, TGArmorBonus type, boolean consumePower, EntityPlayer ply) {
        ...
        if (!stack.isEmpty()) {
            ...
            if (stack.getItem() == TGItems.SHARED_ITEM || (stack.getItemDamage() < stack.getMaxDamage()) ||
    ...
    private String getDoubleTexture() {
        return this.modid + ":textures/armor/" + this.textureName + "_" +
    ...
    protected void addArmorvaluesInformation(ItemStack item, List list) {
        ...
        // list.add(" "+ChatFormatting.GRAY+trans("armorTooltip.penetrationResistance")+":
        // "+formatArmor.format(this.getPenetrationResistance()));
        // }

        list.add(ChatFormatting.DARK_GRAY + " AR: " +
                formatAV(this.material.getArmorValueSlot(slot, DamageType.PHYSICAL), DamageType.PHYSICAL));
        list.add(ChatFormatting.GRAY + " PR: " +
                formatAV(this.material.getArmorValueSlot(slot, DamageType.PROJECTILE), DamageType.PROJECTILE));
        list.add(ChatFormatting.DARK_RED + " EX: " +
                formatAV(this.material.getArmorValueSlot(slot, DamageType.EXPLOSION), DamageType.EXPLOSION));
        list.add(ChatFormatting.DARK_AQUA + " E: " +
                formatAV(this.material.getArmorValueSlot(slot, DamageType.ENERGY), DamageType.ENERGY));

        list.add(ChatFormatting.RED + " F: " +
                formatAV(this.material.getArmorValueSlot(slot, DamageType.FIRE), DamageType.FIRE));
        list.add(ChatFormatting.AQUA + " I: " +
                formatAV(this.material.getArmorValueSlot(slot, DamageType.ICE), DamageType.ICE));
        list.add(ChatFormatting.YELLOW + " L: " +
                formatAV(this.material.getArmorValueSlot(slot, DamageType.LIGHTNING), DamageType.LIGHTNING));
        list.add(ChatFormatting.DARK_GREEN + " P: " +
                formatAV(this.material.getArmorValueSlot(slot, DamageType.POISON), DamageType.POISON));
        list.add(ChatFormatting.DARK_GRAY + " D: " +
                formatAV(this.material.getArmorValueSlot(slot, DamageType.DARK), DamageType.DARK));
        list.add(ChatFormatting.GREEN + " RAD: " +
                formatAV(this.material.getArmorValueSlot(slot, DamageType.RADIATION), DamageType.RADIATION));
    }
    ...
    public void addInformation(ItemStack item, World worldIn, List<String> list, ITooltipFlag flagIn) {
        ...
        if (Keyboard.isKeyDown(Keyboard.KEY_LSHIFT) || Keyboard.isKeyDown(Keyboard.KEY_RSHIFT)) {
            ...
            list.add(trans("armorTooltip.durability") + ": " + (item.getMaxDamage() - item.getItemDamage()) + "/" +
                    (item.getMaxDamage()));
            ...
            // list.add(EnumChatFormatting.DARK_GRAY+" AR:"+this.material.getArmorValueSlot(this.armorType,
            // DamageType.PHYSICAL));
            this.addArmorvaluesInformation(item, list);

            if (this.getBonus(TGArmorBonus.EXTRA_HEART) > 0.0f) {
                list.add(trans("armorTooltip.healthbonus") + ": +" + (int) this.getBonus(TGArmorBonus.EXTRA_HEART) +
                        " " + trans("armorTooltip.hearts"));
            ...
            if (this.getBonus(TGArmorBonus.BREAKSPEED_WATER) > 0.0f) {
                list.add(trans("armorTooltip.underwatermining") + ": +" +
                        this.getBonus(TGArmorBonus.BREAKSPEED_WATER) * 100.0f + "%");
            }
            if (this.getBonus(TGArmorBonus.KNOCKBACK_RESISTANCE) > 0.0f) {
                list.add(trans("armorTooltip.knockbackresistance") + ": +" +
                        this.getBonus(TGArmorBonus.KNOCKBACK_RESISTANCE) * 100.0f + "%");
            }
    ...
    public ArmorProperties getProperties(EntityLivingBase player, ItemStack armor, DamageSource source, double damage,
        ...
        ArmorProperties props = new ArmorProperties(0, 1 - DamageSystem.getDamageAfterAbsorb_TGFormula(1.0f,
                this.getArmorValue(armor, src.damageType), this.material.toughness, src.armorPenetration),
                Integer.MAX_VALUE);
    ...
    public Multimap<String, AttributeModifier> getAttributeModifiers(EntityEquipmentSlot slot, ItemStack stack) {
        ...
        if (slot == this.armorType && radresistance > 0) {
            multimap.put(TGRadiation.RADIATION_RESISTANCE.getName(),
                    new AttributeModifier(RAD_RESIST_MODIFIER[slot.getIndex()],
                            "techguns.radresistance." + slot.toString(), this.radresistance, 0));
        }
        return multimap;
    }
}
