...
public class PoweredArmor extends GenericArmorMultiCamo {
    ...
    public PoweredArmor setFallProtection(float multiplier, float freeheight, float multiplier_unpowered,
                                          float freeheight_unpowered) {
        ...
        if (type == TGArmorBonus.JUMP) {
            if (this.armorType == EntityEquipmentSlot.LEGS) { // Legs
                if (!player.world.isRemote) {
                    ...
                    player.world.playSound(null, player.posX, player.posY, player.posZ, TGSounds.STEAM_JUMP,
                ...
                if (InventoryUtil.consumeAmmoPlayer(player, powerChest.battery)) {
                    ...
                    if (powerChest.battery_empty != null) {
                        int amount = InventoryUtil.addAmmoToPlayerInventory(player,
                                TGItems.newStack(powerChest.battery_empty, 1));
                        if (amount > 0 && !player.world.isRemote) {
                            player.world.spawnEntity(new EntityItem(player.world, player.posX, player.posY, player.posZ,
                                    TGItems.newStack(powerChest.battery_empty, amount)));
                        }

                        // player.inventory.addItemStackToInventory(new
                        // ItemStack(powerChest.battery_empty.getItem(),1,powerChest.battery_empty.getItemDamage()));
                    }
    ...
    public void addInformation(ItemStack item, World worldIn, List<String> list, ITooltipFlag flagIn) {
        ...
        if (this.armorType == EntityEquipmentSlot.CHEST) {
            list.add(ChatFormatting.AQUA + TextUtil.transTG("tooltip.powerType") + ": " + this.powerType.toString() +
                    " (" + TextUtil.trans(this.battery.getTranslationKey() + ".name") + ")");
        } else {
            list.add(ChatFormatting.AQUA + TextUtil.transTG("tooltip.powerType") + ": " + this.powerType.toString());
        }
