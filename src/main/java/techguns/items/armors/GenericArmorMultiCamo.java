...
public class GenericArmorMultiCamo extends GenericArmor implements ICamoChangeable {
    ...
    public GenericArmorMultiCamo(String unlocalizedName, TGArmorMaterial material, String[] textureNames,
                                 EntityEquipmentSlot type) {
        this(Techguns.MODID, unlocalizedName, material, textureNames, type);
    }

    public GenericArmorMultiCamo(String modid, String unlocalizedName, TGArmorMaterial material, String[] textureNames,
                                 EntityEquipmentSlot type) {
     ...
     * return Techguns.MODID + ":textures/armor/" + this.textureNames[camoID] + "_" + (this.armorType ==
     * EntityEquipmentSlot.LEGS ? "2" : "1") + ".png";
        ...
        if (player.isSneaking()) {
            ...
            if (world.isRemote) {
                player.sendMessage(new TextComponentString(
                        TextUtil.trans("techguns.message.camoswitch") + " " + getCurrentCamoName(item)));
            }
    ...
    public String getCurrentCamoName(ItemStack item) {
        ...
        if (camoID >= 0) {
            return TextUtil
                    .trans(this.modid + ".item." + textureNames[0] + "." + camoNameSuffix + "camoname." + camoID);
        } else {
    ...
    public String getArmorTexture(ItemStack stack, Entity entity, EntityEquipmentSlot slot, String type) {
        ...
        if (i >= 0 && i < this.textureNames.length) {
            return this.modid + ":textures/models/armor/" + this.textureNames[i] +
                    (this.hasDoubleTexture() ? ("_layer_" + getArmorLayer(slot)) : "") + ".png";
        } else {
            return null;
        }
    }
