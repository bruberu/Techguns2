...
public class GenericItemShared extends GenericItem implements IItemTGRenderer, ITGSpecialSlot {
    ...
    public ItemStack addsharedVariant(String name, boolean useRenderHack, TGSlotType slottype, int maxStackSize,
                                      boolean enabled) {
    ...
    public static class SharedItemEntry {
        ...
        public SharedItemEntry(String name, int meta, TGSlotType slottype, short maxStackSize, boolean useRenderHack,
                               boolean enabled) {
            super();
            this.name = name;
    ...
    public void initModel() {
        for (int i = 0; i < this.sharedItems.size(); i++) {
            ModelLoader.setCustomModelResourceLocation(this, i, new ModelResourceLocation(
                    new ResourceLocation(Techguns.MODID, this.sharedItems.get(i).name), "inventory"));
        }
    }
    ...
    public void addInformation(ItemStack stack, World worldIn, List<String> tooltip, ITooltipFlag flagIn) {
        ...
        if (this.getSlot(stack) != TGSlotType.NORMAL) {
            tooltip.add(ChatFormatting.GRAY + TextUtil.trans(Techguns.MODID + ".tooltip.slot") + ": " +
                    this.getSlot(stack));
            if (this.getSlot(stack) == TGSlotType.AMMOSLOT) {
                ...
                if (this.getSharedEntry(stack).ammoEntry != null) {
                    tooltip.add(
                            ChatFormatting.GRAY + TextUtil.trans(Techguns.MODID + ".tooltip.sneakrightclickunload"));
                }
        ...
        if (this.getBonus(TGArmorBonus.EXTRA_HEART, stack) > 0.0f) {
            tooltip.add(trans("armorTooltip.healthbonus") + ": +" +
                    (int) this.getBonus(TGArmorBonus.EXTRA_HEART, stack) + " " + trans("armorTooltip.hearts"));
        } else if (this.getBonus(TGArmorBonus.SPEED, stack) > 0.0f) {
            tooltip.add(
                    trans("armorTooltip.movespeed") + ": +" + this.getBonus(TGArmorBonus.SPEED, stack) * 100.0f + "%");
        ...
        if (this.getBonus(TGArmorBonus.BREAKSPEED, stack) > 0.0f) {
            tooltip.add(trans("armorTooltip.miningspeed") + ": +" +
                    this.getBonus(TGArmorBonus.BREAKSPEED, stack) * 100.0f + "%");
        }
        if (this.getBonus(TGArmorBonus.BREAKSPEED_WATER, stack) > 0.0f) {
            tooltip.add(trans("armorTooltip.underwatermining") + ": +" +
                    this.getBonus(TGArmorBonus.BREAKSPEED_WATER, stack) * 100.0f + "%");
        }
        if (this.getBonus(TGArmorBonus.KNOCKBACK_RESISTANCE, stack) > 0.0f) {
            tooltip.add(trans("armorTooltip.knockbackresistance") + ": +" +
                    this.getBonus(TGArmorBonus.KNOCKBACK_RESISTANCE, stack) * 100.0f + "%");
        }
        ...
        if (stack.getItemDamage() == TGItems.OREDRILLHEAD_STEEL.getItemDamage() ||
                stack.getItemDamage() == TGItems.OREDRILLHEAD_MEDIUM_STEEL.getItemDamage() ||
                stack.getItemDamage() == TGItems.OREDRILLHEAD_LARGE_STEEL.getItemDamage()) {
            tooltip.add(trans("oredrill.mininglevel") + " +1");
        } else if (stack.getItemDamage() == TGItems.OREDRILLHEAD_OBSIDIANSTEEL.getItemDamage() ||
                stack.getItemDamage() == TGItems.OREDRILLHEAD_MEDIUM_OBSIDIANSTEEL.getItemDamage() ||
                stack.getItemDamage() == TGItems.OREDRILLHEAD_LARGE_OBSIDIANSTEEL.getItemDamage()) {
                    tooltip.add(trans("oredrill.mininglevel") + " +2");
                } else
            if (stack.getItemDamage() == TGItems.OREDRILLHEAD_CARBON.getItemDamage() ||
                    stack.getItemDamage() == TGItems.OREDRILLHEAD_MEDIUM_CARBON.getItemDamage() ||
                    stack.getItemDamage() == TGItems.OREDRILLHEAD_LARGE_CARBON.getItemDamage()) {
