...
public abstract class ItemTGSpecialSlotAmmo extends ItemTGSpecialSlot {
    ...
    public ItemTGSpecialSlotAmmo(String unlocalizedName, TGSlotType slot, int camoCount, int dur, ItemStack ammo,
                                 ItemStack ammoEmpty) {
    ...
    public void addInformation(ItemStack stack, World worldIn, List<String> tooltip, ITooltipFlag flagIn) {
        ...
        tooltip.add(ChatFormatting.AQUA + TextUtil.transTG("tooltip.reloads_with") + ": " +
                TextUtil.trans(this.ammo.getTranslationKey() + ".name"));
    }
        ...
        if (InventoryUtil.consumeAmmoPlayer(ply, ammo)) {
            ...
            if (InventoryUtil.addAmmoToPlayerInventory(ply, ammoEmpty) > 0) {
                if (!ply.world.isRemote) {
                    ply.world.spawnEntity(
                            new EntityItem(ply.world, ply.posX, ply.posY, ply.posZ, TGItems.newStack(ammoEmpty, 1)));
                }
            }
        }
    }
