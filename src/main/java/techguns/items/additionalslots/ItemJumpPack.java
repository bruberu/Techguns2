...
public class ItemJumpPack extends ItemTGSpecialSlotAmmo {
    ...
    public ItemJumpPack(String unlocalizedName, int camoCount, int dur) {
        super(unlocalizedName, TGSlotType.BACKSLOT, camoCount, dur, TGItems.COMPRESSED_AIR_TANK,
                TGItems.COMPRESSED_AIR_TANK_EMPTY);
    }
    ...
    public void doJumpEffect(EntityPlayer ply) {
        ...
        if (!ply.world.isRemote) {
            ...
            TGPackets.network.sendToAllAround(new PacketPlaySound(TGSounds.STEAM_JUMP_2, ply, 1.0f, 1.0f, false, true,
                    false, true, TGSoundCategory.PLAYER_EFFECT), targetPoint);
        }
    }

    public void doFallEffect(EntityPlayer ply) {
        if (!ply.world.isRemote) {
            TGPackets.network.sendToAllAround(new PacketSpawnParticle("JumpPackFall", ply.posX, ply.posY, ply.posZ),
                    TGPackets.targetPointAroundEnt(ply, 50));
        }
    }
