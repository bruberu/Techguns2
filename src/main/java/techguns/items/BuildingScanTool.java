        ...
        for (int ix = 0; ix < sizeX; ix++) {
            for (int iy = 0; iy < sizeY; iy++) {
                for (int iz = 0; iz < sizeZ; iz++) {
                    ...
                    if ((bs != Blocks.AIR.getDefaultState()) && (bs != Blocks.DIRT.getDefaultState()) &&
                            (bs != Blocks.GRASS.getDefaultState())) {
                        MBlock mblock = new MBlock(bs);
                        if (!blockList.contains(mblock)) {
                            blockList.add(mblock);
                            // sbDefBlocks.append("MBlock block_"+blockList.indexOf(mblock)+" = new
                            // MBlock(Block.getBlockFromName(\""+mblock.block.getUnlocalizedName()+"\"),
                            // "+mblock.meta+");\n");
                            // sbDefBlocks.append("blockList.add(new
                            // MBlock(Block.getBlockFromName(\""+mblock.block.getUnlocalizedName()+"\"),
                            // "+mblock.meta+"));\n");
                            sbDefBlocks.append("blockList.add(new MBlock(\"" + mblock.block.getRegistryName() + "\", " +
                                    mblock.meta + "));\n");
                        }
                        // String name = "block_"+blockList.indexOf(mblock);
                        // System.out.println("Block("+coordX+","+coordY+","+coordZ+"):"+b.getUnlocalizedName()+":"+meta);
                        // sbSetBlocks.append("BlockUtils.setBlockRotated(world, "+name+", posX, posY, posZ, "+ix+",
                        // "+iy+", "+iz+", cntX, cntY, cntZ, rotation);\n");
