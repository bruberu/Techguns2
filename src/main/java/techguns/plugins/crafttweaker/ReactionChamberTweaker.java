...
public class ReactionChamberTweaker {
    ...
    @ZenMethod
    public static void addRecipe(String key, IItemStack input, ILiquidStack fluid, IItemStack[] outputs,
                                 IItemStack beamFocus, int ticks, int requiredCompletion,
                                 int preferredIntensity, int intensityMargin, int liquidLevel, int liquidConsumption,
                                 float instability, String risk, int RFTick) {
    ...
    @ZenMethod
    public static void addRecipe(String key, String input, ILiquidStack fluid, IItemStack[] outputs,
                                 IItemStack beamFocus, int ticks, int requiredCompletion,
                                 int preferredIntensity, int intensityMargin, int liquidLevel, int liquidConsumption,
                                 float instability, String risk, int RFTick) {
        CraftTweakerAPI.apply(new addInputAction(key, TGCraftTweakerHelper.toItemStackOreDict(input), fluid, outputs,
    ...
    @ZenMethod
    public static void addRecipe(String key, IItemStack input, ILiquidStack fluid, IItemStack output,
                                 IItemStack beamFocus, int ticks, int requiredCompletion,
                                 int preferredIntensity, int intensityMargin, int liquidLevel, int liquidConsumption,
                                 float instability, String risk, int RFTick) {
        CraftTweakerAPI.apply(new addInputAction(key, TGCraftTweakerHelper.toItemStackOreDict(input), fluid,
                new IItemStack[] { output },
                beamFocus, ticks, requiredCompletion, preferredIntensity, intensityMargin,
                liquidLevel, liquidConsumption, instability, risk, RFTick));
    }

    @ZenMethod
    public static void addRecipe(String key, String input, ILiquidStack fluid, IItemStack output, IItemStack beamFocus,
                                 int ticks, int requiredCompletion,
                                 int preferredIntensity, int intensityMargin, int liquidLevel, int liquidConsumption,
                                 float instability, String risk, int RFTick) {
        CraftTweakerAPI.apply(new addInputAction(key, TGCraftTweakerHelper.toItemStackOreDict(input), fluid,
                new IItemStack[] { output },
    ...
    private static class addInputAction implements IAction {
        ...
        public addInputAction(String key, ItemStackOreDict input, ILiquidStack fluid, IItemStack[] outputs,
                              IItemStack beamFocus, int ticks, int requiredCompletion,
                              int preferredIntensity, int intensityMargin, int liquidLevel, int liquidConsumtion,
                              float instability, String risk, int RFTick) {
            super();
            this.key = key;
            this.input = input;
    ...
    private static class removeInputAction implements IAction {
        ...
        @Override
        public String describe() {
            return "Removed " + removed + " Recipe(s) using " + input + "/" + fluid.getUnlocalizedName() +
