...
public class ProgressBarReactionChamberCompletion implements IDrawableAnimated {
    ...
    public ProgressBarReactionChamberCompletion(IGuiHelper guiHelper, int w, int h, int recipeTicks,
                                                int reqCompletion) {
        super();
    ...
    public void draw(Minecraft minecraft, int xOffset, int yOffset) {
        ...
        int steps = ((int) (prog * completion_totalTime)) /
                ((int) ((float) completion_totalTime / (float) this.requiredCompletion));
        // System.out.println("P:"+prog+" TT:"+totalTime+" guiSW:"+this.guiStepWidth + "-> Steps:"+steps);

        // System.out.println("Timer value:"+timer.getValue()+" Prog:"+prog+" Steps:"+steps+ "
        // guiSW:"+this.guiStepWidth+ " Ticks:"+this.recipeTicks);
