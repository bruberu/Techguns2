...
public class GrinderRecipeCategory extends BasicRecipeCategory<GrinderJeiRecipe> {
    ...
    public GrinderRecipeCategory(IGuiHelper guiHelper) {
        ...
        this.progress1 = guiHelper.createAnimatedDrawable(progress1_static, 100, IDrawableAnimated.StartDirection.LEFT,
                false);

        this.progress2_static = guiHelper.createDrawable(GrinderGui.texture, 0, 167, 21, 21);
        this.progress2 = guiHelper.createAnimatedDrawable(progress2_static, 100, IDrawableAnimated.StartDirection.RIGHT,
                false);
    }
    ...
    public void setRecipe(IRecipeLayout recipeLayout, GrinderJeiRecipe recipeWrapper, IIngredients ingredients) {
        ...
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 3; j++) {
                guiItemStacks.init(GrinderTileEnt.SLOT_OUTPUT0 + i * 3 + j, false,
                        SLOT_OUTPUT0_X + j * 18 + JEI_OFFSET_X, SLOTS_INPUT_Y + JEI_OFFSET_Y + i * 18);
            }
        }
                ...
                if (jeiRec.rec instanceof GrinderRecipeChance) {
                    ...
                    if (r.getChances()[slotIndex - GrinderTileEnt.SLOT_OUTPUT0] < 1) {
                        tooltip.add(tooltip.size() - 1, TextUtil.transTG("oredrill.jei.chance") + ": " +
                                String.format("%.2f", r.getChances()[slotIndex - GrinderTileEnt.SLOT_OUTPUT0] * 100f) +
                                "%");
                    }
                }
            }
