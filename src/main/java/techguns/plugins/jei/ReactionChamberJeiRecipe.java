...
public class ReactionChamberJeiRecipe extends BasicRecipeWrapper {
    ...
    public ReactionChamberJeiRecipe(ReactionChamberRecipe recipe, IGuiHelper guiHelper) {
        ...
        this.progress = guiHelper.createAnimatedDrawable(progress_static,
                this.recipe.ticks * ReactionChamberOperation.RECIPE_TICKRATE, IDrawableAnimated.StartDirection.LEFT,
                false);

        this.completion_bar = new ProgressBarReactionChamberCompletion(guiHelper, 100, 4, this.recipe.ticks,
                recipe.requiredCompletion);
        this.fluidlevel = recipe.liquidLevel;
        this.intensity = recipe.preferredIntensity;
        this.intensity_bar = new ProgressBarReactionChamberIntensityJump(guiHelper, 5, 40, recipe.preferredIntensity,
                recipe.intensityMargin);
    ...
    public List<String> getTooltipStrings(int mouseX, int mouseY) {
        if (TGBaseGui.isInRect(mouseX, mouseY, 66 + JEI_OFFSET_X, 60 + JEI_OFFSET_Y, 102, 14)) {
            ...
            tooltip.add(TextUtil.trans("techguns.container.reactionchamber.completion") + ": " + ChatFormatting.GREEN +
                    this.requiredCompletion);
            tooltip.add(TextUtil.trans("techguns.container.reactionchamber.recipeticks") + ": " + ChatFormatting.RED +
                    this.recipeticks);
            tooltip.add(TextUtil.trans("techguns.container.reactionchamber.time") + ": " +
                    (this.recipeticks * ReactionChamberOperation.RECIPE_TICKRATE) / 20 + "s");
            return tooltip;
            ...
            if (!stable) {
                tooltip.add(TextUtil.trans("techguns.container.reactionchamber.intensitymargin") + ": +/-" +
                        this.intensityMargin);
                tooltip.add(TextUtil.trans("techguns.container.reactionchamber.chance") + ": " +
                        ((int) (this.chanceToJump * 100)) + "%");
            } else {
            ...
            return tooltip;
        } else if (TGBaseGui.isInRect(mouseX, mouseY, 8 + BasicRecipeCategory.JEI_OFFSET_X,
                17 + BasicRecipeCategory.JEI_OFFSET_Y, 6, 60)) {

                    List<String> tooltip = new ArrayList<>();
                    tooltip.add(TextUtil.trans("techguns.container.power") + ":");

                    tooltip.add("-" + this.getRFperTick() + " " + PoweredTileEntGui.POWER_UNIT + "/" +
                            TextUtil.trans("techguns.container.reactionchamber.recipetick"));
                    tooltip.add(
                            "-" + this.getRFperTick() * this.requiredCompletion + " " + PoweredTileEntGui.POWER_UNIT);
    ...
    public void drawInfo(Minecraft minecraft, int recipeWidth, int recipeHeight, int mouseX, int mouseY) {
        ...
        Gui.drawModalRectWithCustomSizedTexture(JEI_OFFSET_X + 96, JEI_OFFSET_Y + 38, 205, 12 * level, 12, 12, 256,
                256);

        minecraft.fontRenderer.drawString(this.fluidlevel * 10 + "%", 34 + 1 + JEI_OFFSET_X, 50 + 1 + JEI_OFFSET_Y,
                4210752);
