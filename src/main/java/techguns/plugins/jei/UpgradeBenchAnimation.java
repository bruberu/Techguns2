...
public class UpgradeBenchAnimation implements IDrawableAnimated {
    ...
    public void draw(Minecraft minecraft, int xOffset, int yOffset) {
        ...
        drawModalRectWithCustomSizedTextureWithColor(k + 37, l + 40, 0 + x, 167 + y, 89, 31, 256, 256, c.getRed(),
                c.getGreen(), c.getBlue(), 255);
    }

    /**
     * Draws a textured rectangle at z = 0. Args: x, y, u, v, width, height, textureWidth, textureHeight
     */
    protected static void drawModalRectWithCustomSizedTextureWithColor(int x, int y, float u, float v, int width,
                                                                       int height, float textureWidth,
                                                                       float textureHeight, int r, int g, int b,
                                                                       int a) {
        ...
        bufferbuilder.pos((double) x, (double) (y + height), 0.0D)
                .tex((double) (u * f), (double) ((v + (float) height) * f1)).color(r, g, b, a).endVertex();
        bufferbuilder.pos((double) (x + width), (double) (y + height), 0.0D)
                .tex((double) ((u + (float) width) * f), (double) ((v + (float) height) * f1)).color(r, g, b, a)
                .endVertex();
        bufferbuilder.pos((double) (x + width), (double) y, 0.0D)
                .tex((double) ((u + (float) width) * f), (double) (v * f1)).color(r, g, b, a).endVertex();
        bufferbuilder.pos((double) x, (double) y, 0.0D).tex((double) (u * f), (double) (v * f1)).color(r, g, b, a)
                .endVertex();
        tessellator.draw();
    }
}
