...
public class OreDrillJeiRecipeCategory extends BasicRecipeCategory<OreDrillJeiRecipe> {
    ...
    public OreDrillJeiRecipeCategory(IGuiHelper guiHelper) {
        ...
        this.progress = guiHelper.createAnimatedDrawable(progress_static, 100, IDrawableAnimated.StartDirection.TOP,
                false);
    }
    ...
    public void setRecipe(IRecipeLayout recipeLayout, OreDrillJeiRecipe recipeWrapper, IIngredients ingredients) {
         ...
         */
        guiItemStacks.init(OreDrillTileEntMaster.SLOT_DRILL, true, OreDrillContainer.SLOT_DRILL_X + JEI_OFFSET_X,
                OreDrillContainer.SLOTS_ROW1_Y + JEI_OFFSET_Y);
        guiItemStacks.init(1, true, 69, 40);
        guiItemStacks.init(2, true, OreDrillContainer.SLOT_DRILL_X + JEI_OFFSET_X,
                OreDrillContainer.SLOTS_FURNACE_Y + JEI_OFFSET_Y);

        guiItemStacks.init(3, false, OreDrillContainer.SLOTS_OUTPUT_X + JEI_OFFSET_X,
                OreDrillContainer.SLOTS_ROW1_Y + JEI_OFFSET_Y);
        guiItemStacks.addTooltipCallback(new OreDrillOutputTooltipCallback(recipeWrapper));

        guiFluidStacks.init(0, true, OreDrillGui.INPUT_TANK_X + 2 + JEI_OFFSET_X, OreDrillGui.TANK_Y + 1 + JEI_OFFSET_Y,
                OreDrillGui.TANK_W, OreDrillGui.TANK_H, OreDrillTileEntMaster.CAPACITY_INPUT_TANK, false, tank_overlay);
        guiFluidStacks.init(1, false, OreDrillGui.OUTPUT_TANK_X + 2 + JEI_OFFSET_X,
                OreDrillGui.TANK_Y + 1 + JEI_OFFSET_Y, OreDrillGui.TANK_W, OreDrillGui.TANK_H,
                OreDrillTileEntMaster.CAPACITY_OUTPUT_TANK, false, tank_overlay);
    ...
    protected class OreDrillOutputTooltipCallback implements ITooltipCallback<ItemStack> {
        ...
        public void onTooltip(int slotIndex, boolean input, ItemStack ingredient, List<String> tooltip) {
            if (!input) {
                tooltip.add(tooltip.size() - 1, TextUtil.transTG("oredrill.jei.chance") + ": " +
                        String.format("%.2f", jeiRec.getChance() * 100f) + "%");
            }
        }
    }
