...
public class ChiselIMCHelper {
    ...
    public static <T extends Enum<T> & IStringSerializable> void addChiselVariants(String group,
                                                                                   GenericBlockMetaEnum<T> block,
                                                                                   Class<T> clazz) {
        for (T t : clazz.getEnumConstants()) {
            addChiselVariation(group, block.getRegistryName(),
                    block.getMetaFromState(block.getDefaultState().withProperty(block.TYPE, t)));
        }
    }
