...
public class DamageSystem {
    ...
    protected static Field ELB_idleTime = ReflectionHelper.findField(EntityLivingBase.class, "idleTime",
            "field_70708_bq");

    protected static Field ELB_lastDamage = ReflectionHelper.findField(EntityLivingBase.class, "lastDamage",
            "field_110153_bc");

    protected static Field ELB_recentlyHit = ReflectionHelper.findField(EntityLivingBase.class, "recentlyHit",
            "field_70718_bc");
    protected static Field ELB_attackingPlayer = ReflectionHelper.findField(EntityLivingBase.class, "attackingPlayer",
            "field_70717_bb");

    protected static Field ELB_lastDamageSource = ReflectionHelper.findField(EntityLivingBase.class, "lastDamageSource",
            "field_189750_bF");
    protected static Field ELB_lastDamageStamp = ReflectionHelper.findField(EntityLivingBase.class, "lastDamageStamp",
            "field_189751_bG");

    protected static Method ELB_canBlockDamageSource = ReflectionHelper.findMethod(EntityLivingBase.class,
            "canBlockDamageSource", "func_184583_d", DamageSource.class);

    protected static Method ELB_damageShield = ReflectionHelper.findMethod(EntityLivingBase.class, "damageShield",
            "func_184590_k", float.class);

    protected static Method ELB_blockUsingShield = ReflectionHelper.findMethod(EntityLivingBase.class,
            "blockUsingShield", "func_190629_c", EntityLivingBase.class);

    protected static Method ELB_damageEntity = ReflectionHelper.findMethod(EntityLivingBase.class, "damageEntity",
            "func_70665_d", DamageSource.class, float.class);

    protected static Method ELB_setBeenAttacked = ReflectionHelper.findMethod(EntityLivingBase.class,
            "markVelocityChanged", "func_70018_K");

    protected static Method ELB_checkTotemDeathProtection = ReflectionHelper.findMethod(EntityLivingBase.class,
            "checkTotemDeathProtection", "func_190628_d", DamageSource.class);

    protected static Method ELB_getDeathSound = ReflectionHelper.findMethod(EntityLivingBase.class, "getDeathSound",
            "func_184615_bR");
    protected static Method ELB_getSoundVolume = ReflectionHelper.findMethod(EntityLivingBase.class, "getSoundVolume",
            "func_70599_aP");
    protected static Method ELB_getSoundPitch = ReflectionHelper.findMethod(EntityLivingBase.class, "getSoundPitch",
            "func_70647_i");

    protected static Method ELB_playHurtSound = ReflectionHelper.findMethod(EntityLivingBase.class, "playHurtSound",
            "func_184581_c", DamageSource.class);

    protected static Method ELB_applyPotionDamageCalculations = ReflectionHelper.findMethod(EntityLivingBase.class,
            "applyPotionDamageCalculations", "func_70672_c", DamageSource.class, float.class);
    protected static Method ELB_damageArmor = ReflectionHelper.findMethod(EntityLivingBase.class, "damageArmor",
            ...
            if (armor != null) {
                ...
                if (item instanceof GenericArmor) {
                    ...
                    if (type == DamageType.PHYSICAL) {
                        value += ((ItemArmor) item).getArmorMaterial()
                                .getDamageReductionAmount(((ItemArmor) item).armorType);
                    }
                }
     ...
     */
    public static boolean attackEntityFrom(EntityLivingBase ent, DamageSource source,
                                           float amount) throws IllegalArgumentException, IllegalAccessException,
        ...
        if (ent.isEntityInvulnerable(source)) {
            ...
            if (ent.getHealth() <= 0.0F) {
                ...
                if ((source == DamageSource.ANVIL || source == DamageSource.FALLING_BLOCK) &&
                        !ent.getItemStackFromSlot(EntityEquipmentSlot.HEAD).isEmpty()) {
                    ent.getItemStackFromSlot(EntityEquipmentSlot.HEAD).damageItem(
                            (int) (amount * 4.0F + ((java.util.Random) ENT_rand.get(ent)).nextFloat() * amount * 2.0F),
                            ent);
                    amount *= 0.75F;
                }
                ...
                if (!dmgsrc.ignoreHurtresistTime &&
                        ((float) ent.hurtResistantTime > (float) ent.maxHurtResistantTime / 2.0F)) {
                ...
                if (flag1) {
                    ...
                    if (entity1 != null) {
                        ...
                        for (d0 = entity1.posZ - ent.posZ; d1 * d1 + d0 * d0 <
                                1.0E-4D; d0 = (Math.random() - Math.random()) * 0.01D) {
                            d1 = (Math.random() - Math.random()) * 0.01D;
                        }

                        ent.attackedAtYaw = (float) (MathHelper.atan2(d0, d1) * (180D / Math.PI) -
                ...
                if (ent.getHealth() <= 0.0F) {
                    ...
                    if (!((Boolean) ELB_checkTotemDeathProtection.invoke(ent, source))) {
                        ...
                        if (flag1 && soundevent != null) {
                            ent.playSound(soundevent, (float) ELB_getSoundVolume.invoke(ent),
                                    (float) ELB_getSoundPitch.invoke(ent));// ent.getSoundVolume(),
                                                                           // ent.getSoundPitch());
                        }
    ...
    public static void livingHurt(EntityLivingBase elb, DamageSource damageSrc,
                                  float damageAmount) throws IllegalAccessException, IllegalArgumentException,
                                                      InvocationTargetException {
     ...
     */
    public static float ELB_applyArmorCalculations(EntityLivingBase elb, DamageSource source,
                                                   float damage) throws IllegalAccessException,
        ...
        if (!source.isUnblockable()) {
            ...
            float toughness = (float) elb.getEntityAttribute(SharedMonsterAttributes.ARMOR_TOUGHNESS)
                    .getAttributeValue();
            ...
            damage = (float) getDamageAfterAbsorb_TGFormula(damage, tg.getTotalArmorAgainstType(dmgsrc), toughness,
                    dmgsrc.armorPenetration * 4);
        }
     ...
     * @return
     */
    public static double getDamageAfterAbsorb_TGFormula(float damage, float totalArmor, float toughnessAttribute,
                                                        float penetration) {
        // *New Formula*
