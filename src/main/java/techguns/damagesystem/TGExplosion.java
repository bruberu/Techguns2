...
public class TGExplosion {
    ...
    // public TGExplosion(World worldIn, Entity entityIn, double x, double y, double z, float size, List<BlockPos>
    // affectedPositions)
    // {
    ...
    //
    // @SideOnly(Side.CLIENT)
    // public TGExplosion(World worldIn, Entity entityIn, double x, double y, double z, float size, boolean causesFire,
    ...
    public TGExplosion(World world, Entity exploder, Entity projectile, double x, double y, double z,
    ...
    public void doExplosion(boolean playSound) {
        ...
        if (playSound) this.world.playSound((EntityPlayer) null, this.x, this.y, this.z,
                ...
                for (int l = -radius; l < radius; ++l) {
                    if (j == -radius || j == radius - 1 || k == -radius || k == radius - 1 || l == -radius ||
                        ...
                        for (int i = 0; i < steps && explosionPower > 0.0; i++) {
                            ...
                            else if (distance <= secondaryRadius) explosionPower = secondaryDamage +
                            ...
                            if (iblockstate.getMaterial() != Material.AIR) {
                                resistance = iblockstate.getBlock().getExplosionResistance(world, blockpos, exploder,
                                        explosionDummy);
                                // float f2 = this.exploder != null ? this.exploder.getExplosionResistance(this,
                                // this.world, blockpos, iblockstate) :
                                // iblockstate.getBlock().getExplosionResistance(world, blockpos, (Entity)null, this);
                                // f -= (f2 + 0.3F) * 0.3F;

                                if (explosionPower - explosionDamping > 0.0f &&
                                        resistance < (explosionPower - explosionDamping) * blockDamageFactor &&
                                        (this.exploder == null || this.exploder.canExplosionDestroyBlock(explosionDummy,
                                                this.world, blockpos, iblockstate, (float) explosionPower))) {
                                    set.put(blockpos, distance);
                                    if (prevPos == null || !(prevPos.getX() == blockpos.getX() &&
                                            prevPos.getY() == blockpos.getY() && prevPos.getZ() == blockpos.getZ())) {
        ...
        // <debug>
        // System.out.println(String.format("min = (%d, %d, %d); max = (%d, %d, %d)",min[0], min[1], min[2], max[0],
        // max[1], max[2]));
        // </debug>
        ...
        for (int k2 = 0; k2 < list.size(); ++k2) {
            ...
            if (!entity.isImmuneToExplosions() && GenericProjectile.BULLET_TARGETS.apply(entity)) {
                ...
                if (damage > 0.0) {
                    ...
                    float f = 1.0f;
                    if (exploder != null && exploder instanceof EntityLivingBase &&
        ...
        // System.out.println(String.format("BlockPositions: %d ms, BlockBreak: %d ms, Entities: %d ms", t_exp1-t_start,
        // t_exp2-t_exp1, t_exp3-t_exp2));
    }
    ...
    private void breakBlocks() {
        if (this.damagesTerrain) {
            ...
            for (Map.Entry<BlockPos, Double> entry : this.affectedBlockPositions.entrySet()) {
                ...
                if (iblockstate.getMaterial() != Material.AIR) {
                    // System.out.println(String.format("Dist: %.3f, primRad: %.3f", entry.getValue(),
                    // this.primaryRadius));
