...
public class InventoryUtil {
    ...
    public static int addAmmoToPlayerInventory(EntityPlayer ply, ItemStack ammo) {
        ...
        if (props != null) {
            int amount = addItemToInventory(props.tg_inventory.inventory, ammo, props.tg_inventory.SLOTS_AMMO_START,
                    props.tg_inventory.SLOTS_AMMO_END + 1);
            if (amount > 0) {
     ...
     * @return
     */
    public static int addItemToInventory(NonNullList<ItemStack> mainInventory, ItemStack item2, int startIndex,
                                         int endIndex) {
        ItemStack item = item2.copy();// TGItems.newStack(item2, item2.stackSize);

        for (int i = startIndex; i < endIndex; i++) {
            // 1st try a merge
            if (OreDictionary.itemMatches(mainInventory.get(i), item, true) &&
                    (mainInventory.get(i).getCount() < item.getMaxStackSize())) {
     ...
     * @return
     */
    public static int addItemToInventory(ItemStackHandlerPlus mainInventory, ItemStack item2, int startIndex,
                                         int endIndex) {
        ItemStack item = item2.copy();// TGItems.newStack(item2, item2.stackSize);
        ...
        for (int i = startIndex; i < endIndex; i++) {
            ...
            // 1st try a merge
            if (OreDictionary.itemMatches(mainInventory.getStackInSlot(i), item, true) &&
                    (mainInventory.getStackInSlot(i).getCount() < item.getMaxStackSize())) {
    ...
    public static boolean consumeAmmoPlayer(EntityPlayer ply, ItemStack ammo) {
        ...
        if (props != null) {
            ...
            if (amount == 1) {
                if (consumeAmmo(props.tg_inventory.inventory, ammo, TGPlayerInventory.SLOTS_AMMO_START,
                        TGPlayerInventory.SLOTS_AMMO_END + 1)) {
                    return true;
                } else {
                ...
                int needed = canConsumeItem(props.tg_inventory.inventory, ammo, TGPlayerInventory.SLOTS_AMMO_START,
                        TGPlayerInventory.SLOTS_AMMO_END + 1);
                ...
                if (needed + needed2 <= amount) {
                    ...
                    int missing = consumeItem(props.tg_inventory.inventory, ammo, TGPlayerInventory.SLOTS_AMMO_START,
                            TGPlayerInventory.SLOTS_AMMO_END + 1);

                    if (missing > 0) {
                        return consumeItem(ply.inventory.mainInventory, TGItems.newStack(ammo, missing), 0,
                                ply.inventory.mainInventory.size()) <= 0;
                    } else {
                        return true;
                    }
    ...
    public static boolean canConsumeAmmoPlayer(EntityPlayer ply, ItemStack ammo) {
        ...
        if (props != null) {
            int amount = ammo.getCount();
            if (amount == 1) {
                if (canConsumeItem(props.tg_inventory.inventory, ammo, TGPlayerInventory.SLOTS_AMMO_START,
                        TGPlayerInventory.SLOTS_AMMO_END + 1) <= 0) {
                    return true;
                } else {
                    return canConsumeItem(ply.inventory.mainInventory, ammo, 0, ply.inventory.mainInventory.size()) <=
                            0;
                }
            } else {
    ...
    public static int canConsumeItem(NonNullList<ItemStack> inv, ItemStack item, int startIndex, int endIndex) {
        ...
        for (int i = startIndex; i < endIndex; ++i) {
            if (!inv.get(i).isEmpty() && inv.get(i).getItem() == item.getItem() &&
                    inv.get(i).getItemDamage() == item.getItemDamage()) {
    ...
    public static int canConsumeItem(ItemStackHandler inv, ItemStack item, int startIndex, int endIndex) {
        ...
        for (int i = startIndex; i < endIndex; ++i) {
            if (!inv.getStackInSlot(i).isEmpty() && inv.getStackInSlot(i).getItem() == item.getItem() &&
                    inv.getStackInSlot(i).getItemDamage() == item.getItemDamage()) {
    ...
    /**
     * Consumes from multiple stacks, returns amount that could not be consumed, does not check if total ammount can be
     * consumed,
    ...
    private static int searchItem(NonNullList<ItemStack> inv, ItemStack stack, int startIndex, int endIndex) {
        for (int i = startIndex; i < endIndex; ++i) {
            if (!inv.get(i).isEmpty() && inv.get(i).getItem() == stack.getItem() &&
                    inv.get(i).getItemDamage() == stack.getItemDamage()) {
                return i;
            }
        }
    ...
    private static int searchItem(ItemStackHandler inv, ItemStack stack, int startIndex, int endIndex) {
        for (int i = startIndex; i < endIndex; ++i) {
            if (!inv.getStackInSlot(i).isEmpty() && inv.getStackInSlot(i).getItem() == stack.getItem() &&
                    inv.getStackInSlot(i).getItemDamage() == stack.getItemDamage()) {
                return i;
            }
        }
    ...
    public static int countItemInInv(NonNullList<ItemStack> inv, ItemStack stackToSearchFor, int startIndex,
                                     int endIndex) {
        int count = 0;

        for (int i = startIndex; i < endIndex; ++i) {
            if (!inv.get(i).isEmpty() && inv.get(i).getItem() == stackToSearchFor.getItem() &&
                    inv.get(i).getItemDamage() == stackToSearchFor.getItemDamage()) {
