...
public class BlockUtils {
    ...
    protected static boolean checkMiningLevels(World world, EntityPlayer ply, BlockPos b,
                                               GenericGunMeleeCharge miningtool, ItemStack stack) {
    ...
    public static List<BlockPos> getBlockPlaneAroundAxisForMining(World world, EntityPlayer ply, BlockPos center,
                                                                  EnumFacing.Axis axis, int radius,
                                                                  boolean includeCenter,
                                                                  @Nullable GenericGunMeleeCharge miningtool,
                                                                  ItemStack stack) {
    ...
    public static int getValidSpawnYWater(World world, int x, int z, int sizeX, int sizeZ, int heightDiffLimit) {
        ...
        if (!((MathUtil.abs(h0 - h1) < heightDiffLimit) && (MathUtil.abs(h0 - h2) < heightDiffLimit) &&
                (MathUtil.abs(h0 - h3) < heightDiffLimit))) {
            return -1;
        }
    ...
    public static int getValidSpawnYArea(World world, int x, int z, int sizeX, int sizeZ, int heightDiffLimit,
                                         int step) {
        int count = 0;
        int min = 255;
        int max = 0;
    ...
    private static int getTopSolidOrLiquidBlock(World w, int posx, int posz) {
        ...
        for (posz &= 15; k > 0; --k) {
            ...
            if ((block.getMaterial().blocksMovement() && block.getMaterial() != Material.LEAVES &&
                    !b.isFoliage(w, p.setPos(x, k, z))) || block.getMaterial().isLiquid()) {
                return k + 1;
            }
        }
    ...
    public static void removeJunkInArea(World world, int posX, int posZ, int sizeX, int sizeZ) {
        ...
        for (int x = 0; x < sizeX; x++) {
            for (int z = 0; z < sizeZ; z++) {
                int y = BlockUtils.getHeightValueTopBlockAll(world, posX + x, posZ + z);// world.getHeightValue(posX+x,
                                                                                        // posZ+z);
                boolean stop = false;
                for (; y > 0 && !stop; y--) {
    ...
    public static void fillBlocks(World world, MutableBlockPos start, int sizeX, int sizeY, int sizeZ,
                                  IBlockState block) {
        int x = start.getX();
        int y = start.getY();
        int z = start.getZ();
    ...
    public static void fillBlocksHollow(World world, MutableBlockPos start, int sizeX, int sizeY, int sizeZ,
                                        IBlockState block) {
        int x = start.getX();
        int y = start.getY();
        int z = start.getZ();
    ...
    public static BiomeColorType getBiomeType(World world, int posX, int posZ) {
        ...
        if (BiomeDictionary.hasType(biome, Type.SANDY) || BiomeDictionary.hasType(biome, Type.SAVANNA) ||
    ...
    public static void flattenArea(World world, int posX, int posZ, int sizeX, int sizeZ, int maxHeightDiff) {
        ...
        if (heights.size() > 2) {
            if (heights.size() % 2 == 0) {
                median = (heights.get(((int) Math.floor(heights.size() / 2)) - 1) +
                        heights.get(((int) Math.ceil(heights.size() / 2)) - 1)) / 2;
            } else {
            ...
            for (int z = 0; z < sizeZ; z++) {
                ...
                for (int i = 0; i < filter.length; i++) {
                    for (int j = 0; j < filter[i].length; j++) {
                        newY += (getHeightValueNoTrees(world, posX + x + xoffset + i, posZ + z + zoffset + j) - 1.0f) *
                                filter[i][j];
                    }
                }
    ...
    public static short[][] loadStructureFromFile(String filename) {
        try {
            ...
            BufferedReader br = new BufferedReader(
                    new InputStreamReader(WorldgenStructure.class.getResourceAsStream(path)));
    ...
    public static void cleanUpwards(World world, short[][] blocks, ArrayList<MBlock> blockList, int posX, int posY,
                                    int posZ, int centerX, int centerZ, int rotation, int pass, int range) {
        ...
        for (int i = 0; i < blocks.length; i++) {
            ...
            if (y == 0) {
                ...
                for (int o = 1; o <= range; o++) {
                    ...
                    BlockUtils.setMBlockRotated(world, block, p.setPos(posX + x, posY + y + o, posZ + z), axis,
                            rotation, null, BiomeColorType.WOODLAND);

                    // setBlockRotatedReplaceAirOnly(world, block, posX, posY-o, posZ, x, z, centerX, centerZ,
                    // rotation);
    ...
    public static void placeScannedStructure(World world, short[][] blocks, ArrayList<MBlock> blockList, int posX,
                                             int posY, int posZ, int centerX, int centerZ, int rotation, int pass,
                                             EnumLootType loottype, BiomeColorType biome) {
        placeScannedStructure(world, blocks, blockList, posX, posY, posZ, centerX, centerZ, rotation, pass, loottype,
                biome, 0, null);
    }

    public static void placeScannedStructure(World world, short[][] blocks, ArrayList<MBlock> blockList, int posX,
                                             int posY, int posZ, int centerX, int centerZ, int rotation, int pass,
                                             EnumLootType loottype, BiomeColorType biome, int indexRoll, Random rnd) {
        // System.out.println("Blocks.length = "+blocks.length + "Blocks[0].length = "+blocks[0].length);
        MutableBlockPos p = new MutableBlockPos();
        BlockPos axis = new BlockPos(posX + centerX, 1, posZ + centerZ);
        for (int i = 0; i < blocks.length; i++) {
            ...
            if (block.getPass() == pass) {
                if (block instanceof MultiMMBlockIndexRoll) {
                    BlockUtils.setMBlockRotatedIndexRoll(world, (MultiMMBlockIndexRoll) block,
                            p.setPos(posX + x, posY + y, posZ + z), axis, rotation, loottype, biome, indexRoll, rnd);
                } else {
                    BlockUtils.setMBlockRotated(world, block, p.setPos(posX + x, posY + y, posZ + z), axis, rotation,
                            loottype, biome);
                }
    ...
    public static void placeFoundation(World world, short[][] blocks, ArrayList<MBlock> blockList, int posX, int posY,
                                       int posZ, int centerX, int centerZ, int rotation, int pass, int range) {
        // System.out.println("Blocks.length = "+blocks.length + "Blocks[0].length = "+blocks[0].length);
        MutableBlockPos p = new MutableBlockPos();
        BlockPos axis = new BlockPos(posX + centerX, 1, posZ + centerZ);
            ...
            if (y == 0) {
                ...
                if (block.getPass() == pass) {
                    ...
                    for (int o = 1; o <= range; o++) {
                         ...
                         * BlockUtils.setBlockRotated(world, block, posX, posY-o, posZ, x, z, centerX, centerZ,
                         * rotation);
                         * }
                         */
                        // setBlockRotatedReplaceAirOnly(world, block, posX, posY-o, posZ, x, z, centerX, centerZ,
                        ...
                        setMBlockRotatedReplaceableOnly(world, block, p.setPos(posX + x, posY + y - o, posZ + z), axis,
                                rotation, null);
                        // world.setBlock(posX+x, posY-o, posZ+z, Blocks.diamond_block);
                    }
                }
     ...
     */
    public static void placeFoundationNether(World world, short[][] blocks, ArrayList<MBlock> blockList, int posX,
                                             int posY, int posZ, int centerX, int centerZ, int rotation, int pass,
                                             int range) {
        // System.out.println("Blocks.length = "+blocks.length + "Blocks[0].length = "+blocks[0].length);
        MutableBlockPos p = new MutableBlockPos();
                ...
                if (block.getPass() == pass) {
                    ...
                    for (int o = 1; o <= range; o++) {
                        ...
                        if (getIsBlockReplaceable(world, new MutableBlockPos(p), axis, rotation)) {
                            setMBlockRotatedReplaceableOnly(world, block, p.setPos(posX + x, posY + y - o, posZ + z),
                                    axis, rotation, null);
                            solidcount = 0;
                        } else {
                            solidcount++;
    ...
    public static void setMBlockRotated(World w, MBlock mblock, MutableBlockPos pos, BlockPos axis, int rotation,
                                        EnumLootType loottype, BiomeColorType biome) {
        BlockUtils.rotateAroundY(pos, axis, rotation);
        mblock.setBlock(w, pos, rotation, loottype, biome);
    }

    public static void setMBlockRotatedIndexRoll(World w, MultiMMBlockIndexRoll mblock, MutableBlockPos pos,
                                                 BlockPos axis, int rotation, EnumLootType loottype,
                                                 BiomeColorType biome, int indexRoll, Random rnd) {
        BlockUtils.rotateAroundY(pos, axis, rotation);
        mblock.setBlock(w, pos, rotation, loottype, biome, indexRoll, rnd);
    }

    public static void setMBlockRotatedReplaceableOnly(World w, MBlock mblock, MutableBlockPos pos, BlockPos axis,
                                                       int rotation, EnumLootType loottype) {
        BlockUtils.rotateAroundY(pos, axis, rotation);
     ...
     */
    public static void fillSphere2(World world, int posX, int posY, int posZ, float radius, MBlock block1,
                                   MBlock block2) {
        fillSphere2(world, posX, posY, posZ, radius, 1.0f, block1, block2);
    }

    public static void fillSphere2(World world, int posX, int posY, int posZ, float radius, float thickness,
                                   MBlock block1, MBlock block2) {
    ...
    public static void fillCylinder(World world, int x1, int y1, int z1, int x2, int y2, int z2, float radius,
                                    MBlock block1, MBlock block2) {
