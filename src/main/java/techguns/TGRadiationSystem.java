...
public class TGRadiationSystem implements ITGInitializer {
    ...
    public void preInit(FMLPreInitializationEvent event) {
        TGRadiation.RADIATION_RESISTANCE = (new RangedAttribute((IAttribute) null,
                Techguns.MODID + ".radiationResistance", 0.0D, 0.0D, Float.MAX_VALUE)).setShouldWatch(true);

        radiation_effect = new RadiationPotion();
        radiation_effect.setPotionName(Techguns.MODID + ".radiation")
                .setRegistryName(new ResourceLocation(Techguns.MODID, "radiation"));

        radregen_effect = new RadRegenerationPotion();
        radregen_effect.setPotionName(Techguns.MODID + ".radregeneration")
                .setRegistryName(new ResourceLocation(Techguns.MODID, "radregeneration"));

        radresistance_effect = new RadResistancePotion();
        radresistance_effect.setPotionName(Techguns.MODID + ".radresistance")
                .setRegistryName(new ResourceLocation(Techguns.MODID, "radresistance")).setBeneficial()
                .registerPotionAttributeModifier(TGRadiation.RADIATION_RESISTANCE,
                        "515AD21C-3FB2-4E36-BBCE-88C2ED738DE2", 1D, 0);
    }
    ...
    public static void applyRadToEntities(TileEntity tile, double radius, int duration, int strength,
                                          double inner_radius, int strength_outer) {
        applyRadToEntities(tile.getWorld(), tile.getPos().getX() + 0.5, tile.getPos().getY() + 0.5,
                tile.getPos().getZ() + 0.5, radius, duration, strength, inner_radius, strength_outer);
    }

    public static void applyRadToEntities(World world, double posX, double posY, double posZ, double radius,
                                          int duration, int strength, double inner_radius, int strength_outer) {
