        ...
        if (elb instanceof EntityPlayer) {
            if (!((EntityPlayer) elb).capabilities.isCreativeMode) {
                ...
                if (elb.world.isRemote && amount > 0) {
                    if (amplifier >= 2) {
                        elb.world.playSound((EntityPlayer) elb, elb.posX, elb.posY, elb.posZ, TGSounds.GEIGER_HIGH,
                                SoundCategory.PLAYERS, 1f, 1f);
                    } else {
                        elb.world.playSound((EntityPlayer) elb, elb.posX, elb.posY, elb.posZ, TGSounds.GEIGER_LOW,
                                SoundCategory.PLAYERS, 1f, 1f);
                    }
                }
