...
public class FabricatorTileEntMaster extends MultiBlockMachineTileEntMaster {
    ...
    public AxisAlignedBB getRenderBoundingBox() {
        if (this.isFormed()) {
            ...
            BlockPos first = p.offset(multiblockDirection.getOpposite(), 2).offset(left.getOpposite(), 1)
    ...
    protected void checkAndStartOperation() {
        ...
        RecipeData data = FabricatorRecipe.getRecipeDataFor(this.inventory.getStackInSlot(SLOT_INPUT1),
                this.inventory.getStackInSlot(FabricatorTileEntMaster.SLOT_WIRES),
                this.inventory.getStackInSlot(FabricatorTileEntMaster.SLOT_POWDER),
                this.inventory.getStackInSlot(FabricatorTileEntMaster.SLOT_PLATE),
                this.getMaxMachineUpgradeMultiplier(SLOT_UPGRADE),
                this.inventory.getStackInSlot(FabricatorTileEntMaster.SLOT_OUTPUT));
        if (data != null) {
    ...
    protected void playAmbientSound() {
        if (this.world.isRemote && this.progress == 1) {
            ...
            world.playSound(this.getPos().getX(), this.getPos().getY(), this.getPos().getZ(), TGSounds.FABRICATOR_WORK,
