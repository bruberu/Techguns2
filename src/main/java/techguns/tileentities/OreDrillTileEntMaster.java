...
public class OreDrillTileEntMaster extends MultiBlockMachineTileEntMaster implements ITileEntityFluidTanks {
    ...
    protected boolean isItemValidForSlot(int slot, ItemStack stack) {
        ...
        if (slot == SLOT_DRILL && stack.getItem() instanceof ITGSpecialSlot) {
            ...
            return slotType == TGSlotType.DRILL_SMALL || slotType == TGSlotType.DRILL_MEDIUM ||
                    slotType == TGSlotType.DRILL_LARGE;
    ...
    public AxisAlignedBB getRenderBoundingBox() {
        if (this.isFormed()) {
            ...
            BlockPos other = rod_end.offset(OreDrillDefinition.getSide1(drill_direction), +(radius + 2))
                    .offset(OreDrillDefinition.getSide2(drill_direction), +(radius + 1));
    ...
    protected boolean consumePower(int amount) {
        ...
        if (this.fuelBuffer * FACTOR >= amount) {
            ...
            if (fuelValue > 0) {
                ItemStack container = this.inventory.getStackInSlot(SLOT_FURNACE).getItem()
                        .getContainerItem(this.inventory.getStackInSlot(SLOT_FURNACE));// FluidContainerRegistry.drainFluidContainer(this.content[10]);

                if (!container.isEmpty()) {
                    // int leftover = Techguns.addItemToInventory(this.content,container, 1, 10);
                    int leftover = InventoryUtil.addItemToInventory(inventory, container, SLOT_OUTPUT1,
                            SLOT_OUTPUT1 + SLOT_OUTPUT_COUNT);
                    // if (!this.worldObj.isRemote){
                    if (leftover > 0 && !this.world.isRemote) {
                        ItemStack entityItem = container.copy();
                        entityItem.setCount(leftover);
                        this.world.spawnEntity(new EntityItem(this.world, this.pos.getX() + 0.5, this.pos.getY() + 0.5,
                                this.pos.getZ() + 0.5, entityItem));
                    }
                }
    ...
    protected boolean checkDrill() {
        if (this.world.getTotalWorldTime() % 20 == 0) {
            // System.out.println("CheckDrill Last:"+this.lastDrill+"
            // Current:"+this.inventory.getStackInSlot(SLOT_DRILL));
            if (this.lastDrill.isEmpty() && this.inventory.getStackInSlot(SLOT_DRILL).isEmpty()) {
                ...
                if (this.inventory.getStackInSlot(SLOT_DRILL).isEmpty() ||
                        !this.lastDrill.isItemEqual(this.inventory.getStackInSlot(SLOT_DRILL))) {
            ...
            if (OreDrillDefinition.isOreCluster(this.world, targetPos)) {
                ...
                if (drillheadmininglevel > 0) {
                    ...
                    if (effectivemininglevel >= 0) {
                        ...
                        double orePerHour = ((effectiveclustersize * 3) +
                                (effectiveclustersize * effectivemininglevel * 0.5)) * TGConfig.oreDrillMultiplierOres *
                                cluster.getMultiplier_amount();

                        this.currentOperation = cluster.getNewOperation(this.world, orePerHour, this.getDrillRadius(),
                                cluster.getMultiplier_power());
                        this.progress = 0;

                        this.totaltime = (int) (60.0 / orePerHour * 1200.0);
                        // moved to operation
                        // this.currentRFperTick=(int)(80*orePerHour*Math.max(this.radius,
                        // 1)*TGConfig.oreDrillMultiplierPower*cluster.getMultiplier_power());
                    } else {
    ...
    protected void playAmbientSound() {
        ...
        if (!this.world.isRemote && soundLoopDelay-- <= 0) {
            if (this.radius == 0) {
                // world.playSound(this.getPos().getX(), this.getPos().getY(), this.getPos().getZ(),
                // TGSounds.ORE_DRILL_WORK_SMALL, SoundCategory.BLOCKS, SOUND_VOLUME, 1.0F, true );
                // worldplaySound(this.xCoord, this.yCoord, this.zCoord, "techguns:machines.oredrillSmallWork", 1.5F,
                // 1.0F, true );
                TGPackets.network.sendToAllTracking(
                        new PacketPlaySound(TGSounds.ORE_DRILL_WORK_SMALL, soundPos.getX(), soundPos.getY(),
                                soundPos.getZ(), SOUND_VOLUME, 1.0f, false, TGSoundCategory.MACHINE),
                        new TargetPoint(this.world.provider.getDimension(), soundPos.getX() + 0.5,
                                soundPos.getY() + 0.5, soundPos.getZ() + 0.5, 32.0f));
                soundLoopDelay = 61; // 62 ticks = 3.1 sec

            } else if (this.radius <= 2) {
                // world.playSound(this.xCoord, this.yCoord, this.zCoord, "techguns:machines.oredrillMediumWork", 2.25F,
                // 1.0F, true );
                // world.playSound(this.getPos().getX(), this.getPos().getY(), this.getPos().getZ(),
                // TGSounds.ORE_DRILL_WORK_MEDIUM, SoundCategory.BLOCKS, SOUND_VOLUME, 1.0F, true );

                TGPackets.network.sendToAllTracking(
                        new PacketPlaySound(TGSounds.ORE_DRILL_WORK_MEDIUM, soundPos.getX(), soundPos.getY(),
                                soundPos.getZ(), SOUND_VOLUME, 1.0f, false, TGSoundCategory.MACHINE),
                        new TargetPoint(this.world.provider.getDimension(), soundPos.getX() + 0.5,
                                soundPos.getY() + 0.5, soundPos.getZ() + 0.5, 32.0f));
                soundLoopDelay = 61; // 62 ticks = 3.1 sec
            } else {
                // world.playSound(this.xCoord, this.yCoord, this.zCoord, "techguns:machines.oredrillLargeWork", 3.0F,
                // 1.0F, true );
                // world.playSound(this.getPos().getX(), this.getPos().getY(), this.getPos().getZ(),
                // TGSounds.ORE_DRILL_WORK_LARGE, SoundCategory.BLOCKS, SOUND_VOLUME, 1.0F, true );

                TGPackets.network.sendToAllTracking(
                        new PacketPlaySound(TGSounds.ORE_DRILL_WORK_LARGE, soundPos.getX(), soundPos.getY(),
                                soundPos.getZ(), SOUND_VOLUME, 1.0f, false, TGSoundCategory.MACHINE),
                        new TargetPoint(this.world.provider.getDimension(), soundPos.getX() + 0.5,
                                soundPos.getY() + 0.5, soundPos.getZ() + 0.5, 32.0f));
                soundLoopDelay = 61; // 62 ticks = 3.1 sec
    ...
    public int getClusterSize(int count, BlockPos startPos, ArrayList<BlockPos> visited, OreCluster clusterType,
                              IBlockState clusterMeta) {
    ...
    public <T> T getCapability(Capability<T> capability, EnumFacing facing) {
        return capability == CapabilityFluidHandler.FLUID_HANDLER_CAPABILITY ? (T) fluidHandler :
    ...
    public static class OreDrillFluidHandler implements IFluidHandler {
        ...
        public IFluidTankProperties[] getTankProperties() {
            if (tankProperties == null) {
                this.tankProperties = new IFluidTankProperties[] { new FluidTankPropertiesWrapper(tile.inputTank),
                        new FluidTankPropertiesWrapper(tile.outputTank) };
            }
