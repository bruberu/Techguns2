...
public class ChargingStationTileEnt extends BasicMachineTileEnt {
    ...
    protected int getMaxOutputAmount(ItemStack item, int maxMulti) {
        if (this.inventory.getStackInSlot(SLOT_OUTPUT).isEmpty()) {
            ...
            if (this.inventory.getStackInSlot(SLOT_OUTPUT).getItem() == item.getItem() &&
                    this.inventory.getStackInSlot(SLOT_OUTPUT).getItemDamage() == item.getItemDamage()) {
                int multi = maxMulti;
                while (multi > 0 && (inventory.getStackInSlot(SLOT_OUTPUT).getCount() + item.getCount() * multi) >
                        inventory.getStackInSlot(SLOT_OUTPUT).getMaxStackSize()) {
                    multi--;
                }
                return multi;
            }
    ...
    protected void playAmbientSound() {
        if (this.world.isRemote) {
            ...
            if (distSq < 100) {
                ...
                int blockrot = this.world.getBlockState(this.getPos()).getValue(SimpleMachine.FACING)
                        .getHorizontalIndex();

                if (blockrot != 2) {
                    Techguns.proxy.createFX("ChargeFlare2", this.world, this.getPos().getX() + 0.5d,
                            this.getPos().getY() + 0.5d, this.getPos().getZ() + 0.3d, 0, 0, 0);
                }
                if (blockrot != 0) {
                    Techguns.proxy.createFX("ChargeFlare2", this.world, this.getPos().getX() + 0.5d,
                            this.getPos().getY() + 0.5d, this.getPos().getZ() + 0.7d, 0, 0, 0);
                }
                if (blockrot != 1) {
                    Techguns.proxy.createFX("ChargeFlare2", this.world, this.getPos().getX() + 0.3d,
                            this.getPos().getY() + 0.5d, this.getPos().getZ() + 0.5d, 0, 0, 0);
                }
                if (blockrot != 3) {
                    Techguns.proxy.createFX("ChargeFlare2", this.world, this.getPos().getX() + 0.7d,
                            this.getPos().getY() + 0.5d, this.getPos().getZ() + 0.5d, 0, 0, 0);
                }
            }
            ...
            if (this.lastsound <= 0) {
                ...
                world.playSound(this.getPos().getX(), this.getPos().getY(), this.getPos().getZ(),
                        TGSounds.CHARGING_STATION_WORK, SoundCategory.BLOCKS, SOUND_VOLUME, 1.0F, false);
            }
