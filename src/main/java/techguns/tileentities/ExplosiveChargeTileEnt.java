...
public class ExplosiveChargeTileEnt extends BasicOwnedTileEnt implements ITickable {
    ...
    protected void explode() {
        ...
        if (!this.world.isRemote) {
            this.world.playSound((EntityPlayer) null, this.pos, SoundEvents.ENTITY_GENERIC_EXPLODE,
            ...
            this.getPos()
                    .getAllInBoxMutable(pos.offset(side1, rad).offset(side2, rad),
                            pos.offset(side1.getOpposite(), rad).offset(side2.getOpposite(), rad))
                    .forEach(p -> {
                ...
                if (!breakEvent.isCanceled()) {
                    ...
                    if (world.rand.nextFloat() > 0.5f) {
                        TGPackets.network.sendToAllAround(
                                new PacketSpawnParticle("MiningChargeBlockExplosion", coords.getX() + 0.5d,
                                        coords.getY() + 0.5d, coords.getZ() + 0.5d),
                                TGPackets.targetPointAroundBlockPos(this.world.provider.getDimension(), coords, 50));
                    }
