    ...
    public void update() {
        ...
        if (this.delay <= 0 && activeMobs.size() < Math.min(maxActive, mobsLeft) && this.hasMobTypes()) {
            ...
            if (this.world.getDifficulty() != EnumDifficulty.PEACEFUL) {
                ...
                WeightedSpawnerEntity entdata = WeightedRandom.<WeightedSpawnerEntity>getRandomItem(this.rand,
                        this.mobtypes);
                ...
                if (entity != null && entity instanceof ITGSpawnerNPC && entity instanceof EntityLiving) {
                    ...
                    // if (net.minecraftforge.event.ForgeEventFactory.canEntitySpawnSpawner(npc, this.world,
                    // (float)entity.posX, (float)entity.posY, (float)entity.posZ))
                    // {
                    if (!net.minecraftforge.event.ForgeEventFactory.doSpecialSpawn(elb, this.world, (float) entity.posX,
                            (float) entity.posY, (float) entity.posZ, null)) {
                        elb.onInitialSpawn(world.getDifficultyForLocation(new BlockPos(entity)),
                        ...
                        if (!weaponOverride.isEmpty() && elb instanceof EntityLiving) {
                            ((EntityLiving) elb).setItemStackToSlot(EntityEquipmentSlot.MAINHAND,
                                    this.weaponOverride.copy());
                        }
                ...
                while (it.hasNext()) {
                    ...
                    if (npc instanceof EntityLivingBase) {
                        ...
                        if (!ent.isEntityAlive() ||
                                ent.world.provider.getDimension() != this.world.provider.getDimension()) {
                            it.remove();
                        }
                    } else {
                        it.remove();
