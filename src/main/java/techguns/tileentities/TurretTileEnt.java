        ...
        this.inventory = new ItemStackHandlerPlus(SLOT_ARMOR + 1) {
            ...
            protected boolean allowItemInSlot(int slot, ItemStack stack) {
                if (slot == SLOT_WEAPON) {
                    ...
                    return stack.getItem() instanceof ITGSpecialSlot &&
            ...
            for (int i = 0; i < emptyMag.length; i++) {
                ...
                if (InventoryUtil.consumeAmmo(this.inventory, ammo[i], SLOT_INPUT1, SLOT_INPUT1 + INPUTS_SIZE)) {
                    if (!emptyMag[i].isEmpty()) {
                        int tooMuch = InventoryUtil.addItemToInventory(this.inventory, TGItems.newStack(emptyMag[i], 1),
                                SLOT_OUTPUT1, SLOT_OUTPUT1 + OUTPUTS_SIZE);
                        if (!this.world.isRemote) {
                            this.world.spawnEntity(new EntityItem(this.world, this.pos.getX(), this.pos.getY() + 1,
                                    this.pos.getZ(), TGItems.newStack(emptyMag[i], tooMuch)));
                        }
                    }

                    if (gun.getAmmoCount() > 1) {
                        int j = 1;
                        while (j < gun.getAmmoCount() && InventoryUtil.consumeAmmo(this.inventory, ammo[i], SLOT_INPUT1,
                                SLOT_INPUT1 + INPUTS_SIZE)) {
                            j++;
                        }
