...
public class AmmoPressTileEnt extends BasicMachineTileEnt {
    ...
    protected void checkAndStartOperation() {
        ...
        ItemStack output = AmmoPressBuildPlans.getOutputFor(this.inventory.getStackInSlot(SLOT_METAL1),
                this.inventory.getStackInSlot(SLOT_METAL2), this.inventory.getStackInSlot(SLOT_POWDER), this.buildPlan);
        ...
        if (!output.isEmpty() && canOutput(output, SLOT_OUTPUT)) {
            ...
            for (i = maxStack; i > 1; --i) {
                ...
                if (canOutput(outputStack, SLOT_OUTPUT)) {
                    if (this.inputMetal1.canConsume(i) && this.inputMetal2.canConsume(2 * i) &&
                            this.inputPowder.canConsume(i)) {
                        // output=outputStack;
                        multiplier = i;
                        break;
                    }
