...
public class MetalPressTileEnt extends BasicMachineTileEnt {
    ...
    protected void checkAndStartOperation() {
        ...
        if (this.autoSlitMode > 0) {
            if (!this.input1.get().isEmpty() && this.input2.get().isEmpty() && this.input1.get().getCount() > 1) {
                ...
                if (this.input1.get().isEmpty() && !this.input2.get().isEmpty() && this.input2.get().getCount() > 1) {
        ...
        ItemStack output = MetalPressRecipes.getOutputFor(input1.get(), input2.get());// AmmoPressBuildPlans.getOutputFor(content[0],content[1],
                                                                                      // this.buildPlan);//AmmoPressRecipe.isRecipe(input);
    ...
    protected void playAmbientSound() {
        ...
        if (this.progress == soundTick1 || this.progress == soundTick1 + halfTime) {
            world.playSound(this.getPos().getX(), this.getPos().getY(), this.getPos().getZ(), TGSounds.METAL_PRESS_WORK,
    ...
    public int getValidSlotForItemInMachine(ItemStack item) {
        if (!this.input1.get().isEmpty() && OreDictionary.itemMatches(this.input1.get(), item, true)) {
            ...
            return SLOT_INPUT1;
                ...
                (!MetalPressRecipes.getOutputFor(this.input1.get(), item).isEmpty())) {
                    return SLOT_INPUT2;
                } else
            if (!item.isEmpty() && item.getItem() == TGItems.MACHINE_UPGRADE_STACK.getItem() &&
                    item.getItemDamage() == TGItems.MACHINE_UPGRADE_STACK.getItemDamage()) {
                        return SLOT_UPGRADE;
                    }
        return -1;
    }
