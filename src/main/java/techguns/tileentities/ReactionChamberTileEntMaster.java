...
public class ReactionChamberTileEntMaster extends MultiBlockMachineTileEntMaster
                                          implements ITileEntityFluidTanks, SimpleComponent {
    ...
    public static Field playerChunkMapEntry_Players = ObfuscationReflectionHelper.findField(PlayerChunkMapEntry.class,
            "field_187283_c"); // ReflectionHelper.findField(PlayerChunkMapEntry.class, "players","field_187283_c");
    ...
    public <T> T getCapability(Capability<T> capability, EnumFacing facing) {
        return capability == CapabilityFluidHandler.FLUID_HANDLER_CAPABILITY ? (T) inputTank :
    ...
    public AxisAlignedBB getRenderBoundingBox() {
        if (this.isFormed()) {
            ...
            BlockPos first = p.offset(multiblockDirection.getOpposite(), 1).offset(left.getOpposite(), 2)
    ...
    public void update() {
        ...
        if (this.isRedstoneEnabled()) {
            ...
            if (currentReaction != null && currentReaction.getRecipe() != null) {
                ...
                this.progress++;
                boolean state = currentReaction.tick(this.intensity, this.liquidLevel, this.world.isRemote, this,
                        currentReaction.getRecipe().RFTick);
    ...
    protected void checkAndStartOperation() {
        ...
        ReactionChamberRecipe rec = ReactionChamberRecipe.getMatchingRecipe(this.inventory.getStackInSlot(SLOT_INPUT),
                this.inventory.getStackInSlot(SLOT_FOCUS), this.inputTank.getFluid(), this.liquidLevel, this.intensity);
        if (rec != null) {
    ...
    private void mergeOutput() {
        ...
        for (int i = 0; i < outputs.size(); i++) {
            ...
            if (leftover > 0) {
                this.world.spawnEntity(new EntityItem(this.world, this.pos.getX() + 0.5d, this.pos.getY() + 0.5d,
                        this.pos.getZ() + 0.5d, out));
            }

        }

        if (this.getCurrentReaction().getRecipe().liquidConsumtion > 0) {
            ((ReactionChamberFluidTank) this.inputTank)
                    .removeInternal(this.getCurrentReaction().getRecipe().liquidConsumtion);
        }
    }
    ...
    protected void playReactionTickSound(boolean goodTick) {
        ...
        if (focusitem != null) {
            ...
            if (focus != null) {
                world.playSound(pos.getX(), pos.getY(), pos.getZ(), focus.getSound(), SoundCategory.BLOCKS, 1.0F, 1.0F,
                        true);
            }
        }
        ...
        for (int i = 0; i < 4; i++) {
            world.spawnParticle(EnumParticleTypes.SPELL, pos.getX() + rng.nextFloat(), pos.getY() + rng.nextFloat(),
                    pos.getZ() + rng.nextFloat(), 0, 1, 0);
        }

        if (goodTick) {
            world.playSound(pos.getX(), pos.getY(), pos.getZ(), TGSounds.REACTION_CHAMBER_BEEP, SoundCategory.BLOCKS,
                    1.0F, 1.0F, true);
        } else {
            world.playSound(pos.getX(), pos.getY(), pos.getZ(), TGSounds.REACTION_CHAMBER_WARNING, SoundCategory.BLOCKS,
                    1.0F, 1.0F, true);
        }
    }
    ...
    public int getValidSlotForItemInMachine(ItemStack stack) {
        if (!stack.isEmpty() && stack.getItem() instanceof ITGSpecialSlot) {
            ...
            if (itm.getSlot(stack) == TGSlotType.REACTION_CHAMBER_FOCUS &&
                    ReactionBeamFocus.getBeamFocus(stack) != null) {
                return SLOT_FOCUS;
            }
        }
        return SLOT_INPUT;
        ...
        if (!this.world.isRemote) {
            ...
            if (type == 0) {
                ...
                TGPackets.network.sendToAllAround(
                        new PacketSpawnParticle("FragGrenadeExplosion", centerPos.getX() + 0.5d,
                                centerPos.getY() + 0.5d, centerPos.getZ() + 0.5d),
                        new TargetPoint(this.world.provider.getDimension(), centerPos.getX() + 0.5d,
                                centerPos.getY() + 0.5d, centerPos.getZ() + 0.5d, 50.0f));
                this.world.playSound((EntityPlayer) null, centerPos.getX() + 0.5d, centerPos.getY() + 0.5d,
                        centerPos.getZ() + 0.5d, SoundEvents.ENTITY_GENERIC_EXPLODE, SoundCategory.BLOCKS, 4.0F,
                        (1.0F + (this.world.rand.nextFloat() - this.world.rand.nextFloat()) * 0.2F) * 0.7F);

            } else if (type == 1) {
                // int radius=4;
                // TGPackets.network.sendToAllAround(new PacketSpawnParticle("RocketExplosion", centerPos.x+0.5d,
                // centerPos.y+0.5d, centerPos.z+0.5d), new TargetPoint(this.worldObj.provider.dimensionId,
                // centerPos.x+0.5d, centerPos.y+0.5d, centerPos.z+0.5d, 50.0f));
                // ProjectileExplosion explosion = new ProjectileExplosion(worldObj, centerPos.x+0.5d, centerPos.y+0.5d,
                // centerPos.z+0.5d, null, radius, 25, radius*0.5f, radius*1.5f);
                // explosion.setExplosionSound("techguns:effects.explosion1");
                // explosion.doExplosion(true, null);

                // EntityTNTPrimed entitytntprimed = new EntityTNTPrimed(this.worldObj, centerPos.x+0.5d,
                // centerPos.y+0.5d, centerPos.z+0.5d, null);
                // this.worldObj.spawnEntityInWorld(entitytntprimed);

                this.world.createExplosion(null, centerPos.getX() + 0.5d, centerPos.getY() + 0.5d,
                        centerPos.getZ() + 0.5d, 4, true);
            }
            ...
            if (!content.isEmpty()) {
                this.inventory.setStackInSlot(SLOT_INPUT, ItemStack.EMPTY);
                this.world.spawnEntity(new EntityItem(this.world, centerPos.getX() + 0.5d, centerPos.getY() + 0.5d,
                        centerPos.getZ() + 0.5d, content));
            }
    ...
    protected class ReactionChamberFluidTank extends FluidTankPlus {
        ...
        public int fillInternal(FluidStack resource, boolean doFill) {
            ...
            if (fluid == null) {
                ...
                if (tile != null) {
                    FluidEvent.fireEvent(new FluidEvent.FluidFillingEvent(fluid, tile.getWorld(), tile.getPos(), this,
                            fluid.amount));
                }
                return fluid.amount;
            }
        ...
        public FluidStack drainInternal(int maxDrain, boolean doDrain) {
            ...
            if (doDrain) {
                ...
                if (tile != null) {
                    FluidEvent.fireEvent(
                            new FluidEvent.FluidDrainingEvent(fluid, tile.getWorld(), tile.getPos(), this, drained));
                }
            }
            return stack;
        }
