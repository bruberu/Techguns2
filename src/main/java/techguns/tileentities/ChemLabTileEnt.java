...
public class ChemLabTileEnt extends BasicMachineTileEnt implements ITileEntityFluidTanks {
    ...
    protected void playAmbientSound() {
        ...
        if (this.progress == soundTick1 || this.progress == soundTick1 + halfTime) {
            // worldObj.playSound(this.xCoord, this.yCoord, this.zCoord, "techguns:machines.chemlabWork", 1.0F, 1.0F,
            // true);
            world.playSound(this.getPos().getX(), this.getPos().getY(), this.getPos().getZ(), TGSounds.CHEM_LAB_WORK,
                    SoundCategory.BLOCKS, SOUND_VOLUME, 1.0F, true);
        }

        if (this.world.isRemote) {
            int delay = 10;
            if (this.progress % delay == 0) {
                world.spawnParticle(EnumParticleTypes.SPELL, this.getPos().getX() + 0.5d + rng.nextFloat(),
                        this.getPos().getY() + 0.5d + rng.nextFloat(), this.getPos().getZ() + 0.5d + rng.nextFloat(), 0,
                        1, 0);
            }
        }
    }
    ...
    public int getValidSlotForItemInMachine(ItemStack item) {
        if (ChemLabRecipes.allowInFlaskSlot(item)) {
            ...
            return SLOT_INPUT1;
        } else if (!this.input1.get().isEmpty() && this.input2.get().isEmpty() &&
                (ChemLabRecipes.allowAsInput2(this.input1.get(), item))) {
                    return SLOT_INPUT2;
                } else
    ...
    @Override
    public boolean onFluidContainerInteract(EntityPlayer player, EnumHand hand, IFluidHandlerItem fluidhandleritem,
                                            ItemStack stack) {
        ...
        if (this.drainInput) {
            ...
            if (playerInventory != null) {
                FluidActionResult fluidActionResult = FluidUtil.tryFillContainerAndStow(stack, this.outputTank,
                        playerInventory, Integer.MAX_VALUE, player, true);
                if (!fluidActionResult.isSuccess()) {
                    fluidActionResult = FluidUtil.tryEmptyContainerAndStow(stack, this.inputTank, playerInventory,
