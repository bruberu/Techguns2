...
public class OreDrillTileEntSlave extends MultiBlockMachineTileEntSlave {
    ...
    public boolean shouldRefresh(World world, BlockPos pos, IBlockState oldState, IBlockState newState) {
        if (newState.getBlock() == oldState.getBlock()) {
            if (oldState.getValue(getMachineBlockType().MACHINE_TYPE) == EnumOreDrillType.SCAFFOLD &&
                    newState.getValue(getMachineBlockType().MACHINE_TYPE) == EnumOreDrillType.SCAFFOLD_HIDDEN) {
                return false;
            }
        }
