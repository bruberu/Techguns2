...
public class ReactionChamberRecipe implements IMachineRecipe {
    ...
    public static ReactionChamberRecipe addRecipe(String ID, ItemStackOreDict input,
        ...
        ReactionChamberRecipe rec = new ReactionChamberRecipe(ID, input, beamFocus, liquidIn, output, ticks,
                requiredCompletion, preferredIntensity, intensityMargin, liquidLevel, (short) liquidConsumtion,
                instability, risk, RFTick);
    ...
    public static ReactionChamberRecipe addRecipe(String ID, ItemStackOreDict input,
                                                  ...
                                                  int preferredIntensity, int intensityMargin, int liquidLevel,
                                                  int liquidConsumtion, float instability, RiskType risk, int RFTick) {
        ReactionChamberRecipe rec = new ReactionChamberRecipe(ID, input, beamFocus, liquidIn, outputs, ticks,
                requiredCompletion, preferredIntensity, intensityMargin, liquidLevel, (short) liquidConsumtion,
                instability, risk, RFTick);
        recipes.put(rec.ID, rec);
        return rec;
    ...
    public static ReactionChamberRecipe getMatchingRecipe(ItemStack input, ItemStack focus, FluidStack tank,
                                                          byte liquidLevel, byte intensity) {
    ...
    public boolean matches(ItemStack input, ItemStack focus, FluidStack tank, byte liquidLevel, byte intensity) {
        if (!input.isEmpty() && this.input.isEqualWithOreDict(input)) {
            ...
            if (f != null && f == this.beamFocus) {
                if (tank != null && tank.getFluid() == this.liquidIn && liquidLevel == this.liquidLevel &&
                        tank.amount == (this.liquidLevel * Fluid.BUCKET_VOLUME)) {
    ...
    public enum RiskType {
        ...
        public static boolean isFatal(RiskType type) {
            return !(type == EXPLOSION_LOW || type == EXPLOSION_HIGH || type == EXPLOSION_MEDIUM ||
                    type == UNFORSEEN_CONSEQUENCES);
        }
    }
