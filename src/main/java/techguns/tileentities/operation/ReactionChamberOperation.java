...
public class ReactionChamberOperation extends MachineOperation {
    ...
    public boolean tick(byte intensity, byte liquidLevel, boolean client, ReactionChamberTileEntMaster tile,
                        int RFTick) {
        ...
        if (!client && nextTick == 0 && progress < totalTime) {
            ...
            boolean focusMatch = focusMatches(
                    tile.getInventory().getStackInSlot(ReactionChamberTileEntMaster.SLOT_FOCUS));

            // do recipe tick
            if (powered && focusMatch && this.required_intensity == intensity &&
                    this.recipe.liquidLevel == liquidLevel) {
                completion++;
    ...
    public void doRadiation(ReactionChamberTileEntMaster tile, boolean success) {
        if (!tile.getWorld().isRemote && TGRadiationSystem.isEnabled()) {
            ReactionBeamFocus focus = ReactionBeamFocus
                    .getBeamFocus(tile.getInventory().getStackInSlot(ReactionChamberTileEntMaster.SLOT_FOCUS));
            if (focus != null) {
                int rad = 0;
                if (success) {
                ...
                if (rad > 0) {
                    ...
                    TGRadiationSystem.applyRadToEntities(tile, focus.rad_radius_outer, 62, rad - 1,
                            focus.rad_radius_inner, 0);
