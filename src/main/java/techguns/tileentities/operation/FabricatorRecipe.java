...
public class FabricatorRecipe implements IMachineRecipe {
    ...
    public static void addRecipe(ItemStackOreDict inputItem, int amountInput, ItemStackOreDict wireSlot, int amountWire,
                                 ItemStackOreDict powderSlot,
                                 int amountPowder, ItemStackOreDict plateSlot, int amountPlates, ItemStack outputItem,
                                 int amountOutput) {
        recipes.add(new FabricatorRecipe(inputItem, amountInput, wireSlot, amountWire, powderSlot, amountPowder,
                plateSlot, amountPlates, outputItem, amountOutput));
    }

    public boolean usesItem(ItemStack itm) {
        return this.inputItem.isEqualWithOreDict(itm) || this.wireSlot.isEqualWithOreDict(itm) ||
                this.powderSlot.isEqualWithOreDict(itm) || this.plateSlot.isEqualWithOreDict(itm);
    }

    public FabricatorRecipe(ItemStackOreDict inputItem, int amountInput, ItemStackOreDict wireSlot, int amountWire,
                            ItemStackOreDict powderSlot,
                            int amountPowder, ItemStackOreDict plateSlot, int amountPlates, ItemStack outputItem,
                            int amountOutput) {
    ...
    private boolean matches(ItemStack input, ItemStack wires, ItemStack powder, ItemStack plate) {
        if (this.inputItem.isEqualWithOreDict(input) && input.getCount() >= this.amountInput) {
            if (this.wireSlot.isEmpty() || (!wires.isEmpty() && this.wireSlot.isEqualWithOreDict(wires) &&
                    wires.getCount() >= this.amountWire)) {
                if (this.powderSlot.isEmpty() || (!powder.isEmpty() && this.powderSlot.isEqualWithOreDict(powder) &&
                        powder.getCount() >= this.amountPowder)) {
                    if (this.plateSlot.isEmpty() || (!plate.isEmpty() && this.plateSlot.isEqualWithOreDict(plate) &&
                            plate.getCount() >= this.amountPlates)) {
                        return true;
                    }
                }
            }
    ...
    public static RecipeData getRecipeDataFor(ItemStack input, ItemStack wires, ItemStack powder, ItemStack plate,
                                              int maxMultiplier, ItemStack outputSlot) {
        ...
        if (match != null && canMerge(outputSlot, match.outputItem, match.amountOutput)) {
            ...
            int maxMultiOutput = getMaxMultiOutput(outputSlot.isEmpty() ? 0 : outputSlot.getCount(), match.amountOutput,
                    maxMultiplier, match.outputItem.getMaxStackSize());

            int multi = getMinArgument(maxMultiInput, maxMultiWires, maxMultiPowder, maxMultiPlate, maxMultiOutput);

            RecipeData data = new RecipeData(match.outputItem, match.amountOutput, match.amountInput, match.amountWire,
                    match.amountPowder, match.amountPlates, multi);
            return data;
        }
    ...
    public static class RecipeData {
        ...
        public RecipeData(ItemStack output, int outputAmount, int inputConsumption, int wireConsumption,
                          int powderConsumption, int plateConsumption, int stackMultiplier) {
            super();
            this.output = output;
