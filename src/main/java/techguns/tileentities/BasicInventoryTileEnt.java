...
public class BasicInventoryTileEnt extends BasicTGTileEntity {
    ...
    protected boolean canOutput(ItemStack output, int slot) {
        ...
        if (this.inventory.getStackInSlot(slot).isEmpty()) {
            ...
            if (this.inventory.getStackInSlot(slot).isItemEqual(output) &&
                    (this.inventory.getStackInSlot(slot).getCount() + output.getCount() <=
                            this.inventory.getStackInSlot(slot).getMaxStackSize())) {
                return true;
            } else {
                return false;
            }
    ...
    public boolean shouldRefresh(World world, BlockPos pos, IBlockState oldState, IBlockState newState) {
        if (!this.hasRotation) {
            return (oldState.getBlock() != newState.getBlock()) ||
                    (oldState.getValue(this.getMachineBlockType().MACHINE_TYPE) !=
                            newState.getValue(this.getMachineBlockType().MACHINE_TYPE));
        } else {
