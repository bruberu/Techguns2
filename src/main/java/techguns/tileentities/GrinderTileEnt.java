    ...
    protected void playAmbientSound() {
        ...
        if (this.progress % 2 == 0) {
            ...
            if (it.getItem() instanceof GenericGun) {
                this.world.spawnParticle(EnumParticleTypes.ITEM_CRACK, this.pos.getX() + 0.5d, this.pos.getY() + 0.6d,
                        this.pos.getZ() + 0.5d, Math.random() * f - f1, Math.random() * f + f, Math.random() * f - f1,
                        Item.getIdFromItem(TGItems.PLATE_STEEL.getItem()), TGItems.PLATE_STEEL.getItemDamage());

            } else {

                this.world.spawnParticle(EnumParticleTypes.ITEM_CRACK, this.pos.getX() + 0.5d, this.pos.getY() + 0.6d,
                        this.pos.getZ() + 0.5d, Math.random() * f - f1, Math.random() * f + f, Math.random() * f - f1,
                        Item.getIdFromItem(it.getItem()), it.getItemDamage());
            }
            // }
        }
        // }
        if (this.progress == 1) {
            world.playSound(this.getPos().getX(), this.getPos().getY(), this.getPos().getZ(), TGSounds.GRINDER_START,
                    SoundCategory.BLOCKS, 1.0f, 1.0F, true);
        } else if (this.progress == sWork1 || this.progress == sWork2) {
            world.playSound(this.getPos().getX(), this.getPos().getY(), this.getPos().getZ(), TGSounds.GRINDER_WORK,
                    SoundCategory.BLOCKS, 1.0f, 1.0F, true);
        }
    }
}
