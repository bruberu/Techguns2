...
public class BlockSandbags extends GenericBlock {
    ...
    protected static final AxisAlignedBB[] BOUNDING_BOXES = new AxisAlignedBB[] {
            new AxisAlignedBB(size_1, 0.0D, size_1, size_2, 1.0D, size_2),
            new AxisAlignedBB(size_1, 0.0D, size_1, size_2, 1.0D, 1.0D),
            new AxisAlignedBB(0.0D, 0.0D, size_1, size_2, 1.0D, size_2),
            new AxisAlignedBB(0.0D, 0.0D, size_1, size_2, 1.0D, 1.0D),
            new AxisAlignedBB(size_1, 0.0D, 0.0D, size_2, 1.0D, size_2),
            new AxisAlignedBB(size_1, 0.0D, 0.0D, size_2, 1.0D, 1.0D),
            new AxisAlignedBB(0.0D, 0.0D, 0.0D, size_2, 1.0D, size_2),
            new AxisAlignedBB(0.0D, 0.0D, 0.0D, size_2, 1.0D, 1.0D),
            new AxisAlignedBB(size_1, 0.0D, size_1, 1.0D, 1.0D, size_2),
            new AxisAlignedBB(size_1, 0.0D, size_1, 1.0D, 1.0D, 1.0D),
            new AxisAlignedBB(0.0D, 0.0D, size_1, 1.0D, 1.0D, size_2),
            new AxisAlignedBB(0.0D, 0.0D, size_1, 1.0D, 1.0D, 1.0D),
            new AxisAlignedBB(size_1, 0.0D, 0.0D, 1.0D, 1.0D, size_2),
            new AxisAlignedBB(size_1, 0.0D, 0.0D, 1.0D, 1.0D, 1.0D),
            new AxisAlignedBB(0.0D, 0.0D, 0.0D, 1.0D, 1.0D, size_2),
    ...
    public boolean canConnectTo(IBlockAccess worldIn, BlockPos pos, EnumFacing facing) {
        ...
        boolean flag = blockfaceshape == BlockFaceShape.MIDDLE_POLE &&
                (iblockstate.getMaterial() == this.material || block instanceof BlockFenceGate);
    ...
    protected static boolean isExcepBlockForAttachWithPiston(Block p_194142_0_) {
        return Block.isExceptBlockForAttachWithPiston(p_194142_0_) || p_194142_0_ == Blocks.BARRIER ||
                p_194142_0_ == Blocks.MELON_BLOCK || p_194142_0_ == Blocks.PUMPKIN || p_194142_0_ == Blocks.LIT_PUMPKIN;
    }
    ...
    private boolean canConnectCorner(IBlockAccess world, BlockPos pos, EnumFacing f1, EnumFacing f2) {
        return canFenceConnectTo(world, pos, f1) && canFenceConnectTo(world, pos, f2) &&
                (canFenceConnectTo(world, pos.offset(f1), f2) || canFenceConnectTo(world, pos.offset(f2), f1));
    }
     ...
     * {
     * case CLOCKWISE_180:
     * return state.withProperty(NORTH, state.getValue(SOUTH)).withProperty(EAST,
     * state.getValue(WEST)).withProperty(SOUTH, state.getValue(NORTH)).withProperty(WEST, state.getValue(EAST));
     * case COUNTERCLOCKWISE_90:
     * return state.withProperty(NORTH, state.getValue(EAST)).withProperty(EAST,
     * state.getValue(SOUTH)).withProperty(SOUTH, state.getValue(WEST)).withProperty(WEST, state.getValue(NORTH));
     * case CLOCKWISE_90:
     * return state.withProperty(NORTH, state.getValue(WEST)).withProperty(EAST,
     * state.getValue(NORTH)).withProperty(SOUTH, state.getValue(EAST)).withProperty(WEST, state.getValue(SOUTH));
     * default:
     * return state;
     * }
     * }
    ...
    protected BlockStateContainer createBlockState() {
        return new BlockStateContainer(this,
                new IProperty[] { NORTH, EAST, WEST, SOUTH, CORNER_NE, CORNER_ES, CORNER_SW, CORNER_WN });
    }
    ...
    public void registerItemBlockModels() {
        ModelLoader.setCustomModelResourceLocation(this.itemblock, 0,
                new ModelResourceLocation(getRegistryName() + "_inventory", "inventory"));
    }
