...
public class SimpleMachine<T extends Enum<T> & IStringSerializable & IMachineType> extends BasicMachine<T> {
    ...
    public static final PropertyDirection FACING = BlockHorizontal.FACING; // PropertyDirection.create("facing",
    ...
    public void registerItemBlockModels() {
        for (int i = 0; i < clazz.getEnumConstants().length; i++) {
            ...
            if (clazz.getEnumConstants()[i].hasCustomModelLocation()) {
                clazz.getEnumConstants()[i].setCustomModelLocation(this.itemblock, this.getMetaFromState(state),
                        getRegistryName(), state);
            } else {
