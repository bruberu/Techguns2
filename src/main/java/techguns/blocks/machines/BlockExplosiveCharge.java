...
public class BlockExplosiveCharge<T extends Enum<T> & IStringSerializable & IMachineType> extends BasicMachine<T> {
    ...
    public static final PropertyDirection FACING = BlockDirectional.FACING; // PropertyDirection.create("facing",
    ...
    public BlockExplosiveCharge(String name, Class<T> clazz) {
        super(name, clazz);
        this.blockStateOverride = new BlockStateContainer.Builder(this).add(FACING).add(MACHINE_TYPE).add(ARMED)
                .build();
