...
public enum EnumSimpleMachineType2 implements IStringSerializable, IMachineType<EnumSimpleMachineType2> {
    ...
    private EnumSimpleMachineType2(int id, Class<? extends TileEntity> tile, boolean isFullCube,
                                   EnumBlockRenderType renderType) {
        this(id, tile, isFullCube, renderType, BlockRenderLayer.SOLID);
    }

    private EnumSimpleMachineType2(int id, Class<? extends TileEntity> tile, boolean isFullCube,
                                   EnumBlockRenderType renderType, BlockRenderLayer layer) {
        this.id = id;
    ...
    public void setCustomModelLocation(Item itemblock, int meta, ResourceLocation registryName, IBlockState state) {
        ResourceLocation loc = new ResourceLocation(Techguns.MODID,
                registryName.getPath() + "_" + this.name().toLowerCase() + "_inv");
