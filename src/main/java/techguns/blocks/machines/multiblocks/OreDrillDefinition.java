...
public class OreDrillDefinition extends MultiBlockMachineSchematic {
    ...
    protected static final EnumFacing[][] DIRECTIONS_AROUND_AXIS = {
            /* X */{ EnumFacing.DOWN, EnumFacing.NORTH, EnumFacing.UP, EnumFacing.SOUTH },
    ...
    public boolean checkForm(World w, EntityPlayer player, BlockPos masterPos, EnumFacing direction) {
        ...
        if (dir < 0) {
            sendErrorMSG(w, masterPos, player,
                    PacketMultiBlockFormInvalidBlockMessage.MSG_TYPE_ORE_DRILL_CONTROLLER_ALONE);
            return false;
        }
        ...
        if (engines >= 0 && rods > 0 && (rods + engines <= MAX_SIZE)) {
            ...
            if (this.checkDrillSize(rods, engines, rad)) {
                // check engine block
                if (allBlocksMatch(w, player, getEngineBlocks(w, masterPos, drill_direction, rad, engines),
                        TGBlocks.ORE_DRILL_BLOCK.getDefaultState()
                                .withProperty(TGBlocks.ORE_DRILL_BLOCK.MACHINE_TYPE, EnumOreDrillType.ENGINE)
                                .withProperty(TGBlocks.ORE_DRILL_BLOCK.FORMED, false),
                        false)) {
                    // engines OK, check outer frame
                    if (allBlocksMatch(w, player,
                            getFrameBlocks(w, masterPos, drill_direction, rad + 1, engines + rods, true, true),
                            TGBlocks.ORE_DRILL_BLOCK.getDefaultState()
                        ...
                        if (allBlocksMatch(w, player,
                                getFrameBlocks(w, masterPos, drill_direction, rad + 1, engines + rods, false, true),
                                TGBlocks.ORE_DRILL_BLOCK.getDefaultState()
                                        .withProperty(TGBlocks.ORE_DRILL_BLOCK.MACHINE_TYPE, EnumOreDrillType.SCAFFOLD)
                                        .withProperty(TGBlocks.ORE_DRILL_BLOCK.FORMED, false),
                                false)) {

                            if (allBlocksMatch(w, player, getAirBlocks(w, masterPos.offset(drill_direction, engines),
                                    drill_direction, rad, rods - 1), Blocks.AIR.getDefaultState(), false)) {
                                return true;
                            } else {
                                // Air Block Error
                                sendErrorMSG(w, masterPos, player,
                                        PacketMultiBlockFormInvalidBlockMessage.MSG_TYPE_ORE_DRILL_AIR_ERROR);
                            }

                        } else {
                            // Scaffold Error
                            sendErrorMSG(w, masterPos, player,
                                    PacketMultiBlockFormInvalidBlockMessage.MSG_TYPE_ORE_DRILL_SCAFFOLD_ERROR);
                        }

                    } else {
                        // Frame Error
                        sendErrorMSG(w, masterPos, player,
                                PacketMultiBlockFormInvalidBlockMessage.MSG_TYPE_ORE_DRILL_FRAME_ERROR);
                    }

                } else {
                    // Engine Error
                    sendErrorMSG(w, masterPos, player,
                            PacketMultiBlockFormInvalidBlockMessage.MSG_TYPE_ORE_DRILL_ENGINE_ERROR);
                }
            } else {
    ...
    protected ArrayList<BlockPos> getEngineBlocks(World w, BlockPos masterPos, EnumFacing direction, int radius,
                                                  int len) {
        ArrayList<BlockPos> positions = new ArrayList<BlockPos>((radius * 2 + 1) * (radius * 2 + 1) * len);

        for (int r1 = -radius; r1 <= radius; r1++)
            for (int r2 = -radius; r2 <= radius; r2++) for (int h = 0; h < len; h++) {
                BlockPos p = masterPos.offset(direction, h + 1).offset(getSide1(direction), r1)
                        .offset(getSide2(direction), r2);
                positions.add(p);
            }
        return positions;
    }
    ...
    protected ArrayList<BlockPos> getAirBlocks(World w, BlockPos startPos, EnumFacing direction, int radius, int len) {
        ...
        for (int r1 = -radius; r1 <= radius; r1++)
            for (int r2 = -radius; r2 <= radius; r2++) for (int h = 0; h < len; h++) {
                ...
                if (!(r1 == 0 && r2 == 0)) {
                    BlockPos p = startPos.offset(direction, h + 1).offset(getSide1(direction), r1)
                            .offset(getSide2(direction), r2);
                    positions.add(p);
                }
            }
    ...
    protected ArrayList<BlockPos> getFrameBlocks(World w, BlockPos masterPos, EnumFacing direction, int radius, int len,
                                                 boolean outer, boolean lastRow) {
        ArrayList<BlockPos> positions = new ArrayList<BlockPos>();
    ...
    protected ArrayList<BlockPos> getFinalScaffoldRow(World w, BlockPos masterPos, EnumFacing direction, int radius,
                                                      int len) {
        ...
        for (int r1 = -radius; r1 <= radius; r1++) for (int r2 = -radius; r2 <= radius; r2++) {
            if (!(r1 == 0 && r2 == 0)) {
                BlockPos p = masterPos.offset(direction, len).offset(getSide1(direction), r1)
                        .offset(getSide2(direction), r2);
                positions.add(p);
            }
        }
    ...
    public boolean form(World w, EntityPlayer player, BlockPos masterPos, EnumFacing direction) {
        ...
        if (tile != null && tile instanceof OreDrillTileEntMaster) {
            ...
            if (engines >= 0 && rods > 0) {
                ...
                this.getEngineBlocks(w, masterPos, drill_direction, rad, engines)
                        .forEach(p -> linkSlave(w, player, p, 1, masterPos));

                this.getFrameBlocks(w, masterPos, drill_direction, rad + 1, engines + rods, true, true)
                        .forEach(p -> linkSlave(w, player, p, 2, masterPos));
                this.getFrameBlocks(w, masterPos, drill_direction, rad + 1, engines + rods, false, false)
                        .forEach(p -> linkSlave(w, player, p, 3, masterPos));
                this.getFinalScaffoldRow(w, masterPos, drill_direction, rad, engines + rods).forEach(p -> {
                    linkSlave(w, player, p, 4, masterPos);
    ...
    protected void updateBlockStateForm(World w, BlockPos pos, int type) {
        if (type == 4) {
            w.setBlockState(pos, w.getBlockState(pos).withProperty(MultiBlockMachine.FORMED, true)
                    .withProperty(TGBlocks.ORE_DRILL_BLOCK.MACHINE_TYPE, EnumOreDrillType.SCAFFOLD_HIDDEN), 3);
        } else {
    ...
    protected void updateBlockStateUnform(World w, BlockPos pos, IBlockState bs, int type) {
        if (bs.getValue(TGBlocks.ORE_DRILL_BLOCK.MACHINE_TYPE) == EnumOreDrillType.SCAFFOLD_HIDDEN) {
            w.setBlockState(pos, w.getBlockState(pos).withProperty(MultiBlockMachine.FORMED, false)
                    .withProperty(TGBlocks.ORE_DRILL_BLOCK.MACHINE_TYPE, EnumOreDrillType.SCAFFOLD), 3);
        } else {
    ...
    public void unform(World w, MultiBlockMachineTileEntMaster master) {
        if (master instanceof OreDrillTileEntMaster) {
            ...
            this.getFrameBlocks(w, masterPos, drill_direction, rad + 1, engines + rods, true, true)
                    .forEach(p -> unlinkSlave(w, p));
            this.getFrameBlocks(w, masterPos, drill_direction, rad + 1, engines + rods, false, false)
                    .forEach(p -> unlinkSlave(w, p));
            this.getFinalScaffoldRow(w, masterPos, drill_direction, rad, engines + rods)
                    .forEach(p -> unlinkSlave(w, p));
