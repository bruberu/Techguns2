...
public class ReactionChamberDefinition extends MultiBlockMachineSchematic {
    ...
    static {
        ...
        // top row corners
        boundingBoxes.put(new SlavePos(-1, 3, -1),
                new AxisAlignedBB(MIDDLE_DIFF, 0, MIDDLE_DIFF, 1, 1 - HEIGHT_DIFF, 1));
        boundingBoxes.put(new SlavePos(1, 3, -1),
                new AxisAlignedBB(0, 0, MIDDLE_DIFF, 1 - MIDDLE_DIFF, 1 - HEIGHT_DIFF, 1));
        boundingBoxes.put(new SlavePos(-1, 3, 1),
                new AxisAlignedBB(MIDDLE_DIFF, 0, 0, 1, 1 - HEIGHT_DIFF, 1 - MIDDLE_DIFF));
        boundingBoxes.put(new SlavePos(1, 3, 1),
                new AxisAlignedBB(0, 0, 0, 1 - MIDDLE_DIFF, 1 - HEIGHT_DIFF, 1 - MIDDLE_DIFF));
    ...
    public boolean checkForm(World w, EntityPlayer player, BlockPos masterPos, EnumFacing direction) {
        ...
        // IBlockState unformedController =
        // TGBlocks.MULTIBLOCK_MACHINE.getDefaultState().withProperty(TGBlocks.MULTIBLOCK_MACHINE.MACHINE_TYPE,
        // EnumMultiBlockMachineType.REACTIONCHAMBER_CONTROLLER).withProperty(MultiBlockMachine.FORMED, false);
        IBlockState unformedHousing = TGBlocks.MULTIBLOCK_MACHINE.getDefaultState()
                .withProperty(TGBlocks.MULTIBLOCK_MACHINE.MACHINE_TYPE,
                        EnumMultiBlockMachineType.REACTIONCHAMBER_HOUSING)
                .withProperty(MultiBlockMachine.FORMED, false);
        IBlockState unformedGlass = TGBlocks.MULTIBLOCK_MACHINE.getDefaultState()
                .withProperty(TGBlocks.MULTIBLOCK_MACHINE.MACHINE_TYPE, EnumMultiBlockMachineType.REACTIONCHAMBER_GLASS)
                .withProperty(MultiBlockMachine.FORMED, false);
