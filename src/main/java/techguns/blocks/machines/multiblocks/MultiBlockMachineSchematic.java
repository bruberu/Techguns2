...
public abstract class MultiBlockMachineSchematic {
    ...
    protected static boolean allBlocksMatch(World w, EntityPlayer player, ArrayList<BlockPos> positions,
                                            IBlockState blockstate) {
        return allBlocksMatch(w, player, positions, blockstate, false);
    }

    protected static boolean allBlocksMatch(World w, EntityPlayer player, ArrayList<BlockPos> positions,
                                            IBlockState blockstate, boolean message) {
        boolean valid = true;
        for (int i = 0; i < positions.size(); i++) {
            ...
            if (w.getBlockState(p) != blockstate) {
                /*
                 * if(!w.isRemote) {
                 * TGPackets.network.sendTo(new PacketSpawnParticle("MultiblockInvalidPing", p.getX()+0.5d,
                 * p.getY()+0.5d, p.getZ()+0.5d), (EntityPlayerMP) player);
    ...
    protected static void sendErrorPing(World w, BlockPos p, EntityPlayer player, int type, boolean message) {
        if (!w.isRemote) {
            TGPackets.network.sendTo(
                    new PacketSpawnParticle("MultiblockInvalidPing", p.getX() + 0.5d, p.getY() + 0.5d, p.getZ() + 0.5d),
                    (EntityPlayerMP) player);
            if (message)
                TGPackets.network.sendTo(new PacketMultiBlockFormInvalidBlockMessage(p, type), (EntityPlayerMP) player);
        }
    }

    protected static void sendErrorMSG(World w, BlockPos p, EntityPlayer player, int msgID) {
        if (!w.isRemote) {
            // TGPackets.network.sendTo(new PacketSpawnParticle("MultiblockInvalidPing", p.getX()+0.5d, p.getY()+0.5d,
            // p.getZ()+0.5d), (EntityPlayerMP) player);
