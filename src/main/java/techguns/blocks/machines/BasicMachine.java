...
public class BasicMachine<T extends Enum<T> & IStringSerializable & IMachineType> extends GenericBlock {
    ...
    public void getSubBlocks(CreativeTabs tab, NonNullList<ItemStack> items) {
        for (T t : clazz.getEnumConstants()) {
            if ((!t.debugOnly() || TGConfig.debug) && !(t.hideInCreative())) {
                items.add(
                        new ItemStack(this, 1, this.getMetaFromState(getDefaultState().withProperty(MACHINE_TYPE, t))));
            }
        }
    }
            ...
            if (tileent.isUseableByPlayer(player)) {
                ...
                if (!helditem.isEmpty() && helditem.getItem().getToolClasses(helditem).contains("wrench")) {
                    ...
                    if (player.isSneaking() && tileent.canBeWrenchDismantled() && !world.isRemote) {
                        ...
                        world.spawnEntity(
                                new EntityItem(world, pos.getX() + 0.5d, pos.getY() + 0.5d, pos.getZ() + 0.5d, item));
                ...
                } else if (!helditem.isEmpty() && hasBucketInteraction(state) &&
                        helditem.hasCapability(CapabilityFluidHandler.FLUID_HANDLER_ITEM_CAPABILITY, null)) {

                            IFluidHandlerItem fluidhandler = helditem
                            ...
                            if (interacted) {
                                if (!world.isRemote) {
                                    world.playSound(null, pos.getX() + 0.5d, pos.getY() + 0.5d, pos.getZ() + 0.5d,
                                            SoundEvents.ITEM_BUCKET_FILL, SoundCategory.BLOCKS, 1f, 1f);
                                }
                ...
                player.sendStatusMessage(new TextComponentString(TextUtil.trans("techguns.container.security.denied")),
                        true);
            }
    ...
    public void onBlockPlacedByExtended(World world, BlockPos pos, IBlockState state, EntityLivingBase placer,
                                        ItemStack stack, EnumFacing sideHit) {
        TileEntity tile = world.getTileEntity(pos);
    ...
    public void registerItemBlockModels() {
        for (int i = 0; i < clazz.getEnumConstants().length; i++) {
            ...
            if (clazz.getEnumConstants()[i].getRenderType() == EnumBlockRenderType.MODEL) {
                // ModelLoader.setCustomModelResourceLocation(itemblock, i, new
                // ModelResourceLocation(this.getRegistryName(),BlockUtils.getBlockStateVariantString(state)));
                ModelLoader.setCustomModelResourceLocation(itemblock, i,
                        new ModelResourceLocation(
                                new ResourceLocation(Techguns.MODID, clazz.getEnumConstants()[i].name().toLowerCase()),
                                "inventory"));
            } else {
                ForgeHooksClient.registerTESRItemStack(itemblock, this.getMetaFromState(state),
                        state.getValue(MACHINE_TYPE).getTileClass());
                ModelLoader.setCustomModelResourceLocation(itemblock, i,
