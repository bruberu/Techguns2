...
public class BlockTGDoor3x3<T extends Enum<T> & IStringSerializable> extends GenericBlock {
    ...
    public static final PropertyEnum<EnumDoorState> STATE = PropertyEnum.<EnumDoorState>create("state",
            EnumDoorState.class);
    ...
    public BlockTGDoor3x3(String name, Class<T> clazz, ItemTGDoor3x3<T> doorplacer) {
        ...
        this.clazz = clazz;
        this.blockStateOverride = new BlockStateContainer.Builder(this).add(MASTER).add(ZPLANE).add(STATE).add(TYPE)
                .build();
        this.setDefaultState(this.getBlockState().getBaseState().withProperty(MASTER, false).withProperty(ZPLANE, false)
                .withProperty(STATE, EnumDoorState.CLOSED).withProperty(TYPE, EnumDoorType.METAL));
    ...
    public void registerBlock(Register<Block> event) {
        ...
        GameRegistry.registerTileEntity(Door3x3TileEntity.class,
                new ResourceLocation(Techguns.MODID, "door3x3tileent"));
    }
    ...
    public void toggleState(World w, BlockPos masterPos, boolean checkNeighbours) {
        ...
        if (sound != null) {
            w.playSound(null, masterPos, sound, SoundCategory.BLOCKS, (float) (0.9 + Math.random() * 0.2),
                    (float) (0.9 + Math.random() * 0.2));
        }
    ...
    protected boolean isMatchingDoorType(IBlockState master, IBlockState other) {
        ...
        if (type == EnumDoorType.HANGAR_DOWN || type == EnumDoorType.HANGAR_UP) {
            return master.withProperty(TYPE, EnumDoorType.HANGAR_DOWN) == other ||
                    master.withProperty(TYPE, EnumDoorType.HANGAR_UP) == other;
        } else {
            return false;
        }
    }
    ...
    public IBlockState getStateFromMeta(int meta) {
        return this.getDefaultState().withProperty(STATE, EnumDoorState.values()[meta & 3])
                .withProperty(ZPLANE, (meta & 4) > 0).withProperty(MASTER, (meta & 8) > 0);
    }
    ...
    public void onBlockHarvested(World worldIn, BlockPos pos, IBlockState state, EntityPlayer player) {
        ...
        if (!drop.isEmpty() && !worldIn.restoringBlockSnapshots) // do not drop items while restoring blockstates,
                                                                 // prevents item dupe
        {
            NonNullList<ItemStack> drops = NonNullList.<ItemStack>withSize(1, drop);
            float chance = net.minecraftforge.event.ForgeEventFactory.fireBlockHarvesting(drops, worldIn, pos, state, 0,
                    1.0f, false, player);
        ...
        if (tile != null && tile instanceof Door3x3TileEntity) {
            ...
            if (door.isUseableByPlayer(playerIn)) {
                ...
                if (playerIn.isSneaking()) {
                    boolean wrenchRight = (!playerIn.getHeldItemMainhand().isEmpty()) && playerIn.getHeldItemMainhand()
                            .getItem().getToolClasses(playerIn.getHeldItemMainhand()).contains("wrench");
                    boolean wrenchLeft = (!playerIn.getHeldItemMainhand().isEmpty()) && playerIn.getHeldItemMainhand()
                            .getItem().getToolClasses(playerIn.getHeldItemMainhand()).contains("wrench");
        ...
        if (tile != null && tile instanceof Door3x3TileEntity) {
            ...
            if (door.isRedstoneMode() && door.getRedstoneBehaviour() != 0) {
                ...
                if (door.getRedstoneBehaviour() == 1) {
                    if ((powered && masterstate.getValue(STATE) == EnumDoorState.CLOSED) ||
                            (!powered && masterstate.getValue(STATE) == EnumDoorState.OPENED)) {
