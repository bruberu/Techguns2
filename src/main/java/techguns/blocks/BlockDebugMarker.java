...
public class BlockDebugMarker extends GenericBlockMetaEnum<EnumDebugBlockType> {
    ...
    public int damageDropped(IBlockState state) {
        ...
        if (type == EnumDebugBlockType.INTERIORMARKER_NORTH || type == EnumDebugBlockType.INTERIORMARKER_EAST ||
                type == EnumDebugBlockType.INTERIORMARKER_SOUTH || type == EnumDebugBlockType.INTERIORMARKER_WEST) {
    ...
    public void getSubBlocks(CreativeTabs tab, NonNullList<ItemStack> items) {
        if (TGConfig.debug) {
            items.add(new ItemStack(this, 1,
                    this.getMetaFromState(getDefaultState().withProperty(TYPE, EnumDebugBlockType.AIRMARKER))));
            items.add(new ItemStack(this, 1,
                    this.getMetaFromState(getDefaultState().withProperty(TYPE, EnumDebugBlockType.ANTIAIRMARKER))));
            items.add(new ItemStack(this, 1, this
                    .getMetaFromState(getDefaultState().withProperty(TYPE, EnumDebugBlockType.INTERIORMARKER_NORTH))));
        }
    }
    ...
    public IBlockState getStateForPlacement(World world, BlockPos pos, EnumFacing facing, float hitX, float hitY,
        ...
        if (meta >= EnumDebugBlockType.INTERIORMARKER_NORTH.ordinal() &&
                meta <= EnumDebugBlockType.INTERIORMARKER_WEST.ordinal()) {
