...
public class ItemBlockOreDrill extends GenericItemBlockMetaMachineBlock {
    ...
    public boolean canPlaceBlockOnSide(World world, BlockPos posIn, EnumFacing side, EntityPlayer player,
        ...
        if (state.getValue(block.MACHINE_TYPE) == EnumOreDrillType.ROD) {
            ...
            if (clusterPos != null) {
                ...
                if (getHasConnectedDrillRod(world, clusterPos, new ArrayList<BlockPos>(), bs,
                        this.block.getDefaultState().withProperty(block.MACHINE_TYPE, EnumOreDrillType.ROD))) {

                    if (world.isRemote) {
                        // player.addChatMessage(new
                        // ChatComponentText(TextUtil.trans("techguns.msg.error.alreadyrod")));
                        player.sendStatusMessage(
                                new TextComponentString(TextUtil.trans("techguns.msg.error.alreadyrod")), true);
                        // TGPackets.network.sendTo(new
                        // PacketMultiBlockFormInvalidBlockMessage(pos,PacketMultiBlockFormInvalidBlockMessage.MSG_TYPE_ORE_DRILL_ROD_PLACEMENT_ERROR),
                ...
                if (checkForRodInNeighbourhood(world, pos)) {
                    ...
                    player.sendStatusMessage(new TextComponentString(TextUtil.trans("techguns.msg.error.rodplacement")),
                            true);
                    return false;
            ...
            if (checkForControllerInNeighbourhood(world, pos)) {
                if (world.isRemote) {
                    ...
                    player.sendStatusMessage(
                            new TextComponentString(TextUtil.transTG("msg.error.oredrill.controllernext")), true);

                    // TGPackets.network.sendTo(new
                    // PacketMultiBlockFormInvalidBlockMessage(pos,PacketMultiBlockFormInvalidBlockMessage.MSG_TYPE_ORE_DRILL_CONTROLLER_PLACEMENT_ERROR),
                    // (EntityPlayerMP) player);
                }
                return false;
            } else {
    ...
    public boolean getHasConnectedDrillRod(World world, BlockPos p, ArrayList<BlockPos> visited,
                                           IBlockState clusterblock, IBlockState drillRodBlock) {
        ...
        if (!visited.contains(p)) {
            if (target == clusterblock) {
                ...
                boolean b1 = getHasConnectedDrillRod(world, p.offset(EnumFacing.DOWN), visited, clusterblock,
                        drillRodBlock);
                boolean b2 = getHasConnectedDrillRod(world, p.offset(EnumFacing.UP), visited, clusterblock,
                        drillRodBlock);
                boolean b3 = getHasConnectedDrillRod(world, p.offset(EnumFacing.NORTH), visited, clusterblock,
                        drillRodBlock);
                boolean b4 = getHasConnectedDrillRod(world, p.offset(EnumFacing.SOUTH), visited, clusterblock,
                        drillRodBlock);
                boolean b5 = getHasConnectedDrillRod(world, p.offset(EnumFacing.WEST), visited, clusterblock,
                        drillRodBlock);
                boolean b6 = getHasConnectedDrillRod(world, p.offset(EnumFacing.EAST), visited, clusterblock,
                        drillRodBlock);
