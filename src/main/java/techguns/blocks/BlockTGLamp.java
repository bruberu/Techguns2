...
public class BlockTGLamp<T extends Enum<T> & IStringSerializable> extends GenericBlock implements ICamoChangeable {
    ...
    public static final AxisAlignedBB LAMP_DOWN_AABB = new AxisAlignedBB(lamp_side, 0, lamp_side, lamp_side_max, lamp_h,
            lamp_side_max);
    public static final AxisAlignedBB LAMP_UP_AABB = new AxisAlignedBB(lamp_side, 1.0D - lamp_h, lamp_side,
            lamp_side_max, 1.0D, lamp_side_max);
    public static final AxisAlignedBB LAMP_NORTH_AABB = new AxisAlignedBB(lamp_side, lamp_side, 0.0D, lamp_side_max,
            lamp_side_max, lamp_h);
    public static final AxisAlignedBB LAMP_SOUTH_AABB = new AxisAlignedBB(lamp_side, lamp_side, 1.0D - lamp_h,
            lamp_side_max, lamp_side_max, 1.0D);

    public static final AxisAlignedBB LAMP_WEST_AABB = new AxisAlignedBB(0.0D, lamp_side, lamp_side, lamp_h,
            lamp_side_max, lamp_side_max);
    public static final AxisAlignedBB LAMP_EAST_AABB = new AxisAlignedBB(1.0D - lamp_h, lamp_side, lamp_side, 1.0D,
            lamp_side_max, lamp_side_max);
    ...
    public static final AxisAlignedBB LANTERN_AABB = new AxisAlignedBB(lantern_side, lantern_h, lantern_side,
            lantern_side_max, 1.0D - lantern_h, lamp_side_max);
    ...
    public AxisAlignedBB getBoundingBox(IBlockState state, IBlockAccess source, BlockPos pos) {
        ...
        if (state.getValue(LAMP_TYPE) == clazz.getEnumConstants()[0] ||
                state.getValue(LAMP_TYPE) == clazz.getEnumConstants()[1]) {
    ...
    public IBlockState getStateFromMeta(int meta) {
        ...
        if (meta < 6) {
            s = s.withProperty(LAMP_TYPE, this.clazz.getEnumConstants()[0]).withProperty(FACING_ALL,
                    EnumFacing.byIndex(meta));
        } else if (meta < 12) {
            s = s.withProperty(LAMP_TYPE, this.clazz.getEnumConstants()[1]).withProperty(FACING_ALL,
                    EnumFacing.byIndex(meta - 6));
        } else if (meta == 12) {
    ...
    public IBlockState getActualState(IBlockState state, IBlockAccess worldIn, BlockPos pos) {
        ...
        if (type == 0 || type == 1) {
            return state.withProperty(CONNECT_UP, false).withProperty(CONNECT_DOWN, false)
                    .withProperty(CONNECT_E, false)
    ...
    public void neighborChanged(IBlockState state, World worldIn, BlockPos pos, Block blockIn, BlockPos fromPos) {
        if (!this.checkForDrop(worldIn, pos, state)) {
            ...
            if (enumfacing$axis.isHorizontal() &&
                    worldIn.getBlockState(blockpos).getBlockFaceShape(worldIn, blockpos, enumfacing) !=
    ...
    public void registerItemBlockModels() {
        for (int i = 0; i < clazz.getEnumConstants().length; i++) {
            ...
            ModelLoader.setCustomModelResourceLocation(itemblock, this.getMetaFromState(state),
                    new ModelResourceLocation(new ResourceLocation(Techguns.MODID,
                            "lamp_inventory_" + clazz.getEnumConstants()[i].getName()), "inventory"));
        }
    }
    ...
    public int switchCamo(ItemStack item, boolean back) {
        ...
        if (type == 0) {
            newmeta = this
                    .getMetaFromState(this.getDefaultState().withProperty(LAMP_TYPE, clazz.getEnumConstants()[1]));
        } else if (type == 1) {
            newmeta = this
                    .getMetaFromState(this.getDefaultState().withProperty(LAMP_TYPE, clazz.getEnumConstants()[0]));
        } else if (type == 2) {
            newmeta = this
                    .getMetaFromState(this.getDefaultState().withProperty(LAMP_TYPE, clazz.getEnumConstants()[3]));
        } else if (type == 3) {
            newmeta = this
                    .getMetaFromState(this.getDefaultState().withProperty(LAMP_TYPE, clazz.getEnumConstants()[2]));
        }
