...
public class MazeDungeonPath implements IDungeonPath {
     ...
     * (non-Javadoc)
     * 
     * @see techguns.world.dungeon.IDungeonPath#generateSegment(int, int, int, int,
     * techguns.world.dungeon.DungeonPath.PathSegment)
     */
    @Override
                ...
                if (dir == nextDir && !segment.isEntrance && !fork && rand.nextFloat() < chanceRamp) { // ramp
                    ...
                    if (isWithinBounds(elevPos) && !isOccupied(elevPos)) {
                        ...
                        if (success) {
                            ...
                            if (cont) {
                                generateSegment(elevNextPos.getX(), elevNextPos.getY(), elevNextPos.getZ(), nextDir,
                                        rampDummy);
                            }
                        }
                    }
                    ...
                    if (rand.nextFloat() < chanceRoom) {// Try to place a Room
                        ...
                        boolean b = tryGrowRoom(nextPos, segment, nextFacing,
                                minRoomWidth + rand.nextInt(maxRoomWidth - minRoomWidth),
                                minRoomWidth + rand.nextInt(maxRoomWidth - minRoomWidth), area);
                        if (b) {
                            success = true;
    ...
    private boolean tryGrowRoom(Vec3i pos, PathSegment prev, EnumFacing dir, int maxWidth, int maxLength,
                                int preferredArea) {
        int x = pos.getX();
        ...
        while (l < maxLength && !stopL) {
            ...
            while (w_l < maxWidth && w_l < maxw_l && !stop) {
                Vec3i pos_ = new Vec3i(x + vLeft.getX() * (w_l + 1) + vFront.getX() * l, y,
                        z + vLeft.getZ() * (w_l + 1) + vFront.getZ() * l);
            ...
            while (w_r < maxWidth && w_r < maxw_r && !stop) {
                Vec3i pos_ = new Vec3i(x + vRight.getX() * (w_r + 1) + vFront.getX() * l, y,
                        z + vRight.getZ() * (w_r + 1) + vFront.getZ() * l);
                if (!isWithinBounds(pos_) || isOccupied(pos_)) {
                    stop = true;
                    // w_r = w_r-1;
                    maxw_r = w_r;
            ...
            float aspect_ratio = (float) Math.max((w_l + w_r + 1), (l + 1)) /
                    (float) Math.min((w_l + w_r + 1), (l + 1));
        ...
        if (bestl < 2 || (bestw_l + bestw_r + 1) < 2) {
            ...
            Vec3i p2 = new Vec3i(x + vRight.getX() * bestw_r + vFront.getX() * bestl, y,
                    z + vRight.getZ() * bestw_r + vFront.getZ() * bestl);
    ...
    @Deprecated
    private PathSegment addRoomSegment(Vec3i pos, EnumFacing dir, int l, int w_l, int w_r, int maxL, int maxW_l,
                                       int maxW_r) {
     ...
     * (non-Javadoc)
     * 
     * @see techguns.world.dungeon.IDungeonPath#generateDungeon(net.minecraft.world.World, int, int, int,
     * techguns.world.dungeon.presets.IDungeonPreset)
     */
    @Override
                    ...
                    if (segment != null) {
                        ...
                        if (segment.isRamp) {
                            if (segment.elevation == 1) {
                                ...
                                preset.getSegment(SegmentType.RAMP, segment.y, 0, this.sY, above, below,
                                        world.rand.nextInt()).placeSegment(world, px, py, pz, (r + 1) % 4);
                            } else {
                                // Roof segment
                                ...
                                if (useRoof && !above && segment.y + 1 < this.sY) {
                                    ...
                                    preset.getSegment(SegmentType.END, -1, 0, this.sY, above, below, seed)
                                            .placeSegment(world, px, py, pz, (r + 1) % 4);
                                }
                            }
                        } else {
                            ...
                            for (TemplateSegment tempSeg : TemplateSegment.templateSegments.values()) {
                                ...
                                if (match) {
                                    ...
                                    if (tempSeg.type == SegmentType.END && j == (sY + this.startHeightLevel) % sY) {
                                        preset.getSegment(SegmentType.ENTRANCE, segment.y, 0, this.sY, above, below,
                                                world.rand.nextInt()).placeSegment(world, px, py, pz, r);
                                    } else {
                                        int seed = segment.roomID > 0 ? segment.roomID : world.rand.nextInt();
                                        preset.getSegment(tempSeg.type, segment.y, 0, this.sY, above, below, seed)
                                                .placeSegment(world, px, py, pz, r);
                                    }
                                    ok = true;
                                    // Roof segment
                                    if (useRoof && !above && segment.y >= startHeightLevel && segment.y + 1 < this.sY) {
                                        ...
                                        int seed = segment.roomID > 0 ? segment.roomID : world.rand.nextInt();
                                        preset.getSegment(tempSeg.type, -1, 0, this.sY, above, below, seed)
                                                .placeSegment(world, px, py, pz, r);
                                    }
                                    break;
                                }
                        ...
                        if (useFoundations && above && !below) {
                            ...
                            int py = posY + (j * preset.getSizeY());
                            int pz = posZ + (k * preset.getSizeXZ());
                            preset.getSegment(SegmentType.FOUNDATION, j, 0, this.sY, above, below, world.rand.nextInt())
                                    .placeSegment(world, px, py, pz, 0);
                        } else if (usePillars && above) {
                            int px = posX + (i * preset.getSizeXZ());
                            int py = posY + (j * preset.getSizeY());
                            int pz = posZ + (k * preset.getSizeXZ());
                            preset.getSegment(SegmentType.PILLARS, j, 0, this.sY, above, below, world.rand.nextInt())
                                    .placeSegment(world, px, py, pz, 0);
                            ...
                            // int py = posY+(j*preset.getSizeY());
                            // int pz = posZ+(k*preset.getSizeXZ());
                            // preset.getSegment(SegmentType.PILLARS, -1, 0, this.sY, above, below,
                            // world.rand.nextInt()).placeSegment(world, px, py, pz, 0);
                        }
                    }
                }
            }
     ...
     * (non-Javadoc)
     * 
     * @see techguns.world.dungeon.IDungeonPath#generateNPCSpawners(net.minecraft.world.World, int, int, int,
     * techguns.world.dungeon.presets.IDungeonPreset)
     */
    @Override
