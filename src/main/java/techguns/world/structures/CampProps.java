...
public class CampProps extends WorldgenStructure {
    ...
    IBlockState lampstate = TGBlocks.LAMP_0.getDefaultState().withProperty(TGBlocks.LAMP_0.LAMP_TYPE,
            EnumLampType.YELLOW_LANTERN);
    ...
    private void setRoofBlocks(World world, int posX, int posY, int posZ,
        ...
        MutableBlockPos p = new MutableBlockPos();
        BlockUtils.fillBlocks(world, p.setPos(posX, posY + 1, posZ), 1, sizeY - 2, 1,
                Blocks.OAK_FENCE.getDefaultState());
        BlockUtils.fillBlocks(world, p.setPos(posX + sizeX - 1, posY + 1, posZ), 1, sizeY - 2, 1,
                Blocks.OAK_FENCE.getDefaultState());
        BlockUtils.fillBlocks(world, p.setPos(posX, posY + 1, posZ + sizeZ - 1), 1, sizeY - 2, 1,
                Blocks.OAK_FENCE.getDefaultState());
        BlockUtils.fillBlocks(world, p.setPos(posX + sizeX - 1, posY + 1, posZ + sizeZ - 1), 1, sizeY - 2, 1,
                Blocks.OAK_FENCE.getDefaultState());
        BlockUtils.fillBlocks(world, p.setPos(posX, posY + sizeY - 1, posZ), sizeX, 1, sizeZ,
                TGBlocks.CAMONET_TOP.getDefaultState().withProperty(TGBlocks.CAMONET_TOP.TYPE, camo));
    }
        ...
        if (((sizeX + sizeZ) / 2.0f) / 6.0f > rnd.nextFloat()) { // place lantern
            for (int y = sizeY; y > 1; y--) {
                for (int t = 0; t < 3; t++) {
                    ...
                    if (world.isAirBlock(p.setPos(posX + x, posY + y, posZ + z)) &&
                            !isFreeSpace(world, posX + x, posY + y - 1, posZ + z)) {
    ...
    public boolean isFreeSpace(World world, int x, int y, int z) {
        MutableBlockPos p = new MutableBlockPos();
        return world.isAirBlock(p.setPos(x, y, z)) || world.getBlockState(p).getBlock() == Blocks.SNOW_LAYER ||
                world.getBlockState(p).getBlock() == TGBlocks.LAMP_0;
    }
}
