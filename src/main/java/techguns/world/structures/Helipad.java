    ...
    public void setBlocks(World world, int posX, int posY, int posZ, int sizeX,
        ...
        if (sizeX == 9 && sizeZ == 9) {
            for (int z = 1; z < sizeZ - 1; z++) {
                ...
                world.setBlockState(p.setPos(posX + sizeX - 1, posY, posZ + z),
                        MBlockRegister.HELIPAD_WIREFRAME.getState(), 2);
            }
            posX++;
            sizeX = 7;
            for (int x = 0; x < sizeX; x++) {
                world.setBlockState(p.setPos(posX + x, posY, posZ), MBlockRegister.HELIPAD_WIREFRAME.getState(), 2);
                world.setBlockState(p.setPos(posX + x, posY, posZ + sizeZ - 1),
                        MBlockRegister.HELIPAD_WIREFRAME.getState(), 2);
            }
            posZ++;
            sizeZ = 7;
        ...
        for (int x = 0; x < sizeX; x++) {
            for (int z = 0; z < sizeZ; z++) {
                if (x == 0 || z == 0 || x == sizeX - 1 || z == sizeZ - 1) {
                    world.setBlockState(p.setPos(posX + x, posY + 1, posZ + z),
                            MBlockRegister.HELIPAD_HAZARDBLOCK.getState(), 2);
                } else {
                    world.setBlockState(p.setPos(posX + x, posY + 1, posZ + z),
                            MBlockRegister.HELIPAD_CONCRETE.getState(), 2);
                }
            }
        }
