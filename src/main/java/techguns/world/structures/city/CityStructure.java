...
public class CityStructure extends WorldgenStructure {
    ...
    public void setBlocks(World world, int posX, int posY, int posZ, int sizeX, int sizeY, int sizeZ, int direction,
        ...
        // NOT NEEDED
        // spawnStructureWorldgen(world, posX>>4, posY>>4, sizeX, sizeY, sizeZ, rnd, world.getBiome(new
    ...
    public void spawnStructureWorldgen(World world, int chunkX, int chunkZ, int sizeX, int sizeY, int sizeZ, Random rnd,
        ...
        if (y > 0) {
            ...
            BlockUtils.apply2DHeightmapFilter(world, x - 4, z - 4, sizeX + 4, sizeZ + 4,
                    BlockUtils.FILTER_GAUSSIAN_5x5);

            GenericCity.testCity.createCity(world, x, y, z, sizeX, sizeY, sizeZ, BiomeColorType.WOODLAND, rnd);

            // building.setBlocks(world, x,BlockUtils.getHeightValueNoTrees(world, x, z)-1, z, sizeX, sizeY, sizeZ,
            // random.nextInt(4), BiomeColorType.WOODLAND);
