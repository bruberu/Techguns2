...
public class GenericCitySegment {
    ...
    public GenericCitySegment(GenericCity cityType, int x, int y, int z, int sizeX, int sizeZ, int roadWidth,
                              int direction, BiomeColorType colorType, Random rand, GenericCitySegment rootSegment) {
        super();
        ...
        if (rootSegment == null) {
            ...
            this.rootRadiusSquared = (float) Vec2.substract(
                    ...
                    new Vec2((float) this.x + ((float) this.sizeX / 2.0f),
                            (float) this.z + ((float) this.sizeZ / 2.0f)))
                    .lenSquared();
        } else {
            ...
            for (int i = 0; i < subSegments.length; i++) {
                ...
                if (i < subSegments.length - 1) {
                    if (subAlignment) {
                        ...
                        int y2 = BlockUtils.getGroundY(world, this.x + sizeX,
                                subSegments[i].z + subSegments[i].sizeZ + roadWidth);
                        int heightDif = y2 - y1;
                        cityType.roadType.setRoadBlocks(world, this.x, this.y, subSegments[i].z + subSegments[i].sizeZ,
                                this.roadWidth, sizeX, heightDif, subAlignment, colorType);
                    } else {
                        int y1 = BlockUtils.getGroundY(world, subSegments[i].x + subSegments[i].sizeX, this.z);
                        int y2 = BlockUtils.getGroundY(world, subSegments[i].x + subSegments[i].sizeX + roadWidth,
                                this.z + sizeZ);
                        int heightDif = y2 - y1;
                        cityType.roadType.setRoadBlocks(world, subSegments[i].x + subSegments[i].sizeX, this.y, this.z,
                                this.roadWidth, sizeZ, heightDif, subAlignment, colorType);
                    }
                }
            }
        } else {
    ...
    public boolean generateSubSegments() {
        ...
        double s = Math.sqrt((double) (size - cityType.minSubSegmentSize) /
                (double) (cityType.maxSubSegmentSize - cityType.minSubSegmentSize));
        // System.out.println("size="+size+", s="+s+" - centerfactor:"+getCenterFactor());
        if (size2 < cityType.minSubSegmentSize ||
                ((size < cityType.maxSubSegmentSize && rand.nextFloat() > (s * 0.75f + getCenterFactor() * 0.25f)))) {
            ...
            if (this != rootSegment && rand.nextFloat() >= 0.5) {
                count = Math.max(2,
                        Math.round(1.0f + (float) rand.nextFloat() * 0.5f *
                                (((float) size - (((float) (cityType.minSubSegmentSize + 1)) / 2.0f)) /
                                        (float) (roadWidth + 2))));
            }
            ...
            for (int i = 0; i < count; i++) {
                ...
                if (!subAlignment) {
                    subSegments[i] = new GenericCitySegment(cityType, x1 + sizeTotal, y, z1, splitSize, sizeZ,
                            nextRoadWidth, dir, colorType, rand, rootSegment);
                } else {
                    subSegments[i] = new GenericCitySegment(cityType, x1, y, z1 + sizeTotal, sizeX, splitSize,
                            nextRoadWidth, dir, colorType, rand, rootSegment);
                }
