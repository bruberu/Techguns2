...
public class CityBuildingType {
    ...
    static {
        ...
        BuildingStyle factoryBuilding = new BuildingStyle(MBlockRegister.FACTORY_PLATE,
                new MBlock(Blocks.STAINED_GLASS_PANE, 8), MBlockRegister.FACTORY_WIREFRAME);
        ...
        // MBlock[] limeStair = new MBlock[]{new
        // MBlock(ChiselBlocks.limestoneStairs[0],8,true,BlockType.STAIRS2_CHISEL),new
        // MBlock(ChiselBlocks.limestone,1)};

        BuildingStyle limeBuilding = new BuildingStyle(limeWalls, glassPanes, limeFloors, limeBorders,
                limeDoorDecoration);
    ...
    public void setBlocks(World world, int posX, int posY, int posZ, int sizeX,
        ...
        for (int i = 0; i < segY; i++) {
            ...
            placeWall(world, posX + sx - 1, y, posZ + sz - 1, segX, -1, 0, 2, colorType, (i == 0 && direction == 2),
                    blocklist);
        ...
        for (int i = 0; i < segY - 1; i++) {
            ...
            if (!isSmall) { // Place Stairs
                if ((i % 2) == 1) {
                    Staircase.placeStaircase(world, px + dx, y + 1, pz + dz, wall_y, (stairsdir + 2) % 4, true,
                            colorType, stairs, stairs_2);
                } else {
                    Staircase.placeStaircase(world, px, y + 1, pz, wall_y, (stairsdir + 2) % 4, false, colorType,
                            stairs, stairs_2);
                }
            } else { // Place Ladder
    ...
    public void placeWall(World world, int posX, int posY, int posZ, int segments, int dx, int dz, int direction,
                          BiomeColorType colorType, boolean placeDoor, ArrayList<MBlock> blocklist) {
        int mid = (segments / 2);
            ...
            if (placeDoor && i == mid) { // Place door segment
                for (int j = 0; j < wall_x; j++) {
                    for (int l = 1; l <= wall_y; l++) {
                        ...
                        block.setBlock(world, p.setPos(posX + (offset * dx), posY + l - 1, posZ + (offset * dz)),
                                direction);
                    }
                }
                int offset = (i * wall_x) + doorOffset;
                for (int k = 0; k < doorWidth; k++) {
                    // ItemDoor.placeDoorBlock(world, posX+((offset+k)*dx), posY+1, posZ+((offset+k)*dz),
                    // BlockUtils.getBlockDirMeta(BlockType.DOOR, (direction+3)%4), Blocks.wooden_door);

                    EnumFacing doorFacing = EnumFacing.HORIZONTALS[(direction + 3) % 4];
                    ItemDoor.placeDoor(world,
                            p.setPos(posX + ((offset + k) * dx), posY + 1, posZ + ((offset + k) * dz)), doorFacing,
