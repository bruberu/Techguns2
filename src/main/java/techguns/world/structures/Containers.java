...
public class Containers extends WorldgenStructure {
    ...
    public Containers(int minX, int minY, int minZ, int maxX, int maxY, int maxZ, int containerWidth,
                      int containerHeight, int containerMinLength, MBlock containerBlock) {
    ...
    public void setBlocks(World world, int posX, int posY, int posZ, int sizeX,
        ...
        if (sizeX > sizeZ) {
            ...
            for (int i = 0; i < count; i++) {
                BlockUtils.fillBlocks(world, pos.setPos(posX, posY + 1, posZ + offset), sizeX, containerHeight,
                        containerWidth, containerBlock);
            ...
            for (int i = 0; i < count; i++) {
                BlockUtils.fillBlocks(world, pos.setPos(posX + offset, posY + 1, posZ), containerWidth, containerHeight,
                        sizeZ, containerBlock);
                offset += (containerWidth + 1);
            }
        }
    }
