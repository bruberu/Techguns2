...
public class Bunker extends WorldgenStructure {
    ...
    private static final ResourceLocation CHEST_LOOT = new ResourceLocation(Techguns.MODID,
            "chests/militarybase_bunker");
    ...
    MBlock[] crateBlocks = MBlockRegister.crates; // new MBlock[] { new MBlock(Blocks.PLANKS,0), new
                                                  // MBlock(Blocks.PLANKS,1) } ;//TGChiselBlocks.factory_box,
                                                  // TGChiselBlocks.woodenCrate};
    ...
    public void setBlocks(World world, int posX, int posY, int posZ, int sizeX,
        ...
        // ItemDoor.placeDoorBlock(world, posX+xoffset, posY+0, posZ+zoffset, BlockUtils.getBlockDirMeta(BlockType.DOOR,
        // direction), TGBlocks.door01);
        ItemTGDoor2x1.placeDoor(world, p.setPos(posX + xoffset, posY + 0, posZ + zoffset), facing, TGBlocks.BUNKER_DOOR,
                true);

        // Doorway
        world.setBlockState(
                p.setPos(posX + xoffset + ((dx == 0) ? 1 : 0), posY + 0, posZ + zoffset + ((dz == 0) ? 1 : 0)),
                wallBlock.getState(), 2);
        world.setBlockState(
                p.setPos(posX + xoffset + ((dx == 0) ? 1 : 0), posY + 1, posZ + zoffset + ((dz == 0) ? 1 : 0)),
                wallBlock.getState(), 2);
        world.setBlockState(
                p.setPos(posX + xoffset + ((dx == 0) ? -1 : 0), posY + 0, posZ + zoffset + ((dz == 0) ? -1 : 0)),
                wallBlock.getState(), 2);
        ...
        world.setBlockState(
                p.setPos(posX + xoffset + ((dx == 0) ? 1 : dx), posY - 1, posZ + zoffset + ((dz == 0) ? 1 : dz)),
                wallBlock.getState(), 2);
        world.setBlockState(
                p.setPos(posX + xoffset + ((dx == 0) ? -1 : dx), posY - 1, posZ + zoffset + ((dz == 0) ? -1 : dz)),
                wallBlock.getState(), 2);
        world.setBlockState(p.setPos(posX + xoffset + ((dx == 0) ? 1 : dx * 2), posY - 1,
                posZ + zoffset + ((dz == 0) ? 1 : dz * 2)), wallBlock.getState(), 2);
        world.setBlockState(p.setPos(posX + xoffset + ((dx == 0) ? -1 : dx * 2), posY - 1,
                posZ + zoffset + ((dz == 0) ? -1 : dz * 2)), wallBlock.getState(), 2);
        world.setBlockState(
                p.setPos(posX + xoffset + ((dx == 0) ? 1 : dx), posY + 0, posZ + zoffset + ((dz == 0) ? 1 : dz)),
                wallBlock.getState(), 2);
        world.setBlockState(
                p.setPos(posX + xoffset + ((dx == 0) ? -1 : dx), posY + 0, posZ + zoffset + ((dz == 0) ? -1 : dz)),
                wallBlock.getState(), 2);
        world.setBlockState(p.setPos(posX + xoffset + ((dx == 0) ? 1 : dx * 2), posY + 0,
                posZ + zoffset + ((dz == 0) ? 1 : dz * 2)), wallBlock.getState(), 2);
        world.setBlockState(p.setPos(posX + xoffset + ((dx == 0) ? -1 : dx * 2), posY + 0,
                posZ + zoffset + ((dz == 0) ? -1 : dz * 2)), wallBlock.getState(), 2);

        world.setBlockState(
                p.setPos(posX + xoffset + ((dx == 0) ? 1 : dx), posY + 2, posZ + zoffset + ((dz == 0) ? 1 : dz)),
                BlockRotator.getWithFacing(lamps.getState(), facing.getOpposite()), 2);
        world.setBlockState(
                p.setPos(posX + xoffset + ((dx == 0) ? -1 : dx), posY + 2, posZ + zoffset + ((dz == 0) ? -1 : dz)),
                BlockRotator.getWithFacing(lamps.getState(), facing.getOpposite()), 2);
        ...
        // Stairs
        world.setBlockState(p.setPos(posX + xoffset + dx * 2, posY + 0, posZ + zoffset + dz * 2),
                BlockRotator.getWithFacing(stairsblock.getState(), facing), 2);
        ...
        if (type == 2 || type == 3) {
            // Lamp
            world.setBlockState(p.setPos(posX + xoffset - dx, posY + 1, posZ + zoffset - dz),
                    BlockRotator.getWithFacing(lamps.getState(), EnumFacing.UP), 2);
            // if (type == 2) {
            ...
            // Lamp
            world.setBlockState(p.setPos(posX + (sizeX / 2), posY + 1, posZ + (sizeZ / 2)),
                    BlockRotator.getWithFacing(lamps.getState(), EnumFacing.UP), 2);
        ...
        if (type == 1 || type == 3) {
             ...
             */
            world.setBlockState(p.setPos(posX + xoffset, posY + 0, posZ + zoffset),
                    BlockRotator.getWithFacing(ladder.getState(), ladderFacing), 2);
            world.setBlockState(p.setPos(posX + xoffset, posY + 1, posZ + zoffset),
                    BlockRotator.getWithFacing(ladder.getState(), ladderFacing), 2);
            world.setBlockState(p.setPos(posX + xoffset, posY + 2, posZ + zoffset),
                    BlockRotator.getWithFacing(ladder.getState(), ladderFacing), 2);
        }
    }
                ...
                if (chest == null) {
                    ...
                    for (int y = 0; y < sizeY; y++) {
                        if (!world.isAirBlock(p.setPos(posX + x, posY + y - 1, posZ + z)) && r.nextFloat() > 0.5) {
                            ...
                            world.setBlockState(p.setPos(posX + x, posY + y, posZ + z), crateBlocks[index].getState(),
                                    2);
                        }
                    }
                } else {
                    ...
                    world.setBlockState(p.setPos(posX + x, posY, posZ + z),
                            BlockRotator.getRotatedHorizontal(chest.getDefaultState(), meta));
