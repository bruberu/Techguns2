...
public class Barracks extends WorldgenStructure {
    ...
    public void setBlocks(World world, int posX, int posY, int posZ, int sizeX,
        ...
        ItemDoor.placeDoor(world, p.setPos(posX + xoffset, posY + 3, posZ + zoffset), facing.getOpposite(),
                Blocks.OAK_DOOR, false);
        // ItemDoor.placeDoorBlock(world, posX+xoffset, posY+3, posZ+zoffset, BlockUtils.getBlockDirMeta(BlockType.DOOR,
        // direction), Blocks.wooden_door);

        // Lamp
        world.setBlockState(p.setPos(posX + xoffset + dx, posY + 5, posZ + zoffset + dz),
                BlockRotator.getWithFacing(b_torch.getState(), facing.getOpposite()), 2);

        // Ramp
        world.setBlockState(p.setPos(posX + xoffset + dx, posY + 2, posZ + zoffset + dz),
                BlockRotator.getWithFacing(b_stairs.getState(), facing.getOpposite()), 2);
        world.setBlockState(p.setPos(posX + xoffset + dx * 2, posY + 1, posZ + zoffset + dz * 2),
                BlockRotator.getWithFacing(b_stairs.getState(), facing.getOpposite()), 2);
                ...
                for (int y = 3; y < 6; y++) {
                    if (x == 1 || z == 1 || x == sizeX - 2 || z == sizeZ - 2) {
                        if (y == 5 && (x % 4 == 2 || z % 4 == 2)) { // torch
                            world.setBlockState(p.setPos(posX + x, posY + y, posZ + z),
                                    BlockRotator.getWithFacing(b_torch.getState(), EnumFacing.UP), 2);
                        }
                        if (y == 3 && (x % 4 == 0 || z % 4 == 0) && x != xoffset && z != zoffset) {
                            ...
                            world.setBlockState(p.setPos(posX + x, posY + y, posZ + z),
                                    BlockRotator.getWithFacing(Blocks.CHEST.getDefaultState(), chestrot), 2);
                        ...
                        if (x == 2 || z == 2 || x == sizeX - 3 || z == sizeZ - 3) {
                            ...
                            if (sizeX / 4 > 1 || sizeZ / 4 > 1) {
                                if (y == 3 && (x % 4 == 2 && z % 4 == 2) && x != xoffset && z != zoffset) {
                                    if (x == 2) {
                                        world.setBlockState(p.setPos(posX + x, posY + y, posZ + z),
                                                Blocks.BED.getStateFromMeta(1), 3);
                                        world.setBlockState(p.setPos(posX + x - 1, posY + y, posZ + z),
                                                Blocks.BED.getStateFromMeta(1 + 8), 3);
                                    } else if (x == sizeX - 3) {
                                        world.setBlockState(p.setPos(posX + x, posY + y, posZ + z),
                                                Blocks.BED.getStateFromMeta(3), 3);
                                        world.setBlockState(p.setPos(posX + x + 1, posY + y, posZ + z),
                                                Blocks.BED.getStateFromMeta(3 + 8), 3);
                                    } else if (z == 2) {
                                        world.setBlockState(p.setPos(posX + x, posY + y, posZ + z),
                                                Blocks.BED.getStateFromMeta(2), 3);
                                        world.setBlockState(p.setPos(posX + x, posY + y, posZ + z - 1),
                                                Blocks.BED.getStateFromMeta(2 + 8), 3);
                                    } else if (z == sizeZ - 3) {
                                        world.setBlockState(p.setPos(posX + x, posY + y, posZ + z),
                                                Blocks.BED.getStateFromMeta(0), 3);
                                        world.setBlockState(p.setPos(posX + x, posY + y, posZ + z + 1),
                                                Blocks.BED.getStateFromMeta(0 + 8), 3);
                                    }
                                }
                            }
                            // if (y == 3 && (x % 4 == 0 || z % 4 == 0) && x!= xoffset && z!= zoffset) {
