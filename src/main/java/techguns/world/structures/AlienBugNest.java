...
public class AlienBugNest {
    ...
    static {
        wallBlock = new MultiMBlock(new Block[] { TGBlocks.SAND_HARD, TGBlocks.SLIMY_BLOCK, Blocks.SANDSTONE },
                new int[] { EnumTGSandHardTypes.BUGNEST_SAND.ordinal(), EnumTGSlimyType.BUGNEST_EGGS.ordinal(), 0 },
                new int[] { 16, 1, 2 });
        wallBlock2 = new MultiMBlock(new Block[] { TGBlocks.SAND_HARD, Blocks.SANDSTONE },
                new int[] { EnumTGSandHardTypes.BUGNEST_SAND.ordinal(), 0 }, new int[] { 12, 1 });
    ...
    public AlienBugNest(int posX, int posY, int posZ, int sizeX, int sizeY, int sizeZ, Random rand) {
        ...
        for (int i = 0; i < segments; i++) {
            ...
            if (largeNest && rand.nextBoolean()) {
                ...
                xz = MathUtil.polarOffsetXZ(xz.x, xz.y, dist,
                        (angle * i) + (Math.PI * 0.5 * (rand.nextDouble() - 0.5)));
    ...
    private void placeSegment(World world, BugNestSegment segment) {
        ...
        if (segment.type != 2) { // Not entrance
            ...
            BlockUtils.fillSphere2(world, segment.posX, segment.posY, segment.posZ, segment.radius, 1.0f,
                    MBlockRegister.AIR, wallBlock);
            segment.status = 1;
    ...
    private void placeInterior(World world, BugNestSegment segment) {
        ...
        if (segment.type != 2) { // Not entrance
            ...
            BlockUtils.fillSphere(world, segment.posX, segment.posY, segment.posZ, segment.radius - 1,
                    MBlockRegister.AIR);// , innerBlock );
            segment.status = 3;
            ...
            for (BugNestSegment seg : segment.neighbors) {
                if (segment.type == 0 && seg.type == 2) { // main segment to Entrance
                    ...
                    for (int i = 0; i < k; i++) {
                        ...
                        // Get proper distance
                        Vec2 xz = MathUtil.polarOffsetXZ(0, 0, 1.0 + (world.rand.nextDouble() * 0.66),
                                angle + ((Math.PI * 2.0) / k) * i);
                        int xo = (int) xz.x;
                        int zo = (int) xz.y;
                        Vec3d pos1 = new Vec3d(segment.posX, segment.posY, segment.posZ);
                        Vec3d pos2 = new Vec3d(seg.posX, seg.posY, seg.posZ);
                        ...
                        // BlockUtils.fillCylinder(world, x1, y1, z1, x2, y2, z2, 1.0f, new MBlock(Blocks.dirt, 0), new
                        // MBlock(Blocks.dirt, 0));
            ...
            for (int y = y1 - r; y < y2 + r; y++) {
                for (int z = z1 - r; z < z2 + r; z++) {
                    ...
                    if (distance < radius && TGBlocks.SLIMY_LADDER.canPlaceBlockAt(world, p)) {
                        world.setBlockState(p, TGBlocks.SLIMY_LADDER.getStateForPlacement(world, p, EnumFacing.NORTH, 0,
                                0, 0, 0, null), 2);
                        // slimeLadder.setBlock(world, p, 0, null);
                        // world.setBlockMetadataWithNotify(x,y,z,((BlockBugSlime)TGBlocks.bugnestslimy).getDirectionMeta(world,
                        // x, y, z), 2);
                    }
                }
            }
    ...
    class BugNestSegment {
        ...
        ArrayList<BugNestSegment> neighbors = new ArrayList<BugNestSegment>(); // one-directional list of connected
                                                                               // segments;
    ...
    static class MBlockSlime extends MBlock {
        ...
        private void placeSlimeAt(World world, MutableBlockPos pos) {
            if (world.isAirBlock(pos) && TGBlocks.SLIMY_LADDER.canPlaceBlockAt(world, pos)) { // TGBlocks.bugnestslimy.canPlaceBlockAt(world,
                                                                                              // x, y, z)) {
                // slimeLadder.setBlock(world, pos, 0, null);
                world.setBlockState(pos,
                        TGBlocks.SLIMY_LADDER.getStateForPlacement(world, pos, EnumFacing.NORTH, 0, 0, 0, 0, null), 2);
                // world.setBlockMetadataWithNotify(x,y,z,((BlockBugSlime)TGBlocks.bugnestslimy).getDirectionMeta(world,
                // x, y, z), 2);
            }
        }
