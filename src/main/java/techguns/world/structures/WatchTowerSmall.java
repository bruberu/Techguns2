...
public class WatchTowerSmall extends WorldgenStructure {
    ...
    public void setBlocks(World world, int posX, int posY, int posZ, int sizeX,
        ...
        for (int y = 1; y < sizeY - 1; y++) {
            ...
            world.setBlockState(p.setPos(posX + towerSize - 1, posY + y, posZ + towerSize - 1),
        ...
        // top
        BlockUtils.fillBlocksHollow(world, p.setPos(posX, posY + sizeY - 4, posZ), towerSize, 2, towerSize,
                Blocks.OAK_FENCE.getDefaultState());
        BlockUtils.fillBlocks(world, p.setPos(posX, posY + sizeY - 1, posZ), towerSize, 1, towerSize,
                TGBlocks.CAMONET_TOP.getDefaultState());
        ...
        for (int y = 1; y < sizeY - 3; y++) {
            world.setBlockState(p.setPos(posX + xoffset, posY + y, posZ + zoffset),
                    Blocks.LADDER.getStateFromMeta(meta), 3);
        }
        world.setBlockState(p.setPos(posX + xoffset, posY + sizeY - 3, posZ + zoffset),
                Blocks.OAK_FENCE_GATE.getStateFromMeta((direction + 1) % 4), 3);
