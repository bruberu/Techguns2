...
public class MilitaryCamp {
    ...
    // static MBlock roadBlockSecondary = new MBlock(Blocks.CONCRETE.getDefaultState().withProperty(BlockColored.COLOR,
    // EnumDyeColor.GRAY));//TGChiselBlocks.concrete; //TODO
    ...
    public void init(int posX, int posY, int posZ, int sizeX, int sizeZ) {
        ...
        campRadiusSquared = Vec2.substract(
                ...
                new Vec2((float) this.posX + ((float) this.sizeX / 2.0f),
                        (float) this.posZ + ((float) this.sizeZ / 2.0f)))
                .lenSquared();
        ...
        borderChanceX = ((float) (sizeX - minBorderStructureSize) /
                (float) (maxBorderStructureSize - minBorderStructureSize));
        borderChanceZ = ((float) (sizeZ - minBorderStructureSize) /
                (float) (maxBorderStructureSize - minBorderStructureSize));
        cornerChance = ((float) (((sizeX + sizeZ) / 2) - minCornerStructureSize) /
                (float) (maxCornerStructureSize - minCornerStructureSize));
        ...
        this.rootSegment = new CampSegment(x, posY, z, sizeX - 2 * (outerFenceWidth + outerFenceSpacing),
                sizeZ - 2 * (outerFenceWidth + outerFenceSpacing), roadWidth, 0);
    ...
    public void setBlocks(World world, Random rnd) {
        ...
        if (maxHeightDiff != -1) {
            ...
            BlockUtils.apply2DHeightmapFilter(world, posX - 1, posZ - 1, sizeX + 2, sizeZ + 2,
                    BlockUtils.FILTER_GAUSSIAN_5x5);
        }

        if (sizeX < ((outerFenceWidth + outerFenceSpacing) * 2) + 1 ||
                sizeZ < ((outerFenceWidth + outerFenceSpacing) * 2) + 1) {
            return;
        }
    ...
    private void initCampFlagEntity(World world) {
        ...
        world.setBlockState(p, TGBlocks.MONSTER_SPAWNER.getDefaultState().withProperty(TGBlocks.MONSTER_SPAWNER.TYPE,
                EnumMonsterSpawnerType.SOLDIER_SPAWN), 3);
        ...
        for (int i = 0; i < this.spawnPositions.size(); i += 3) {
            ...
            world.setBlockState(p.setPos(x, y, z), TGBlocks.MONSTER_SPAWNER.getDefaultState()
                    .withProperty(TGBlocks.MONSTER_SPAWNER.TYPE, EnumMonsterSpawnerType.SOLDIER_SPAWN), 3);
    ...
    public void setFenceGateBlocks(World world, int x, int y, int z, int roadWidth, int length, int height,
                                   boolean alignment) {
        int a, b;
        MutableBlockPos p = new MutableBlockPos();
        ...
        for (a = 0; a < length; a++) {
            for (b = 0; b < roadWidth; b++) {
                for (int c = 0; c < height + 10; c++) { // +10 = cheap workaround
                    if (alignment) { // Road: in x dir
                        if (maxHeightDiff != 0) y = getGroundY(world, x + a, z + b);
                        if (world.getBlockState(p.setPos(x + a, y + c + 1, z + b)) ==
                                MBlockRegister.MILBASE_FENCE.getState())
                            world.setBlockToAir(p.setPos(x + a, y + c + 1, z + b));
                    } else {
                        if (maxHeightDiff != 0) y = getGroundY(world, x + b, z + a);
                        if (world.getBlockState(p.setPos(x + b, y + c + 1, z + a)) ==
                                MBlockRegister.MILBASE_FENCE.getState())
    ...
    class CampSegment {
                ...
                for (int i = 0; i < subSegments.length; i++) {
                    ...
                    if (i < subSegments.length - 1) {
                        if (this == rootSegment) { // Main-road
                            ...
                            if (subAlignment) {
                                setRoadBlocks(world, this.x - extralength, this.y,
                                        subSegments[i].z + subSegments[i].sizeZ, this.roadWidth,
                                        sizeX + (2 * extralength), subAlignment);
                                // Create Fence gate
                                setFenceGateBlocks(world, x - (outerFenceSpacing + outerFenceWidth), y,
                                        subSegments[i].z + subSegments[i].sizeZ, this.roadWidth, outerFenceWidth,
                                        outerFenceHeight, subAlignment);
                                setFenceGateBlocks(world, x + sizeX + outerFenceSpacing, y,
                                        subSegments[i].z + subSegments[i].sizeZ, this.roadWidth, outerFenceWidth,
                                        outerFenceHeight, subAlignment);

                            } else {
                                setRoadBlocks(world, subSegments[i].x + subSegments[i].sizeX, this.y,
                                        this.z - extralength, this.roadWidth, sizeZ + (2 * extralength), subAlignment);
                                // Create Fence gate
                                setFenceGateBlocks(world, subSegments[i].x + subSegments[i].sizeX, y,
                                        z - (outerFenceSpacing + outerFenceWidth), this.roadWidth, outerFenceWidth,
                                        outerFenceHeight, subAlignment);
                                setFenceGateBlocks(world, subSegments[i].x + subSegments[i].sizeX, y,
                                        z + sizeZ + outerFenceSpacing, this.roadWidth, outerFenceWidth,
                                        outerFenceHeight, subAlignment);

                            }
                        } else {
                            if (subAlignment) setRoadBlocks(world, this.x - 1, this.y,
                                    subSegments[i].z + subSegments[i].sizeZ, this.roadWidth, sizeX + 2, subAlignment);
                            else setRoadBlocks(world, subSegments[i].x + subSegments[i].sizeX, this.y, this.z - 1,
                                    this.roadWidth, sizeZ + 2, subAlignment);
                        }
                    }
                }
            } else {
        ...
        public boolean generateSubSegments() {
            ...
            double s = Math
                    .sqrt((double) (size - minSubSegmentSize) / (double) (maxSubSegmentSize - minSubSegmentSize));
            // System.out.println("size="+size+", s="+s+" - centerfactor:"+getCenterFactor());
            if (size < minSubSegmentSize ||
                    (size < maxSubSegmentSize && rand.nextFloat() > (s * 0.75f + getCenterFactor() * 0.25f))) {
                ...
                if (this != rootSegment && rand.nextFloat() >= 0.5) {
                    count = Math.max(2, Math.round(1.0f + (float) rand.nextFloat() * 0.5f *
                            (((float) size - (((float) (minSubSegmentSize + 1)) / 2.0f)) / (float) (roadWidth + 2))));
                }
        ...
        public boolean shouldPlaceCornerStructure() {
            int border = MilitaryCamp.this.outerFenceWidth + MilitaryCamp.this.outerFenceSpacing;
            boolean isCorner = ((this.x <= MilitaryCamp.this.posX + border &&
                    this.z <= MilitaryCamp.this.posZ + border) ||
                    (this.x <= MilitaryCamp.this.posX + border &&
                            this.z + this.sizeZ >= MilitaryCamp.this.posZ + MilitaryCamp.this.sizeZ - border) ||
                    (this.x + this.sizeX >= MilitaryCamp.this.posX + MilitaryCamp.this.sizeX - border &&
                            this.z <= MilitaryCamp.this.posZ + border) ||
                    (this.x + this.sizeX >= MilitaryCamp.this.posX + MilitaryCamp.this.sizeX - border &&
