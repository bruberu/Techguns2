...
public class Tanks extends WorldgenStructure {
    ...
    protected MBlock baseBlock = MBlockRegister.TANKS_BASE; // new
                                                            // MBlock(TGBlocks.METAL_PANEL,7);//TGChiselBlocks.technical_scaffold;
    protected MBlock wallBlock = MBlockRegister.TANKS_WALL;// new
                                                           // MBlock(TGBlocks.METAL_PANEL,0);//TGChiselBlocks.factory_wall;
    protected MBlock borderBlock = MBlockRegister.TANKS_BORDER; // new
                                                                // MBlock(TGBlocks.METAL_PANEL,3);//TGChiselBlocks.factory_hazard;
    ...
    public void setBlocks(World world, int posX, int posY, int posZ, int sizeX,
        ...
        if (sizeX > sizeZ) {
            ...
            for (int i = 0; i < count; i++) {
                // BlockUtils.fillBlocks(world, posX, posY+1, posZ+offset, sizeX, containerHeight, tank,
                // containerBlock);
                ...
                world.setBlockState(p.setPos(posX + 1, posY + 1, posZ + offset + tankWidth - 1), baseBlock.getState(),
                        2);
                world.setBlockState(p.setPos(posX + sizeX - 2, posY + 1, posZ + offset + tankWidth - 1),
                        baseBlock.getState(), 2);
                world.setBlockState(p.setPos(posX + 1, posY + 2, posZ + offset + tankWidth - 1), baseBlock.getState(),
                        2);
                world.setBlockState(p.setPos(posX + sizeX - 2, posY + 2, posZ + offset + tankWidth - 1),
                        baseBlock.getState(), 2);
            ...
            for (int i = 0; i < count; i++) {
                // BlockUtils.fillBlocks(world, posX+offset, posY+1, posZ, containerWidth, containerHeight, sizeZ,
                // containerBlock);
                ...
                world.setBlockState(p.setPos(posX + offset + tankWidth - 1, posY + 1, posZ + 1), baseBlock.getState(),
                        2);
                world.setBlockState(p.setPos(posX + offset + tankWidth - 1, posY + 1, posZ + sizeZ - 2),
                        baseBlock.getState(), 2);
                world.setBlockState(p.setPos(posX + offset + tankWidth - 1, posY + 2, posZ + 1), baseBlock.getState(),
                        2);
                world.setBlockState(p.setPos(posX + offset + tankWidth - 1, posY + 2, posZ + sizeZ - 2),
                        baseBlock.getState(), 2);
