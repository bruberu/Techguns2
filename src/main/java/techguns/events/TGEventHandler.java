...
public class TGEventHandler {
    ...
    public static void onMouseEvent(MouseEvent event) {
        if (event.getButton() != -1) {
            ...
            if (Minecraft.getMinecraft().inGameHasFocus) {
                ...
                if (event.getButton() == 0 && !ply.getHeldItemMainhand().isEmpty() &&
                        ply.getHeldItemMainhand().getItem() instanceof IGenericGun) {
                    ...
                    if (((IGenericGun) ply.getHeldItemMainhand().getItem()).isShootWithLeftClick()) {
                        ...
                        long diff = ShooterValues.getReloadtime(ClientProxy.get().getPlayerClient(), false) -
                   ...
                   * if(!ply.getHeldItemMainhand().isEmpty()&&ply.getHeldItemMainhand().getItem() instanceof
                   * GenericGunCharge) {
                   * //Charging gun is allowed
                   * } else if (!ply.getHeldItemMainhand().isEmpty() && ply.getHeldItemMainhand().getItem() instanceof
                   * GenericGun){
                   ...
                   * 
                   * }
                   */ else if (event.getButton() == 1 && !ply.isSneaking() && !ply.getHeldItemOffhand().isEmpty() &&
                        ply.getHeldItemOffhand().getItem() instanceof IGenericGun && GunManager.canUseOffhand(ply)) {
                            ...
                            if (((IGenericGun) ply.getHeldItemOffhand().getItem()).isShootWithLeftClick()) {
                                ...
                                long diff = ShooterValues.getReloadtime(ClientProxy.get().getPlayerClient(), true) -
                                        System.currentTimeMillis();
                                if (diff > 0) {
                                    if (event.isButtonstate()) {
                                        event.setCanceled(true);
                    ...
                    if (event.getButton() == 1 && ply.getHeldItemMainhand().getItem() instanceof GenericGunCharge &&
                            ((GenericGunCharge) ply.getHeldItemMainhand().getItem()).getLockOnTicks() > 0) {
    ...
    public static void rightClickEvent(PlayerInteractEvent.RightClickBlock event) {
        ...
        if (cancel) {
            ...
            if (ply.isSneaking() && !ply.getHeldItemOffhand().isEmpty() &&
                    (ply.getHeldItemOffhand().getItem() instanceof GenericGun) &&
                    (!event.getItemStack().isEmpty() && !(event.getItemStack().getItem() instanceof GenericGun))) {
                event.setUseBlock(Result.ALLOW);
            }
    ...
    public static void handleFovEvent(FOVUpdateEvent event) {
        ...
        if (TGConfig.cl_lockSpeedFov) {
            ...
            f = 1 / ((float) (((iattributeinstance.getAttributeValue() /
    ...
    public static void onRenderLivingEventPre(RenderLivingEvent.Pre event) {
        if (event.getEntity() instanceof EntityPlayer) {
            ...
            if (!stack.isEmpty() && stack.getItem() instanceof GenericGun &&
                    ((GenericGun) stack.getItem()).hasBowAnim()) {
                ...
                if (!stack2.isEmpty() && stack2.getItem() instanceof GenericGun &&
                        ((GenericGun) stack2.getItem()).hasBowAnim()) {
                    ModelBase mdl = event.getRenderer().getMainModel();
                    if (mdl instanceof ModelPlayer) {
                        ModelPlayer model = (ModelPlayer) mdl;
        ...
        switch (dt) {
            ...
            case LASER:
                // TODO
                event.setCanceled(true);
                DeathEffectEntityRenderer.doRender(event.getRenderer(), event.getEntity(), event.getX(), event.getY(),
                        event.getZ(), 0f, dt);
                break;
            case DEFAULT:
            default:
    ...
    public static void OnLivingAttack(LivingAttackEvent event) {
        if (event.getSource() instanceof TGDamageSource) {
            ...
            try {
                ...
                event.getSource() == DamageSource.IN_FIRE) && event.getEntityLiving() instanceof EntityPlayer) {
                    float bonus = GenericArmor.getArmorBonusForPlayer((EntityPlayer) event.getEntityLiving(),
                            TGArmorBonus.COOLING_SYSTEM, event.getEntityLiving().world.getTotalWorldTime() % 5 == 0);
        ...
        if (!item.isEmpty() && item.getItem() instanceof GenericGunMeleeCharge) {
            ...
            if (g.getMiningRadius(item) > 0) {
                ...
                if (sidehit != null) {
                    ...
                    List<BlockPos> blocks = BlockUtils.getBlockPlaneAroundAxisForMining(ply.world, ply, event.getPos(),
                            sidehit.getAxis(), g.getMiningRadius(item), false, g, item);
                ...
                if (tgs.deathType != DeathType.DEFAULT) {
                    if (Math.random() < tgs.goreChance) {
                        if (EntityDeathUtils.hasSpecialDeathAnim(entity, tgs.deathType)) {
                            ...
                            TGPackets.network.sendToAllAround(new PacketEntityDeathType(entity, tgs.deathType),
                                    TGPackets.targetPointAroundEnt(entity, 100.0f));
                        }
                    }
                }
            }
        ...
        if (!event.getEntity().world.isRemote) {
            if (event.getEntity() instanceof EntityPlayer) {
                ...
                if (props != null) {
                    ...
                    // TGPackets.network.sendToDimension(new PacketTGExtendedPlayerSync(props, false),
                    // event.entity.dimension);
                    TGPackets.network.sendTo(new PacketTGExtendedPlayerSync(ply, props, true), (EntityPlayerMP) ply);

                    ply.getDataManager().set(TGExtendedPlayer.DATA_FACE_SLOT,
                            props.tg_inventory.getStackInSlot(TGPlayerInventory.SLOT_FACE));
                    ply.getDataManager().set(TGExtendedPlayer.DATA_BACK_SLOT,
                            props.tg_inventory.getStackInSlot(TGPlayerInventory.SLOT_BACK));
                    ply.getDataManager().set(TGExtendedPlayer.DATA_HAND_SLOT,
                            props.tg_inventory.getStackInSlot(TGPlayerInventory.SLOT_HAND));
                    // ply.getDataWatcher().updateObject(TechgunsExtendedPlayerProperties.DATA_WATCHER_ID_FACESLOT,
                    // props.TG_inventory.inventory[TGPlayerInventory.SLOT_FACE]);
                    // ply.getDataWatcher().updateObject(TechgunsExtendedPlayerProperties.DATA_WATCHER_ID_BACKSLOT,
                    // props.TG_inventory.inventory[TGPlayerInventory.SLOT_BACK]);
    ...
    public static void onRenderHand(RenderHandEvent event) {
        ...
        if (!stack.isEmpty() && ((stack.getItem() instanceof GenericGunCharge &&
                ((GenericGunCharge) stack.getItem()).hasRightClickAction()) ||
                stack.getItem() instanceof GenericGrenade)) {
    ...
    public static void onRenderWorldLast(RenderWorldLastEvent event) {
        ...
        // states.printDebug();
        ClientProxy.get().particleManager.renderParticles(Minecraft.getMinecraft().getRenderViewEntity(),
        ...
        if (event.crafting.getItem() instanceof GenericGun) {
            ...
            for (int i = 0; i < event.craftMatrix.getSizeInventory(); i++) {
                ...
                if (!stack.isEmpty()) {
                    ...
                    } else if (stack.getItem() instanceof ITGSpecialSlot &&
                            ((ITGSpecialSlot) stack.getItem()).getSlot(stack) == TGSlotType.AMMOSLOT) {
                                if (!hasAmmo) {
                                    hasAmmo = true;
                                } else {
                                    hasInvalid = true;
            ...
            if (!hasInvalid && hasGun && hasAmmo) {
                ...
                items.forEach(i -> {
                    ...
                    if (amount > 0 && !event.player.world.isRemote) {
                        ItemStack it = i.copy();
                        it.setCount(amount);
                        event.player.world.spawnEntity(new EntityItem(event.player.world, event.player.posX,
                                event.player.posY, event.player.posZ, it));
                    }
                });
            }
    ...
    public static Method Block_getSilkTouchDrop = ReflectionHelper.findMethod(Block.class, "getSilkTouchDrop",
            "func_180643_i", IBlockState.class);
    ...
    public static void MilitaryCrateDrops(HarvestDropsEvent event) {
        ...
        if (state.getBlock() == TGBlocks.MILITARY_CRATE && !event.isSilkTouching() && !event.getWorld().isRemote) {
            ...
            if (ply != null) {
                ...
                LootTable loottable = ply.world.getLootTableManager()
                        .getLootTableFromLocation(TGBlocks.MILITARY_CRATE.getLootableForState(state));
                LootContext lootcontext = new LootContext.Builder((WorldServer) ply.world).withLuck(fortune)
                        .withPlayer(ply).build();
        ...
        if (!ply.isSneaking() && !item.isEmpty() && item.getItem() instanceof GenericGunMeleeCharge) {
            ...
            if (g.getMiningRadius(item) > 0 && g.getAmmoLeft(item) > 0) {
                ...
                if (target != null && target.typeOfHit == Type.BLOCK) {
                    ...
                    List<BlockPos> otherblocks = BlockUtils.getBlockPlaneAroundAxisForMining(ply.world, ply, p,
                            target.sideHit.getAxis(), g.getMiningRadius(item), false, g, item);
                    otherblocks.forEach(b -> {
                        RayTraceResult result = new RayTraceResult(
                                new Vec3d(b.getX() + 0.5d, b.getY() + 0.5d, b.getZ() + 0.5d), target.sideHit, b);
    ...
    public static void onItemSwitch(LivingEquipmentChangeEvent event) {
        if (event.getSlot() == EntityEquipmentSlot.MAINHAND || event.getSlot() == EntityEquipmentSlot.OFFHAND) {
            ...
            if (fromEffect || toEffect) {
                ...
                TGPackets.network.sendToDimension(new PacketNotifyAmbientEffectChange(event.getEntityLiving(), hand),
                        event.getEntityLiving().world.provider.getDimension());
            }
     ...
     * if(data!=null) {
     * event.getEntityPlayer().addPotionEffect(new PotionEffect(TGRadiationSystem.radiation_effect, data.radduration,
     * data.radamount-1, false,false));
     * }
     * }
     * }
     */
    ...
    public static void ItemRadiationTooltip(ItemTooltipEvent event) {
        ...
        if (!stack.isEmpty()) {
            ...
            if (data != null && data.radamount > 0) {
                event.getToolTip().add(ChatFormatting.GREEN + TextUtil.trans("techguns.radiation") + " " +
                        TextUtil.trans("potion.potency." + (data.radamount - 1)));
            }
        }
    }
