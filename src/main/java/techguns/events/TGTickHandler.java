...
public class TGTickHandler {
    ...
    static {
        // ITEMFOOD_onFoodEaten = ReflectionHelper.findMethod(ItemFood.class,"onFoodEaten","func_77849_c",
        // ItemStack.class, World.class, EntityPlayer.class);
        ITEMFOOD_onFoodEaten = ObfuscationReflectionHelper.findMethod(ItemFood.class, "func_77849_c", void.class,
                ItemStack.class, World.class, EntityPlayer.class);
    }
            ...
            if (event.player == cp.getPlayerClient()) {
                ...
                if (Minecraft.getMinecraft().inGameHasFocus && !event.player.isSpectator()) {
                    ...
                    if (!stack.isEmpty() && stack.getItem() instanceof IGenericGun &&
                            ((IGenericGun) stack.getItem()).isShootWithLeftClick()) {
                        if (cp.keyFirePressedMainhand) {
                            ...
                            if (props.getFireDelay(EnumHand.MAIN_HAND) <= 0) {
                                if (gun instanceof GenericGunCharge && ((GenericGunCharge) gun).getLockOnTicks() > 0 &&
                                        props.lockOnEntity != null &&
                                        props.lockOnTicks > ((GenericGunCharge) gun).getLockOnTicks()) {
                                    TGPackets.network.sendToServer(new PacketShootGunTarget(gun.isZooming(),
                                            EnumHand.MAIN_HAND, props.lockOnEntity));
                                    gun.shootGunPrimary(stack, event.player.world, event.player, gun.isZooming(),
                                            EnumHand.MAIN_HAND, props.lockOnEntity);
                                } else {
                                    TGPackets.network
                                            .sendToServer(new PacketShootGun(gun.isZooming(), EnumHand.MAIN_HAND));
                                    gun.shootGunPrimary(stack, event.player.world, event.player, gun.isZooming(),
                                            EnumHand.MAIN_HAND, null);
                                }
                    ...
                    if (!stackOff.isEmpty() && stackOff.getItem() instanceof IGenericGun &&
                            ((IGenericGun) stackOff.getItem()).isShootWithLeftClick()) {
                        if (cp.keyFirePressedOffhand) {
                            ...
                            if (props.getFireDelay(EnumHand.OFF_HAND) <= 0) {
                                ...
                                gun.shootGunPrimary(stackOff, event.player.world, event.player, gun.isZooming(),
                    ...
                    if (!stack.isEmpty() && stack.getItem() instanceof GenericGunCharge &&
                            ((GenericGunCharge) stack.getItem()).getLockOnTicks() > 0) {
                        // System.out.println("RMB: "+cp.keyFirePressedOffhand);
                        if (((GenericGunCharge) stack.getItem()).getAmmoLeft(stack) <= 0 &&
                                props.lockOnEntity != null) {
                            props.lockOnEntity = null;
                            props.lockOnTicks = -1;
    ...
    public static void onPlayerTick(PlayerTickEvent event) {
        ...
        if (event.phase == Phase.START) {
            ...
            if (!event.player.world.isRemote) {
                event.player.getDataManager().set(TGExtendedPlayer.DATA_FACE_SLOT,
                        props.tg_inventory.getStackInSlot(TGPlayerInventory.SLOT_FACE));
                event.player.getDataManager().set(TGExtendedPlayer.DATA_BACK_SLOT,
                        props.tg_inventory.getStackInSlot(TGPlayerInventory.SLOT_BACK));
                event.player.getDataManager().set(TGExtendedPlayer.DATA_HAND_SLOT,
                        props.tg_inventory.getStackInSlot(TGPlayerInventory.SLOT_HAND));
            } else {
                props.tg_inventory.setInventorySlotContents(TGPlayerInventory.SLOT_FACE,
                        event.player.getDataManager().get(TGExtendedPlayer.DATA_FACE_SLOT));
                props.tg_inventory.setInventorySlotContents(TGPlayerInventory.SLOT_BACK,
                        event.player.getDataManager().get(TGExtendedPlayer.DATA_BACK_SLOT));
                props.tg_inventory.setInventorySlotContents(TGPlayerInventory.SLOT_HAND,
                        event.player.getDataManager().get(TGExtendedPlayer.DATA_HAND_SLOT));
            }
            ...
            IAttributeInstance attributeMovespeed = event.player.getAttributeMap()
            ...
            IAttributeInstance attributeMaxHealth = event.player.getAttributeMap()
            ...
            IAttributeInstance attributeKnockbackResist = event.player.getAttributeMap()
                    .getAttributeInstance(SharedMonsterAttributes.KNOCKBACK_RESISTANCE);
            if (attributeKnockbackResist != null) {
                AttributeModifier mod_knockback_resistance = attributeKnockbackResist
                        .getModifier(UUID_KNOCKBACK_RESISTANCE);
            ...
            if (wearingTechgunsArmor) {
                ...
                if (event.player.isBurning()) {
                    float cooling = GenericArmor.getArmorBonusForPlayer(event.player, TGArmorBonus.COOLING_SYSTEM,
                            false);
                    if (cooling >= 1.0f) {
                ...
                if (healthBonus > 0.0f) {
                    ...
                    attributeMaxHealth
                            .applyModifier(new AttributeModifier(UUID_HEALTH, "TechgunsHealthbonus", healthBonus, 0));

                }

                float knockbackresistance = GenericArmor.getArmorBonusForPlayer(event.player,
                        TGArmorBonus.KNOCKBACK_RESISTANCE, false);
                if (knockbackresistance > 0.0f) {

                    attributeKnockbackResist.applyModifier(new AttributeModifier(UUID_KNOCKBACK_RESISTANCE,
                            "TechgunsKnockbackresistbonus", knockbackresistance, 0));
            ...
            if (event.player.world.isRemote) {
                ...
                if (event.player == ClientProxy.get().getPlayerClient()) {
                    float stepassist = GenericArmor.getArmorBonusForPlayer(event.player, TGArmorBonus.STEPASSIST,
                            false);
                    ...
                    if (!props.wasFlying) {
                        ...
                        if (event.player.world.isRemote) {
                            ...
                            TGPackets.network.sendToAllAround(
                                    new PacketPlaySound(TGSounds.ANTI_GRAV_START, event.player, 1.0f, 1.0f, false, true,
                                            false, true, TGSoundCategory.PLAYER_EFFECT),
                                    TGPackets.targetPointAroundEnt(event.player, 50.0f));
                        }
            ...
            if (event.player.world.isRemote) {
                ...
                // ONLY DO FOR OWN PLAYER!
                if (event.player == ClientProxy.get().getPlayerClient()) {
                    float flyspeedBonus = GenericArmor.getArmorBonusForPlayer(event.player, TGArmorBonus.FLYSPEED,
                            false);
                    if (flyspeedBonus > 0) {
            ...
            if (!TGConfig.disableAutofeeder && event.player.getFoodStats().getFoodLevel() <= 19) {
                ...
                if (props != null) {
                    ...
                    if (props.foodleft > 0) {
                        ...
                        if (!event.player.world.isRemote) {
                            TGPackets.network.sendTo(new PacketTGExtendedPlayerSync(event.player, props, true),
                                    (EntityPlayerMP) event.player);
                        }
                    } else {
                        ItemStack stack = InventoryUtil.consumeFood(props.tg_inventory.inventory,
                                props.tg_inventory.SLOTS_AUTOFOOD_START, props.tg_inventory.SLOTS_AUTOFOOD_END + 1);
                        if (!stack.isEmpty()) {
                            ...
                            if (!event.player.world.isRemote) {
                                // event.player.world.playSoundAtEntity(event.player, SoundEvents.ENTITY_PLAYER_BURP,
                                // 1.0f, 1.0f);
                                event.player.world.playSound(null, event.player.posX, event.player.posY,
                                        event.player.posZ, SoundEvents.ENTITY_PLAYER_BURP, SoundCategory.PLAYERS, 1f,
                                        1f);
                            }
                            ...
                            if (left > 0) {
                                ...
                                event.player.getFoodStats().addStats(food.getHealAmount(stack),
                                        food.getSaturationModifier(stack));
                                props.foodleft = 0;
                                props.lastSaturation = 0.0f;
                            }
                            if (!event.player.world.isRemote) {
             ...
             */
            IAttributeInstance attributeRadresistance = event.player.getAttributeMap()
                    .getAttributeInstance(TGRadiation.RADIATION_RESISTANCE);
            if (attributeRadresistance != null) {
                AttributeModifier rad_resist_faceslot = attributeRadresistance
                        .getModifier(ItemGasMask.UUID_RAD_RESIST_FACE);
            ...
            if (TGRadiationSystem.isEnabled() && event.side == Side.SERVER &&
                    event.player.world.getTotalWorldTime() % TGRadiationSystem.INVENTORY_RADIATION_INVERVALL == 0) {
                int maxrad = 0;
                ...
                if (maxrad > 0) {
                    event.player.addPotionEffect(new PotionEffect(TGRadiationSystem.radiation_effect,
                            TGRadiationSystem.INVENTORY_RADIATION_INVERVALL, maxrad - 1, false, false));
                }
            }
            ...
            if (event.side == Side.SERVER && event.player.world.getTotalWorldTime() % 20 == 0) {
                if (props.radlevel >= TGRadiationSystem.MINOR_POISONING &&
                        props.radlevel < TGRadiationSystem.SEVERE_POISONING) {
                ...
                if (props.radlevel >= TGRadiationSystem.LETHAL_POISONING) {
                    ...
                    event.player.attackEntityFrom(
                            TGDamageSource.causeLethalRadPoisoningDamage(null, null, DeathType.LASER), 2f);
                }
            }
            ...
            if (event.side == Side.SERVER) { // does only work server side
                ...
                if (!event.player.getHeldItemMainhand().isEmpty() &&
                        event.player.getHeldItemMainhand().getItem() instanceof GenericGun) {
                    gunMH = event.player.getHeldItemMainhand();
                }
                if (!event.player.getHeldItemOffhand().isEmpty() &&
                        event.player.getHeldItemOffhand().getItem() instanceof GenericGun) {
                    gunOH = event.player.getHeldItemOffhand();
                }

                if (!gunOH.isEmpty() && props.gunMainHand == gunOH || !gunMH.isEmpty() && props.gunOffHand == gunMH) {
                    TGPackets.network.sendToAllAround(new PacketSwapWeapon(event.player),
                            TGPackets.targetPointAroundEnt(event.player, 50.0f));
            ...
            // TODO: Remove this
            // if (props.isChargingWeapon() && (event.player.getItemInUseCount() <= 0)) { // ||
            // !(event.player.getHeldItemMainhand().getItem() instanceof GenericGunCharge)) {
    ...
    public static void TickParticleSystems(ClientTickEvent event) {
        if (event.phase == Phase.END) {
            ...
            // System.out.println("TGParticleCount:"+ClientProxy.get().particleManager.getList().getSizeDebug()+ " ::
            // "+ClientProxy.get().particleManager.getList().getSize());
        } else {
