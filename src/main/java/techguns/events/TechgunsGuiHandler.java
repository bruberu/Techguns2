...
public class TechgunsGuiHandler implements IGuiHandler {
    ...
    public static void openGuiForPlayer(EntityPlayer ply, TileEntity tile) {
        ...
        if (id > 0) {
            ply.openGui(Techguns.instance, id, tile.getWorld(), tile.getPos().getX(), tile.getPos().getY(),
                    tile.getPos().getZ());
        }
    }
    ...
    public static class GuiHandlerRegister {
        ...
        public <T extends TileEntity> void addEntry(Class<? extends T> tile, IGuiFactory<T> clientgui,
                                                    IGuiFactory<T> servergui) {
