...
public class TGPlayerInventoryContainer extends Container {
    ...
    public TGPlayerInventoryContainer(EntityPlayer player) {
        ...
        this.addSlotToContainer(
                new SlotFood(tgplayerinv, TGPlayerInventory.SLOTS_AUTOFOOD_START + 2, 116 + 18 * 2, 24));

        this.addSlotToContainer(new SlotTG(tgplayerinv, TGPlayerInventory.SLOT_AUTOHEAL, 97, 24, TGSlotType.HEALSLOT));

        for (int i = 0; i < 2; i++) {
            for (int j = 0; j < 4; j++) {
                this.addSlotToContainer(new SlotTG(tgplayerinv, TGPlayerInventory.SLOTS_AMMO_START + (i * 4) + j,
                        98 + j * 18, 44 + i * 18, TGSlotType.AMMOSLOT));
            }
        }
    ...
    public ItemStack transferStackInSlot(EntityPlayer player, int id) {
        ...
        if (slot != null && slot.getHasStack()) {
             ...
             */
            // Try merge Armor
            else if (itemstack.getItem() instanceof ItemArmor && !((Slot) this.inventorySlots
                    .get(TGPlayerInventory.NUMSLOTS + ((ItemArmor) itemstack.getItem()).armorType.getIndex()))
                            .getHasStack()) {
                                int j = TGPlayerInventory.NUMSLOTS +
                                        ((ItemArmor) itemstack.getItem()).armorType.getIndex();

                                if (!this.mergeItemStack(itemstack1, j, j + 1, false)) {
                                    return ItemStack.EMPTY;
                                }
                            } else
                if ((id >= DEFAULT_INV_START && id < DEFAULT_INV_START + 36) && hasTGSpecialSlot(itemstack)) {
