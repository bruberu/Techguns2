    ...
    public OreDrillContainer(InventoryPlayer player, OreDrillTileEntMaster ent) {
        ...
        if (inventory instanceof ItemStackHandlerPlus) {
            ...
            this.addSlotToContainer(
                    new SlotDrill(handler, OreDrillTileEntMaster.SLOT_DRILL, SLOT_DRILL_X, SLOTS_ROW1_Y, tile));
            this.addSlotToContainer(
                    new SlotFurnaceFuelTG(handler, OreDrillTileEntMaster.SLOT_FURNACE, SLOT_DRILL_X, SLOTS_FURNACE_Y));

            for (int i = 0; i < 3; i++) {
                for (int j = 0; j < 3; j++) {
                    this.addSlotToContainer(
                            new SlotItemHandlerOutput(handler, OreDrillTileEntMaster.SLOT_OUTPUT1 + (i * 3) + j,
                                    SLOTS_OUTPUT_X + 18 * i, SLOTS_ROW1_Y + 18 * j));
                }
            }
        }
    ...
    public ItemStack transferStackInSlot(EntityPlayer ply, int id) {
        ...
        if (slot.getHasStack()) {
            ...
            if (!stack.isEmpty()) {
                ...
                int machine_inventory_end = OreDrillTileEntMaster.SLOT_OUTPUT1 +
                        OreDrillTileEntMaster.SLOT_OUTPUT_COUNT;
