            ...
            for (int i = 0; i < 3; i++) {
                for (int j = 0; j < 3; j++) {
                    this.addSlotToContainer(
                            new SlotItemHandlerOutput(handler, 9 + (i * 3) + j, 116 + 18 * i, 17 + 18 * j) {
    ...
    public void detectAndSendChanges() {
        ...
        for (int j = 0; j < this.listeners.size(); ++j) {
            ...
            if (this.lastPowerStored != this.tile.getEnergyStorage().getEnergyStored()) {
                listener.sendWindowProperty(this, FIELD_SYNC_ID_POWER_STORED,
                        this.tile.getEnergyStorage().getEnergyStored());
            }
        ...
        if (slot.getHasStack()) {
            ...
            if (!stack.isEmpty()) {
                ...
                if (slotid <= TurretTileEnt.SLOT_ARMOR) {
                    ...
                    if (!this.mergeItemStack(stack1, TurretTileEnt.SLOT_ARMOR + 1, TurretTileEnt.SLOT_ARMOR + 1 + 36,
                            false)) {
                    ...
                    if (stack.getItem() instanceof GenericGun) {
                        ...
                        if (!this.mergeItemStack(stack1, TurretTileEnt.SLOT_WEAPON, TurretTileEnt.SLOT_WEAPON + 1,
                                false)) {
                            return ItemStack.EMPTY;
                        }
                        slot.onSlotChange(stack1, stack);
                    } else if (stack.getItem() instanceof ITGSpecialSlot &&
                            (((ITGSpecialSlot) stack.getItem()).getSlot(stack) == TGSlotType.TURRETARMOR)) {
                                // put in armorslot
                                if (!this.mergeItemStack(stack1, TurretTileEnt.SLOT_ARMOR, TurretTileEnt.SLOT_ARMOR + 1,
                                        false)) {
                                    return ItemStack.EMPTY;
                                }
                                slot.onSlotChange(stack1, stack);
                                ...
                                if (!this.mergeItemStack(stack1, TurretTileEnt.SLOT_OUTPUT1,
                                        TurretTileEnt.SLOT_INPUT1 + TurretTileEnt.INPUTS_SIZE, false)) {
                                    return ItemStack.EMPTY;
                                }
                                slot.onSlotChange(stack1, stack);
                            }
