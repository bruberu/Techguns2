    ...
    public GrinderContainer(InventoryPlayer player, GrinderTileEnt ent) {
        ...
        if (inventory instanceof ItemStackHandlerPlus) {
            ...
            this.addSlotToContainer(
                    new SlotMachineInput(handler, GrinderTileEnt.SLOT_INPUT, SLOT_INPUT_X, SLOTS_INPUT_Y));
            this.addSlotToContainer(
                    new SlotMachineUpgrade(handler, GrinderTileEnt.SLOT_UPGRADE, SLOT_UPGRADE_X, SLOT_UPGRADE_Y));

            for (int i = 0; i < 3; i++) {
                for (int j = 0; j < 3; j++) {
                    // System.out.println("AddSlot: "+(GrinderTileEnt.SLOT_OUTPUT0+i*3+j) +" AT:"+ (SLOT_OUTPUT0_X+i*18)
                    // + ","+ (SLOTS_INPUT_Y+i*18));
                    this.addSlotToContainer(new SlotItemHandlerOutput(inventory,
                            GrinderTileEnt.SLOT_OUTPUT0 + i * 3 + j, SLOT_OUTPUT0_X + j * 18, SLOTS_INPUT_Y + i * 18));
                }
            }
