...
public class AmmoPressContainer extends BasicMachineContainer {
    ...
    public AmmoPressContainer(InventoryPlayer player, AmmoPressTileEnt ent) {
        ...
        if (inventory instanceof ItemStackHandlerPlus) {
            ...
            this.addSlotToContainer(new SlotAmmoPressInput(handler, AmmoPressTileEnt.SLOT_METAL1, SLOT_METAL1_X,
                    SLOTS_INPUT_Y, SlotTG.INGOTDARKSLOT_TEX));
            this.addSlotToContainer(new SlotAmmoPressInput(handler, AmmoPressTileEnt.SLOT_METAL2, SLOT_METAL2_X,
                    SLOTS_INPUT_Y, SlotTG.INGOTSLOT_TEX));
            this.addSlotToContainer(new SlotAmmoPressInput(handler, AmmoPressTileEnt.SLOT_POWDER, SLOT_POWDER_X,
                    SLOTS_INPUT_Y, SlotFabricator.FABRICATOR_SLOTTEX_POWDER));

            this.addSlotToContainer(
                    new SlotItemHandlerOutput(inventory, AmmoPressTileEnt.SLOT_OUTPUT, SLOT_OUTPUT_X, SLOTS_OUTPUT_Y));
            this.addSlotToContainer(
                    new SlotMachineUpgrade(handler, AmmoPressTileEnt.SLOT_UPGRADE, SLOT_UPGRADE_X, SLOTS_OUTPUT_Y));
        }
        ...
        if (slot.getHasStack()) {
            ...
            if (!stack.isEmpty()) {
                ...
                if (id >= 0 && id <= AmmoPressTileEnt.SLOT_UPGRADE) {
                    ...
                    if (!this.mergeItemStack(stack1, AmmoPressTileEnt.SLOT_UPGRADE + 1,
                            AmmoPressTileEnt.SLOT_UPGRADE + 37, false)) {
