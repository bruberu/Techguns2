...
public class SlotDrill extends SlotMachineInput {
    ...
    public boolean isItemValid(ItemStack item) {
        ...
        if (!item.isEmpty() && item.getItem() instanceof ITGSpecialSlot) {
            ...
            return (itm.getSlot(item) == TGSlotType.DRILL_SMALL && radius == 0) ||
                    (itm.getSlot(item) == TGSlotType.DRILL_MEDIUM && radius > 0 && radius < 3) ||
                    (itm.getSlot(item) == TGSlotType.DRILL_LARGE && radius >= 3);
        }
        return false;
    }
