...
public class RepairBenchGui extends OwnedTileEntGui {
    ...
    public static final ResourceLocation texture = new ResourceLocation("techguns:textures/gui/repair_bench_gui.png");// new
    ...
    private void drawTooltipRepairMats(ItemStack item, int mx, int my) {
        if (!item.isEmpty()) {
            ...
            if (item.getItem() instanceof GenericArmor &&
                    ((GenericArmor) item.getItem()).canRepairOnRepairBench(item)) {
                ...
                if (item.getItemDamage() > 0) {
                    ...
                    for (int i = 1; i < mats.size() + 1; i++) {
                        String prefix = "";
                        if (InventoryUtil.canConsumeItem(this.tileent.getInventory(), mats.get(i - 1), 0,
                                this.tileent.getInventory().getSlots()) > 0) {
                            prefix = "\u00A7c";
                        }
                        tooltips.add(prefix + mats.get(i - 1).getCount() + "x " +
                                TextUtil.trans(mats.get(i - 1).getTranslationKey() + ".name"));
                    }
            ...
            } else if (item.getItem() instanceof GenericShield &&
                    ((GenericShield) item.getItem()).canRepairOnRepairBench(item)) {
