...
public class TGOreClusters implements ITGInitializer {
    ...
    public void preInit(FMLPreInitializationEvent event) {
        registry.put(EnumOreClusterType.COAL,
                new OreCluster(TGConfig.mininglevel_coal, TGConfig.oremult_coal, TGConfig.powermult_coal));
        registry.put(EnumOreClusterType.COMMON_METAL, new OreCluster(TGConfig.mininglevel_common_metal,
                TGConfig.oremult_common_metal, TGConfig.powermult_common_metal));
        registry.put(EnumOreClusterType.RARE_METAL, new OreCluster(TGConfig.mininglevel_rare_metal,
                TGConfig.oremult_rare_metal, TGConfig.powermult_rare_metal));
        registry.put(EnumOreClusterType.SHINY_METAL, new OreCluster(TGConfig.mininglevel_shiny_metal,
                TGConfig.oremult_shiny_metal, TGConfig.powermult_shiny_metal));
        registry.put(EnumOreClusterType.URANIUM,
                new OreCluster(TGConfig.mininglevel_uranium, TGConfig.oremult_uranium, TGConfig.powermult_uranium));
        registry.put(EnumOreClusterType.COMMON_GEM, new OreCluster(TGConfig.mininglevel_common_gem,
                TGConfig.oremult_common_gem, TGConfig.powermult_common_gem));
        registry.put(EnumOreClusterType.SHINY_GEM, new OreCluster(TGConfig.mininglevel_shiny_gem,
                TGConfig.oremult_shiny_gem, TGConfig.powermult_shiny_gem));
        registry.put(EnumOreClusterType.NETHER_CRYSTAL, new OreCluster(TGConfig.mininglevel_nether_crystal,
                TGConfig.oremult_nether_crystal, TGConfig.powermult_nether_crystal));
        registry.put(EnumOreClusterType.OIL,
                new OreCluster(TGConfig.mininglevel_oil, TGConfig.oremult_oil, TGConfig.powermult_oil));
    }
    ...
    public void RecipeInit() {
        ...
        this.addOreToCluster(new ItemStack(TGBlocks.TG_ORE, 1, EnumOreType.ORE_COPPER.ordinal()),
                EnumOreClusterType.COMMON_METAL, 30);
        this.addOreToCluster(new ItemStack(TGBlocks.TG_ORE, 1, EnumOreType.ORE_TIN.ordinal()),
                EnumOreClusterType.COMMON_METAL, 25);

        this.addOreToCluster(new ItemStack(TGBlocks.TG_ORE, 1, EnumOreType.ORE_LEAD.ordinal()),
                EnumOreClusterType.RARE_METAL, 50);

        this.addOreToCluster(new ItemStack(TGBlocks.TG_ORE, 1, EnumOreType.ORE_TITANIUM.ordinal()),
                EnumOreClusterType.SHINY_METAL, 20);
        ...
        this.addOreToCluster(new ItemStack(TGBlocks.TG_ORE, 1, EnumOreType.ORE_URANIUM.ordinal()),
                EnumOreClusterType.URANIUM, 50);
            ...
            if (obj instanceof OreClusterWeightedEntry) {
                ...
                if (this.oredictname != null && other.oredictname != null) {
                    ...
                    return this.ore.getItem() == other.ore.getItem() &&
                            this.ore.getItemDamage() == other.ore.getItemDamage();
        ...
        public MachineOperation getNewOperation(World w, double orePerHour, int radius, double powerMult) {
            if (!oreEntries.isEmpty()) {
                ...
                if (!entry.ore.isEmpty()) {
                    op = new MachineOperation(new ItemStack(entry.ore.getItem(), 1, entry.ore.getItemDamage()), null,
                            new ItemStack[0]);
                } else if (entry.fluid != null) {
                    op = new MachineOperation(ItemStack.EMPTY,
                            new FluidStack(entry.fluid.getFluid(), Fluid.BUCKET_VOLUME), new ItemStack[0]);
                }
                if (op != null) {
                    op.setPowerPerTick((int) (8 * orePerHour * (1 + Math.max(radius - 1, 0) * 0.2) * powerMult *
                            TGConfig.oreDrillMultiplierPower));
                    return op;
                }
