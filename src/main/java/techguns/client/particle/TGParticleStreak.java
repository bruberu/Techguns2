...
public class TGParticleStreak extends TGParticle {
    ...
    public void renderParticle(BufferBuilder buffer, Entity entityIn, float partialTickTime, float rotX, float rotZ,
        ...
        if (prev == null) {
            ...
            // Vec3d v_prev = new Vec3d(prev.posX, prev.posY,
            // prev.posZ).subtract(ClientProxy.get().getPlayerClient().getPositionEyes(partialTickTime));
            Vec3d v_prev = new Vec3d(prev.posX, prev.posY, prev.posZ)
                    .subtract(ClientProxy.get().getPlayerClient().getPositionVector());
            ...
            if (prev.pos1 != null && prev.pos2 != null) {
                ...
                p4 = new Vec3d(v_cross.x * fscaleP + v_prev.x, v_cross.y * fscaleP + v_prev.y,
                        v_cross.z * fscaleP + v_prev.z);
                p3 = new Vec3d(v_cross.x * -fscaleP + v_prev.x, v_cross.y * -fscaleP + v_prev.y,
                        v_cross.z * -fscaleP + v_prev.z);
            ...
            // p3 = new Vec3d(v_cross.x* -fscaleP + v_prev.x, v_cross.y* -fscaleP + v_prev.y, v_cross.z* -fscaleP +
            // v_prev.z);
            ...
            buffer.pos(p1.x, p1.y, p1.z).tex((double) ua, (double) va)
            ...
            buffer.pos(p2.x, p2.y, p2.z).tex((double) ub, (double) vb)
                    .color(this.particleRed, this.particleGreen, this.particleBlue, this.particleAlpha).lightmap(0, 240)
                    .normal(0.0f, 1.0f, 0.0f).endVertex();
            buffer.pos(p3.x, p3.y, p3.z).tex((double) uc, (double) vc)
                    .color(prev.particleRed, prev.particleGreen, prev.particleBlue, prev.particleAlpha).lightmap(0, 240)
                    .normal(0.0f, 1.0f, 0.0f).endVertex();
            buffer.pos(p4.x, p4.y, p4.z).tex((double) ud, (double) vd)
                    .color(prev.particleRed, prev.particleGreen, prev.particleBlue, prev.particleAlpha).lightmap(0, 240)
                    .normal(0.0f, 1.0f, 0.0f).endVertex();
