...
public class TGSound extends MovingSound {
    ...
    public TGSound(SoundEvent soundname, Entity entity, float volume, float pitch, boolean repeat, boolean moving,
                   boolean gunPosition, TGSoundCategory category, EntityCondition condition) {
        this(soundname, entity, volume, pitch, repeat, moving, gunPosition, category);
        this.condition = condition;
    }

    public TGSound(SoundEvent soundname, Entity entity, float volume, float pitch, boolean repeat, boolean moving,
                   boolean gunPosition, TGSoundCategory category) {
        this(soundname, entity, volume, pitch, repeat, moving, category);
        this.gunPosition = gunPosition;
        if (gunPosition) {
            if (entity instanceof EntityLivingBase) {
                Vec2f pos = MathUtil.polarOffsetXZ(xPosF, zPosF, 1.0f,
                        (float) (((EntityLivingBase) entity).rotationYawHead * Math.PI / 180.0f));
                xPosF = pos.x;
                zPosF = pos.y;
    ...
    public TGSound(SoundEvent soundname, Entity entity, float volume, float pitch, boolean repeat,
                   TGSoundCategory category) {
    ...
    public TGSound(SoundEvent soundname, float posx, float posy, float posz, float volume, float pitch, boolean repeat,
                   TGSoundCategory category) {
