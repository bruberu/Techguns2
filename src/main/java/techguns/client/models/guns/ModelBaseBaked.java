...
public class ModelBaseBaked extends ModelMultipart {
    ...
    protected static Method RenderItem_renderModel = ReflectionHelper.findMethod(RenderItem.class, "renderModel",
            "func_191961_a", IBakedModel.class, ItemStack.class);
    ...
    public ModelBaseBaked(ResourceLocation texture_loc, ModelResourceLocation... modellocs) {
        Arrays.stream(modellocs).forEach(modelLoc -> {
            ...
            IModel model = ModelLoaderRegistry.getModelOrLogError(modelLoc,
                    "Could not load model" + modelLoc.toString());
            IBakedModel bakedModel = model.bake(TRSRTransformation.identity(), DefaultVertexFormats.ITEM, r -> {
                ...
                try {
                    PngSizeInfo png = PngSizeInfo
                            .makeFromResource(Minecraft.getMinecraft().getResourceManager().getResource(texture_loc));
    ...
    public ModelBaseBaked(Item item, int parts) {
        for (int i = 0; i < parts; i++) {
            IBakedModel m = Minecraft.getMinecraft().getRenderItem().getItemModelMesher().getModelManager()
                    .getModel(
                            new ModelResourceLocation(item.getRegistryName() + (i == 0 ? "" : ("_" + i)), "inventory"));
            bakedModels.add(m);
        }
    }
    ...
    public void render(Entity entityIn, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw,
        ...
        try {
            RenderItem_renderModel.invoke(Minecraft.getMinecraft().getRenderItem(), bakedModels.get(0),
                    ItemStack.EMPTY);
