...
public class ScreenEffect implements IScreenEffect {
     ...
     * (non-Javadoc)
     * 
     * @see techguns.client.render.fx.IScreenEffect#doRender(float, float, float, float, float, float, float, float,
     * boolean)
     */
    @Override
    public void doRender(float progress, float offsetX, float offsetY, float offsetZ, float scale, float rot_x,
                         float rot_y, float rot_z, boolean is3p) {
        ...
        if (currentFrame < numSprites) {
            ...
            if (is3p) {
                ...
                buffer.pos(offsetX - f, offsetY, offsetZ + f).tex(U1, V2).color(colorR, colorG, colorB, alpha)
                        .endVertex();
                buffer.pos(offsetX - f, offsetY, offsetZ - f).tex(U1, V1).color(colorR, colorG, colorB, alpha)
                        .endVertex();
                buffer.pos(offsetX + f, offsetY, offsetZ - f).tex(U2, V1).color(colorR, colorG, colorB, alpha)
                        .endVertex();
                buffer.pos(offsetX + f, offsetY, offsetZ + f).tex(U2, V2).color(colorR, colorG, colorB, alpha)
                        .endVertex();

                buffer.pos(offsetX, offsetY - f, offsetZ + f).tex(U1, V2).color(colorR, colorG, colorB, alpha)
                        .endVertex();
                buffer.pos(offsetX, offsetY - f, offsetZ - f).tex(U1, V1).color(colorR, colorG, colorB, alpha)
                        .endVertex();
                buffer.pos(offsetX, offsetY + f, offsetZ - f).tex(U2, V1).color(colorR, colorG, colorB, alpha)
                        .endVertex();
                buffer.pos(offsetX, offsetY + f, offsetZ + f).tex(U2, V2).color(colorR, colorG, colorB, alpha)
                        .endVertex();
            }
