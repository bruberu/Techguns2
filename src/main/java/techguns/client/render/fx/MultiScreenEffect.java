    ...
    public void doRender(float progress, float offsetX, float offsetY, float offsetZ, float scale, float rot_x,
        ...
        for (EffectEntry entry : effects) {
            if (progress > entry.start && progress < entry.end) {
                float prog = (progress - entry.start) / (entry.end - entry.start); // Math.max(0.0f,Math.min(1.0f,
                                                                                   // (progress - entry.start) /
                                                                                   // (entry.end-entry.start)));
                entry.effect.doRender(prog, offsetX, offsetY, offsetZ + z_offset * i, scale * entry.scale, rot_x, rot_y,
                        rot_z, is3p);
                i++;
            }
        }
