...
public class RenderGunBase extends RenderItemBase {
    ...
    public void renderItem(TransformType transform, ItemStack stack, EntityLivingBase entityIn, boolean leftHand) {
        ...
        if (entityIn != null) {
            ...
            isOffhand = ((!leftHand) && entityIn.getPrimaryHand() == EnumHandSide.LEFT) ||
                    ((leftHand) && entityIn.getPrimaryHand() == EnumHandSide.RIGHT);

            // if (entityIn.getPrimaryHand()== EnumHandSide.LEFT){
            // leftHand = !leftHand;
            // }

            // if ((transform == TransformType.FIRST_PERSON_LEFT_HAND || transform ==
            // TransformType.FIRST_PERSON_RIGHT_HAND) &&ClientProxy.PARTIAL_TICK_TIME < 0.05f)
        ...
        if (values != null && (TransformType.FIRST_PERSON_LEFT_HAND == transform ||
                TransformType.FIRST_PERSON_RIGHT_HAND == transform ||
                TransformType.THIRD_PERSON_LEFT_HAND == transform ||
                TransformType.THIRD_PERSON_RIGHT_HAND == transform)) {
            ...
            if (TransformType.FIRST_PERSON_LEFT_HAND == transform ||
                    TransformType.FIRST_PERSON_RIGHT_HAND == transform ||
                    TransformType.THIRD_PERSON_LEFT_HAND == transform ||
                    TransformType.THIRD_PERSON_RIGHT_HAND == transform) {
        ...
        if (TransformType.FIRST_PERSON_LEFT_HAND == transform || TransformType.FIRST_PERSON_RIGHT_HAND == transform) {
            ...
            if (!isOffhand && gun.isZooming() && this.scope != null) {
                ...
                this.transformFirstPerson(fireProgress, reloadProgress, chargeProgress,
                        TransformType.FIRST_PERSON_LEFT_HAND == transform, sneaking && isOffhand);
            }

        } else if (TransformType.THIRD_PERSON_LEFT_HAND == transform ||
                TransformType.THIRD_PERSON_RIGHT_HAND == transform) {
                    this.transformThirdPerson(entityIn, fireProgress, reloadProgress,
                            TransformType.THIRD_PERSON_LEFT_HAND == transform);
        ...
        if (!renderScope) {
            ...
            for (int i = 0; i < parts; i++) {
                ...
                model.render(entityIn, 0, 0, 0, 0, 0, SCALE, gun.getAmmoLeft(stack), reloadProgress, transform, i,
                        fireProgress, chargeProgress);
                GlStateManager.color(1f, 1f, 1f, 1f);
            }
            ...
            if (this.scopeRecoilAnim != null && fireProgress > 0f) {
                this.scopeRecoilAnim.play(fireProgress, TransformType.FIRST_PERSON_LEFT_HAND == transform,
                        this.scopeRecoilParams);
            }
    ...
    public void applyAnimForParticles(EntityLivingBase entity, float reloadProgress, TransformType transform,
                                      boolean sneaking, boolean isOffhand) {
        GlStateManager.pushMatrix();
        ...
        if (transform == TransformType.FIRST_PERSON_LEFT_HAND || transform == TransformType.FIRST_PERSON_RIGHT_HAND) {
            this.transformFirstPerson(0f, reloadProgress, 0f, TransformType.FIRST_PERSON_LEFT_HAND == transform,
                    sneaking && isOffhand);
        } else if (transform == TransformType.THIRD_PERSON_LEFT_HAND ||
                transform == TransformType.THIRD_PERSON_RIGHT_HAND) {
                    this.transformThirdPerson(entity, 0f, reloadProgress,
                            TransformType.THIRD_PERSON_LEFT_HAND == transform);
                }
    }

    protected void transformFirstPerson(float fireProgress, float reloadProgress, float chargeProgress, boolean left,
                                        boolean shoudLowerWeapon) {
        if (chargeProgress > 0) {
    ...
    protected void drawMuzzleFx(float progress, byte attackType, boolean leftHand) {
        if (this.muzzleFX != null) {
            ...
            // this.muzzleFX.doRender(progress, (x+Keybinds.X)/*+leftOffset*/, this.muzzleFX_y+Keybinds.Y,
            // this.muzzleFX_z+Keybinds.Z, this.muzzleFX_scale, false);
    ...
    protected void drawMuzzleFx3P(float progress, byte attackType, boolean leftHand) {
        if (this.muzzleFX != null) {
            // float x= leftHand?this.muzzleFX_x_l:this.muzzleFX_x_r;
            // this.muzzleFX.doRender(progress, 0, this.muzzleFX_3p_y+Keybinds.Y, this.muzzleFX_3p_z+Keybinds.Z,
            // this.muzzleFX_scale*this.muzzleFX_3p_scale, true);
            this.muzzleFX.doRender(progress, 0, this.muzzleFX_3p_y, this.muzzleFX_3p_z,
                    this.muzzleFX_scale * this.muzzleFX_3p_scale, true);
        }
    }
