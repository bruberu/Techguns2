...
public class RenderItemBase implements IItemRenderer {
    ...
    protected void applyTranslation(TransformType transform) {
        ...
        if (index >= 0) {
            ...
            // GlStateManager.translate((translateType[index][0]+Keybinds.X)*mirror, translateType[index][1]+Keybinds.Y,
            // translateType[index][2]+Keybinds.Z);
            GlStateManager.translate((translateType[index][0]) * mirror, translateType[index][1],
                    translateType[index][2]);
        }
    }
    ...
    protected void applyBaseTranslation() {
        ...
        // GlStateManager.translate(this.translateBase[0]+Keybinds.X, this.translateBase[1]+Keybinds.Y,
        // this.translateBase[2]+Keybinds.Z);
    }
    ...
    public static Vec3d getTransformAngles(EntityLivingBase entity, EnumHandSide hand, float ptt) {
        ...
        float limbSwingAmount = entity.prevLimbSwingAmount +
                (entity.limbSwingAmount - entity.prevLimbSwingAmount) * ptt;
        ...
        if (entity.swingProgress > 0.0f) {
            float bbry = MathHelper.sin(MathHelper.sqrt(entity.swingProgress) * ((float) Math.PI * 2F)) * 0.2F; // =
        ...
        // Entity Rotation
        float entityYaw = (float) MathUtil.D2R *
                (entity.prevRotationYaw + (entity.rotationYaw - entity.prevRotationYaw) * ptt);
        lAngleY += entityYaw;
        rAngleY += entityYaw;
        //
        if (hand == EnumHandSide.LEFT) {
