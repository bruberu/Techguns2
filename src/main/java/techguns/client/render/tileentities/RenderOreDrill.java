...
public class RenderOreDrill extends TileEntitySpecialRenderer<OreDrillTileEntMaster> {
    ...
    protected ResourceLocation texture2 = new ResourceLocation(Techguns.MODID,
            "textures/blocks/drillhead_obsidian.png");
    ...
    public void render(OreDrillTileEntMaster te, double x, double y, double z, float partialTick, int destroyStage,
        ...
        if (tile.isFormed()) {
            ...
            // System.out.println("Radius:"+tile.getDrillRadius()+" HasDrill:"+tile.hasDrill()+"
            // DrillType:"+tile.getDrillType());
    ...
    protected void drawDriller(OreDrillTileEntMaster tile, OreDrillCube cube, Tessellator tess) {
        ...
        int[] worldpos = getNextBlockInDirOffset(tilePos.getX(), tilePos.getY(), tilePos.getZ(),
                tile.getDrill_direction(), tile.getEngines());
        ...
        for (int i = 0; i < length * DRILLERCUBESPERBLOCK; i++) {
            ...
            float size = ((tile.getDrillRadius() == 0 ? 1.0f : tile.getDrillRadius() * 1.0f) * factor) * (1.0f / max) *
                    radiusfactor;
    ...
    protected void drawDrillRod(OreDrillTileEntMaster tile, Tessellator tess) {
        ...
        for (int i = 0; i < length; i++) {
            ...
            tessellatorcube.drawCube(axis[0] == 0 ? size : pos, (axis[1] == 0 ? size : pos),
                    (axis[2] == 0 ? size : pos), tess);
