...
public class TGLayerRendererer implements LayerRenderer<EntityPlayer> {
    ...
    public void doRenderLayer(EntityPlayer player, float limbSwing, float limbSwingAmount, float partialTicks,
        ...
        if (isSlotVisible(TGPlayerInventory.SLOT_FACE, player)) {
            this.renderSlot(props.tg_inventory.inventory.get(TGPlayerInventory.SLOT_FACE), player, limbSwing,
                    limbSwingAmount, partialTicks, ageInTicks, netHeadYaw, headPitch, scale);
        }
        if (isSlotVisible(TGPlayerInventory.SLOT_BACK, player)) {
            this.renderSlot(props.tg_inventory.inventory.get(TGPlayerInventory.SLOT_BACK), player, limbSwing,
                    limbSwingAmount, partialTicks, ageInTicks, netHeadYaw, headPitch, scale);
        }
        if (isSlotVisible(TGPlayerInventory.SLOT_HAND, player)) {
            this.renderSlot(props.tg_inventory.inventory.get(TGPlayerInventory.SLOT_HAND), player, limbSwing,
                    limbSwingAmount, partialTicks, ageInTicks, netHeadYaw, headPitch, scale);
        }
    }
    ...
    protected void renderSlot(ItemStack slot, EntityPlayer player, float limbSwing, float limbSwingAmount,
