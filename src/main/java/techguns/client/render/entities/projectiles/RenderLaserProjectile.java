...
public class RenderLaserProjectile extends Render<LaserProjectile> {
    ...
    public void doRender(LaserProjectile laser, double x, double y, double z, float entityYaw, float partialTicks) {
        ...
        // Vec3 viewer = Vec3.createVectorHelper(this.renderManager.viewerPosX, this.renderManager.viewerPosY,
        // this.renderManager.viewerPosZ);

        // System.out.printf("Renderer: Pitch: %.3f, Yaw: %.3f, Distance: %.3f\n", laser.laserPitch, laser.laserYaw,
        // laser.distance);
        ...
        if (distance > distance_start) { // Beam Segment
            ...
            for (int i = 0; i < 2; ++i) {
                ...
                bufferbuilder.pos(distance, -width, 0.0D).tex(u + prog, 0).color(1.0f, 1.0f, 1.0f, brightness)
                        .endVertex();
                bufferbuilder.pos(distance_start, -width, 0.0D).tex(prog, 0).color(1.0f, 1.0f, 1.0f, brightness)
                        .endVertex();
                bufferbuilder.pos(distance_start, width, 0.0D).tex(prog, 1).color(1.0f, 1.0f, 1.0f, brightness)
                        .endVertex();
                bufferbuilder.pos(distance, width, 0.0D).tex(u + prog, 1).color(1.0f, 1.0f, 1.0f, brightness)
                        .endVertex();
                tessellator.draw();
            }
