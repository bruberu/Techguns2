...
public class RenderGrenade40mmProjectile extends Render<Grenade40mmProjectile> {
    ...
    private static final ResourceLocation texture = new ResourceLocation(Techguns.MODID,
            "textures/entity/launchergrenade.png");
    private static final ModelResourceLocation modelLoc = new ModelResourceLocation(
            new ResourceLocation(Techguns.MODID, "grenade40mm"), "inventory");
    private static IBakedModel bakedModel;

    public static void initModel() {
        IModel model = ModelLoaderRegistry.getModelOrLogError(modelLoc,
                "Could not load grenande launcher projectile model");
        bakedModel = model.bake(TRSRTransformation.identity(), DefaultVertexFormats.ITEM, r -> {
            ...
            try {
                PngSizeInfo png = PngSizeInfo
                        .makeFromResource(Minecraft.getMinecraft().getResourceManager().getResource(texture));
    ...
    @Override
    public void doRender(Grenade40mmProjectile entity, double x, double y, double z, float entityYaw,
                         float partialTicks) {
