...
public class LayerHeldItemTranslateGun extends LayerHeldItem {
    ...
    public void doRenderLayer(EntityLivingBase entitylivingbaseIn, float limbSwing, float limbSwingAmount,
        ...
        if (!itemstack.isEmpty() || !itemstack1.isEmpty()) {
            ...
            this.renderHeldItemTranslateGun(entitylivingbaseIn, itemstack1,
                    ItemCameraTransforms.TransformType.THIRD_PERSON_RIGHT_HAND, EnumHandSide.RIGHT);
            this.renderHeldItemTranslateGun(entitylivingbaseIn, itemstack,
                    ItemCameraTransforms.TransformType.THIRD_PERSON_LEFT_HAND, EnumHandSide.LEFT);
            GlStateManager.popMatrix();
        }
    }

    protected void renderHeldItemTranslateGun(EntityLivingBase ent, ItemStack stack,
                                              ItemCameraTransforms.TransformType transformType, EnumHandSide handSide) {
        if (!stack.isEmpty()) {
            GlStateManager.pushMatrix();
    ...
    protected void setEntityTranslation(EntityLivingBase ent, boolean lefthand) {
        if (ent instanceof INPCTechgunsShooter) {
            ...
            GlStateManager.translate(lefthand ? -shooter.getWeaponPosX() : shooter.getWeaponPosX(),
                    shooter.getWeaponPosY(), shooter.getWeaponPosZ());
        }
    }
}
