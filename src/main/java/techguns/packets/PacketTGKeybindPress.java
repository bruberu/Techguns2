            ...
            if (message.buttonID == TGKeybindsID.TOGGLE_NIGHTVISION) {
                ...
                if (!props.enableNightVision) {
                    if (ply.getActivePotionEffect(MobEffects.NIGHT_VISION) != null) {
                        ...
                        ply.world.playSound(null, ply.posX, ply.posY + ply.getEyeHeight(), ply.posZ,
                                TGSounds.NIGHTVISION_OFF, SoundCategory.PLAYERS, 1f, 1f);
                        // Techguns.proxy.playSoundOnEntity(ply, TGSounds.NIGHTVISION_OFF, 1f, 1f, false, true, false,
                    ...
                    ply.world.playSound(null, ply.posX, ply.posY + ply.getEyeHeight(), ply.posZ,
                            TGSounds.NIGHTVISION_ON, SoundCategory.PLAYERS, 1f, 1f);

                }
                if (message.showMsg) {
                    TGPackets.network.sendTo(
                            new PacketShowKeybindConfirmMessage(message.buttonID, props.enableNightVision),
                            (EntityPlayerMP) ply);
                }
                ...
                if (message.showMsg) {
                    TGPackets.network.sendTo(
                            new PacketShowKeybindConfirmMessage(message.buttonID, props.enableSafemode),
                            (EntityPlayerMP) ply);
                }
                TGPackets.network.sendTo(new PacketTGExtendedPlayerSync(ply, props, true), (EntityPlayerMP) ply);
                ...
                if (!props.isJumpkeyPressed()) {
                    ...
                    TGPackets.network.sendTo(new PacketTGPlayerFieldSync(ply,
                            PacketTGPlayerFieldSync.FIELD_ENABLEJETPACK, props.enableJetpack), (EntityPlayerMP) ply);
                    if (message.showMsg) {
                        TGPackets.network.sendTo(
                                new PacketShowKeybindConfirmMessage(message.buttonID, props.enableJetpack),
                                (EntityPlayerMP) ply);
                    }
                    ...
                    if (!props.enableJetpack && props.isJumpkeyPressed()) {
                        ...
                        TGPackets.network
                                .sendToDimension(
                                        new PacketTGPlayerFieldSync(ply,
                                                PacketTGPlayerFieldSync.FIELD_JUMPBUTTONPRESSED, false),
                                        ply.world.provider.getDimension());
                    }
                }
                ...
                TGPackets.network.sendToDimension(
                        new PacketTGPlayerFieldSync(ply, PacketTGPlayerFieldSync.FIELD_JUMPBUTTONPRESSED, true),
                        ply.world.provider.getDimension());
                ...
                props.setJumpkeyPressed(false);
                TGPackets.network.sendToDimension(
                        new PacketTGPlayerFieldSync(ply, PacketTGPlayerFieldSync.FIELD_JUMPBUTTONPRESSED, false),
                        ply.world.provider.getDimension());
                ...
                if (message.showMsg) {
                    TGPackets.network.sendTo(
                            new PacketShowKeybindConfirmMessage(message.buttonID, props.enableStepAssist),
                            (EntityPlayerMP) ply);
                }
                TGPackets.network.sendTo(new PacketTGExtendedPlayerSync(ply, props, true), (EntityPlayerMP) ply);
            }
