...
public class PacketPlaySound implements IMessage {
    ...
    public PacketPlaySound(SoundEvent soundname, Entity entity, float volume, float pitch, boolean repeat,
                           boolean moving, boolean gunPosition, boolean playOnOwnPlayer, TGSoundCategory category) {
    ...
    public PacketPlaySound(SoundEvent soundname, Entity entity, float volume, float pitch, boolean repeat,
                           boolean moving, TGSoundCategory category) {
        if (entity == null) {
            this.entityId = -1;
        } else {
            this.entityId = entity.getEntityId();
    ...
    public PacketPlaySound(SoundEvent soundname, int posx, int posy, int posz, float volume, float pitch,
    ...
    public static class Handler extends HandlerTemplate<PacketPlaySound> {
        ...
        protected void handle(PacketPlaySound msg, MessageContext ctx) {
            ...
            if (msg.entityId != -1) {
                Techguns.proxy.handleSoundEvent(ply, msg.entityId, event, msg.volume, msg.pitch, msg.repeat, msg.moving,
                        msg.gunPosition, msg.playOnOwnPlayer, msg.category, msg.condition);
                // System.out.println("Received sound Event:"+msg.soundname+" play for own:"+msg.playOnOwnPlayer);
            } else {
                Techguns.proxy.playSoundOnPosition(event, msg.soundx, msg.soundy, msg.soundz, msg.volume, msg.pitch,
                        msg.repeat, msg.category);
            }
        }
    }
}
