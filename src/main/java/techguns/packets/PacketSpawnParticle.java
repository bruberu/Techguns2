...
public class PacketSpawnParticle implements IMessage {
    ...
    public PacketSpawnParticle(String name, double posX, double posY, double posZ, double motionX, double motionY,
                               double motionZ) {
        super();
        this.name = name;
        this.posX = posX;
    ...
    public static class Handler implements IMessageHandler<PacketSpawnParticle, IMessage> {
        ...
        private void handle(PacketSpawnParticle m, MessageContext ctx) {
            ClientProxy.get().createFX(m.name, TGPackets.getPlayerFromContext(ctx).world, m.posX, m.posY, m.posZ,
                    m.motionX, m.motionY, m.motionZ, m.scale);
        }
    }
}
