...
public class GunFiredMessage implements IMessage {
    ...
    public GunFiredMessage(EntityLivingBase shooter, int firetime, int muzzleflashtime, int attackType,
                           boolean checkRecoil, boolean offhand) {
    ...
    public static class Handler implements IMessageHandler<GunFiredMessage, IMessage> {
        ...
        private void handle(GunFiredMessage message, MessageContext ctx) {
            ...
            if (shooter != null) {
                ...
                if (!message.checkRecoil ||
                        !ShooterValues.isStillRecoiling(shooter, message.offhand, message.attackType)) {

                    ShooterValues.setRecoiltime(shooter, message.offhand, System.currentTimeMillis() + message.time,
                            message.time, message.attackType);
                    ShooterValues.setMuzzleFlashTime(shooter, message.offhand,
                            System.currentTimeMillis() + message.muzzleflashtime, message.muzzleflashtime);
