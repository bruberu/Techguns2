...
public class TechgunsASMTransformer implements IClassTransformer {
    ...
    static {
        addMapping("TransformType", "bwa$b",
    ...
    private static String getHookSignature(boolean deobf) {
        return "(L" + getMapping("ItemStack", deobf) + ";L" + getMapping("ELB", deobf) + ";L" +
                getMapping("TransformType", deobf) + ";Z)Z";
    }
    ...
    private static String getHookSignature2(boolean deobf) {
        // return
        // "(L"+getMapping("EntityPlayer",deobf)+";FL"+getMapping("ItemStack",deobf)+";L"+getMapping("ItemStack",deobf)+";)F";
        return "()F";
    }
            ...
            if (m.name.equals(targetMethodName) && m.desc.equals(signature)) {
                ...
                for (AbstractInsnNode instruction : m.instructions.toArray()) {
                    ...
                    if (instruction.getOpcode() == INVOKEVIRTUAL) {
                        ...
                        if (instruction.getPrevious() != null && instruction.getPrevious().getOpcode() == ALOAD &&
                                ((VarInsnNode) instruction.getPrevious()).var == 17) {
                            if (instruction.getPrevious().getPrevious() != null &&
                                    instruction.getPrevious().getPrevious().getOpcode() == ALOAD &&
                                    ((VarInsnNode) instruction.getPrevious().getPrevious()).var == 10) {
                                ...
                                toInsert.add(new FieldInsnNode(GETSTATIC, getMapping("TransformType", deobf),
                                        getMapping("GROUND", deobf), "L" + getMapping("TransformType", deobf) + ";"));
                                toInsert.add(new InsnNode(ICONST_0));
                                toInsert.add(new MethodInsnNode(INVOKESTATIC, ITEMRENDERHACK, "renderItem",
                                        getHookSignature(deobf), false));
    ...
    protected static void patchRenderItem_patchMethodRenderItem(MethodNode method, boolean deobf) {
        ...
        if (targetNode != null && targetNode2 != null) {
             ...
             * mv.visitMethodInsn(INVOKESTATIC, "techguns/client/render/ItemRenderHack", "renderItem",
             * "(Lnet/minecraft/item/ItemStack;Lnet/minecraft/entity/EntityLivingBase;Lnet/minecraft/client/renderer/block/model/ItemCameraTransforms$TransformType;Z)Z",
             * false);
             * Label l1 = new Label();
            ...
            toInsert.add(
                    new MethodInsnNode(INVOKESTATIC, ITEMRENDERHACK, "renderItem", getHookSignature(deobf), false));
            LabelNode labelNode1 = new LabelNode();
            toInsert.add(new JumpInsnNode(IFNE, labelNode1));
    ...
    protected static void patchRenderItem_patchMethodRenderItemGUI(MethodNode method, boolean deobf) {
        ...
        for (AbstractInsnNode instruction : method.instructions.toArray()) {
            ...
            if (instruction.getOpcode() == INVOKEVIRTUAL) {
                targetNode = instruction;
                if (instruction.getPrevious() != null && instruction.getPrevious().getOpcode() == ALOAD &&
                        ((VarInsnNode) instruction.getPrevious()).var == 4) {

                    if (instruction.getPrevious().getPrevious() != null &&
                            instruction.getPrevious().getPrevious().getOpcode() == ALOAD &&
                            ((VarInsnNode) instruction.getPrevious().getPrevious()).var == 1) {

                        if (instruction.getPrevious().getPrevious().getPrevious() != null &&
                                instruction.getPrevious().getPrevious().getPrevious().getOpcode() == ALOAD &&
                                ((VarInsnNode) instruction.getPrevious().getPrevious().getPrevious()).var == 0) {
        ...
        if (targetNode != null && targetNode2 != null) {
            ...
            toInsert.add(new InsnNode(ACONST_NULL));
            // toInsert.add(new VarInsnNode(ALOAD, 2));
            toInsert.add(new FieldInsnNode(GETSTATIC, getMapping("TransformType", deobf), getMapping("GUI", deobf),
                    "L" + getMapping("TransformType", deobf) + ";"));
            toInsert.add(new InsnNode(ICONST_0));
            toInsert.add(
                    new MethodInsnNode(INVOKESTATIC, ITEMRENDERHACK, "renderItem", getHookSignature(deobf), false));
            LabelNode labelNode1 = new LabelNode();
    ...
    protected static void patchRenderItem_patchMethodRenderModel(MethodNode method, boolean deobf) {
        ...
        for (AbstractInsnNode instruction : method.instructions.toArray()) {
            ...
            if (instruction.getOpcode() == ALOAD) {
                if (((VarInsnNode) instruction).var == 0 && instruction.getNext().getOpcode() == ALOAD &&
    ...
    protected static void patchItemRenderer_updateEquippedItem(MethodNode method, boolean deobf) {
        ...
        for (AbstractInsnNode instruction : method.instructions.toArray()) {
            ...
            if (instruction.getOpcode() == FSTORE && ((VarInsnNode) instruction).var == 4) {
                if (instruction.getPrevious() != null && instruction.getPrevious().getOpcode() == INVOKEVIRTUAL) {
                    ...
                    if (instruction.getPrevious().getPrevious() != null &&
                            instruction.getPrevious().getPrevious().getOpcode() == FCONST_1) {

                        if (instruction.getPrevious().getPrevious().getPrevious() != null &&
                                instruction.getPrevious().getPrevious().getPrevious().getOpcode() == ALOAD &&
                                ((VarInsnNode) instruction.getPrevious().getPrevious().getPrevious()).var == 1) {
                            targetNode = instruction;
                            break;
                        }
                    }
        ...
        if (targetNode != null) {
            ...
            toInsert.add(new MethodInsnNode(INVOKESTATIC, ITEMRENDERHACK, "getAttackStrengthForRendering",
                    getHookSignature2(deobf), false));
